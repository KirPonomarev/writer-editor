...
import { evaluateX24Ws01PostMergeBaselineReconfirmState } from './x24-ws01-post-merge-baseline-reconfirm-state.mjs';

const DEFAULT_OUTPUT_DIR = 'docs/OPS/EVIDENCE/X24_CONTOUR/TICKET_01';
const DEFAULT_STATUS_PATH = 'docs/OPS/STATUS/X24_WS01_POST_MERGE_BASELINE_RECONFIRM_v1.json';
...
function main() {
  ...
  const run1 = evaluateX24Ws01PostMergeBaselineReconfirmState({ repoRoot });
  const run2 = evaluateX24Ws01PostMergeBaselineReconfirmState({ repoRoot });
  const run3 = evaluateX24Ws01PostMergeBaselineReconfirmState({ repoRoot });
  ...
  const summary = {
    ...
    objective: 'RECONFIRM_POST_MERGE_BASELINE_AND_PREPARE_STABLE_RELEASE_HANDOFF_WITHOUT_CORE_CHANGES',
  ...
  const statusDoc = {
    version: 1,
    token: 'X24_WS01_POST_MERGE_BASELINE_RECONFIRM_OK',
    ...
    headBindingMismatchCount: summary.counts.headBindingMismatchCount,
    branchBindingMismatchCount: summary.counts.branchBindingMismatchCount,
    worktreeDirtyViolationCount: summary.counts.worktreeDirtyViolationCount,
    ...
    postMergeStabilityHash: run1.baseline.postMergeStabilityHash,
    status,
  ...
  writeJson(path.join(outputDir, 'negative-results.json'), {
    target: {
      NEXT_TZ_NEGATIVE_01: 'HEAD_BINDING_MISMATCH_EXPECT_REJECT',
      NEXT_TZ_NEGATIVE_02: 'REQUIRED_BLOCKING_TOKEN_GAP_EXPECT_REJECT',
      NEXT_TZ_NEGATIVE_03: 'MODE_DISPOSITION_DRIFT_EXPECT_REJECT',
      NEXT_TZ_NEGATIVE_04: 'OFFLINE_ATTESTATION_CHAIN_BREAK_EXPECT_REJECT',
      NEXT_TZ_NEGATIVE_05: 'NON_DETERMINISTIC_STABILITY_PACKET_OUTPUT_EXPECT_REJECT',
    },
  ...
  writeJson(path.join(outputDir, 'positive-results.json'), {
    target: {
      NEXT_TZ_POSITIVE_01: 'BASELINE_STATE_STABLE_AND_VALID',
      NEXT_TZ_POSITIVE_02: 'REQUIRED_RELEASE_SURFACE_STILL_COHERENT',
      NEXT_TZ_POSITIVE_03: 'STABILITY_PACKET_OUTPUT_DETERMINISTIC',
    },
    results: run1.positiveResults,
    details: run1.positiveDetails,
  });

  writeJson(path.join(outputDir, 'post-merge-stability-summary.json'), {
    ...run1.baseline.postMergeStabilitySummary,
    postMergeStabilityHash: run1.baseline.postMergeStabilityHash,
  ...
  writeJson(path.join(outputDir, 'final-dod-acceptance-summary.json'), {
    ...
    finalGate: summary.status === 'PASS' ? 'GO_TO_X24_WS02_ONLY_IF_ALL_DOD_AND_ACCEPTANCE_TRUE' : 'HOLD',
  });
