...
const TOKEN_NAME = 'X41_WS03_EVIDENCE_MAINTENANCE_OK';
const EXPECTED_CANON_VERSION = 'v3.13a-final';

const PACKET_FILE_ALLOWLIST = new Set([
  'docs/OPS/EVIDENCE/X41_CONTOUR/',
  'docs/OPS/EVIDENCE/X41_CONTOUR/TICKET_03/',
  'docs/OPS/EVIDENCE/X41_CONTOUR/TICKET_03/negative-results.json',
  'docs/OPS/EVIDENCE/X41_CONTOUR/TICKET_03/positive-results.json',
  'docs/OPS/EVIDENCE/X41_CONTOUR/TICKET_03/final-dod-acceptance-summary.json',
  'docs/OPS/EVIDENCE/X41_CONTOUR/TICKET_03/summary.json',
  'docs/OPS/EVIDENCE/X41_CONTOUR/TICKET_03/ticket-meta.json',
  'docs/OPS/STATUS/X41_WS03_EVIDENCE_MAINTENANCE_STATUS_V1.json',
  'scripts/ops/x41-ws03-evidence-maintenance-state.mjs',
  'scripts/ops/x41-ws03-evidence-maintenance-report.mjs',
  'test/contracts/x41-ws03-evidence-maintenance.contract.test.js',
]);
...
function evaluateEvidenceChain(repoRoot) {
  const nextRecordPath = path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V26.json');
  const x40SignedPath = path.join(repoRoot, 'docs/OPS/STATUS/X40_CONTOUR_CLOSEOUT_SIGNED_V1.json');
  const x40FreezePath = path.join(repoRoot, 'docs/OPS/EVIDENCE/X40_CONTOUR/x40-contour-freeze-packet-v1.json');

  const nextRecord = readJsonObject(nextRecordPath);
  const x40Signed = readJsonObject(x40SignedPath);
  const x40Freeze = readJsonObject(x40FreezePath);

  const missing = [];
  if (!nextRecord) missing.push('NEXT_RECORD_MISSING');
  if (!x40Signed) missing.push('X40_SIGNED_MISSING');
  if (!x40Freeze) missing.push('X40_FREEZE_MISSING');

  const nextRecordRefOk = normalizeString(nextRecord?.dependsOn?.x40CloseoutArtifactRef)
    === 'docs/OPS/STATUS/X40_CONTOUR_CLOSEOUT_SIGNED_V1.json';

  const signedRefOk = normalizeString(x40Signed?.evidencePacketRef)
    === 'docs/OPS/EVIDENCE/X40_CONTOUR/x40-contour-freeze-packet-v1.json';

  let hashMatch = false;
  if (x40Signed && fs.existsSync(x40FreezePath)) {
    const actual = createHash('sha256').update(fs.readFileSync(x40FreezePath)).digest('hex');
    hashMatch = normalizeString(x40Signed.evidencePacketSha256) === `sha256:${actual}`;
  }
...
function evaluateEvidenceRefCompleteness(repoRoot) {
  const requiredRefs = [
    'docs/OPS/STATUS/X41_WS01_NEXT_CONTOUR_ALIGNMENT_STATUS_V1.json',
    'docs/OPS/STATUS/X41_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json',
    'docs/OPS/EVIDENCE/X41_CONTOUR/TICKET_01/summary.json',
    'docs/OPS/EVIDENCE/X41_CONTOUR/TICKET_01/final-dod-acceptance-summary.json',
    'docs/OPS/EVIDENCE/X41_CONTOUR/TICKET_02/summary.json',
    'docs/OPS/EVIDENCE/X41_CONTOUR/TICKET_02/final-dod-acceptance-summary.json',
    'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V26.json',
    'docs/OPS/STATUS/X40_CONTOUR_CLOSEOUT_SIGNED_V1.json',
    'docs/OPS/EVIDENCE/X40_CONTOUR/x40-contour-freeze-packet-v1.json',
  ];
...
function evaluateLinkStaleness(repoRoot) {
  const ws02StatusPath = path.join(repoRoot, 'docs/OPS/STATUS/X41_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json');
...
export function evaluateX41Ws03EvidenceMaintenanceState(input = {}) {
  ...
  const nextRecord = readJsonObject(path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V26.json'));
  ...
  return {
    ...
    acceptance,
    determinism,
    finalGate: 'GO_TO_X41_WS04_ONLY_IF_ALL_DOD_AND_ACCEPTANCE_TRUE',
...
function main() {
  ...
  const state = evaluateX41Ws03EvidenceMaintenanceState({ repoRoot: process.cwd() });

  if (args.json) {
    process.stdout.write(`${stableStringify(state)}\n`);
  } else {
    process.stdout.write(`X41_WS03_EVIDENCE_MAINTENANCE_OK=${state.X41_WS03_EVIDENCE_MAINTENANCE_OK}\n`);
