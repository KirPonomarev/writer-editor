...
const TOKEN_NAME = 'X37_WS03_EVIDENCE_MAINTENANCE_OK';
const EXPECTED_CANON_VERSION = 'v3.13a-final';

const PACKET_FILE_ALLOWLIST = new Set([
  'docs/OPS/EVIDENCE/X37_CONTOUR/',
  'docs/OPS/EVIDENCE/X37_CONTOUR/TICKET_03/',
  'docs/OPS/EVIDENCE/X37_CONTOUR/TICKET_03/negative-results.json',
  'docs/OPS/EVIDENCE/X37_CONTOUR/TICKET_03/positive-results.json',
  'docs/OPS/EVIDENCE/X37_CONTOUR/TICKET_03/final-dod-acceptance-summary.json',
  'docs/OPS/EVIDENCE/X37_CONTOUR/TICKET_03/summary.json',
  'docs/OPS/EVIDENCE/X37_CONTOUR/TICKET_03/ticket-meta.json',
  'docs/OPS/STATUS/X37_WS03_EVIDENCE_MAINTENANCE_STATUS_V1.json',
  'scripts/ops/x37-ws03-evidence-maintenance-state.mjs',
  'scripts/ops/x37-ws03-evidence-maintenance-report.mjs',
  'test/contracts/x37-ws03-evidence-maintenance.contract.test.js',
]);
...
function evaluateEvidenceChain(repoRoot) {
  ...
  const x36SignedPath = path.join(repoRoot, 'docs/OPS/STATUS/X36_CONTOUR_CLOSEOUT_SIGNED_V1.json');
  const x36FreezePath = path.join(repoRoot, 'docs/OPS/EVIDENCE/X36_CONTOUR/x36-contour-freeze-packet-v1.json');

  const nextRecord = readJsonObject(nextRecordPath);
  const x36Signed = readJsonObject(x36SignedPath);
  const x36Freeze = readJsonObject(x36FreezePath);

  const missing = [];
  if (!nextRecord) missing.push('NEXT_RECORD_MISSING');
  if (!x36Signed) missing.push('X36_SIGNED_MISSING');
  if (!x36Freeze) missing.push('X36_FREEZE_MISSING');

  const nextRecordRefOk = normalizeString(nextRecord?.dependsOn?.x36CloseoutArtifactRef)
    === 'docs/OPS/STATUS/X36_CONTOUR_CLOSEOUT_SIGNED_V1.json';

  const signedRefOk = normalizeString(x36Signed?.evidencePacketRef)
    === 'docs/OPS/EVIDENCE/X36_CONTOUR/x36-contour-freeze-packet-v1.json';

  let hashMatch = false;
  if (x36Signed && fs.existsSync(x36FreezePath)) {
    const actual = createHash('sha256').update(fs.readFileSync(x36FreezePath)).digest('hex');
    hashMatch = normalizeString(x36Signed.evidencePacketSha256) === `sha256:${actual}`;
  }
...
function evaluateEvidenceRefCompleteness(repoRoot) {
  const requiredRefs = [
    ...
    'docs/OPS/STATUS/X37_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json',
    ...
    'docs/OPS/EVIDENCE/X37_CONTOUR/TICKET_02/summary.json',
    'docs/OPS/EVIDENCE/X37_CONTOUR/TICKET_02/final-dod-acceptance-summary.json',
    'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V22.json',
    'docs/OPS/STATUS/X36_CONTOUR_CLOSEOUT_SIGNED_V1.json',
    'docs/OPS/EVIDENCE/X36_CONTOUR/x36-contour-freeze-packet-v1.json',
  ];
...
function evaluateLinkStaleness(repoRoot) {
  const ws02StatusPath = path.join(repoRoot, 'docs/OPS/STATUS/X37_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json');
...
export function evaluateX37Ws03EvidenceMaintenanceState(input = {}) {
  ...
  const nextRecord = readJsonObject(path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V22.json'));
  ...
  return {
    ...
    acceptance,
    determinism,
    finalGate: 'GO_TO_X37_WS04_ONLY_IF_ALL_DOD_AND_ACCEPTANCE_TRUE',
...
function main() {
  ...
  const state = evaluateX37Ws03EvidenceMaintenanceState({ repoRoot: process.cwd() });

  if (args.json) {
    process.stdout.write(`${stableStringify(state)}\n`);
  } else {
    process.stdout.write(`X37_WS03_EVIDENCE_MAINTENANCE_OK=${state.X37_WS03_EVIDENCE_MAINTENANCE_OK}\n`);
