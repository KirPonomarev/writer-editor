...
const TOKEN_NAME = 'X31_WS03_EVIDENCE_MAINTENANCE_OK';
const EXPECTED_CANON_VERSION = 'v3.13a-final';

const PACKET_FILE_ALLOWLIST = new Set([
  'docs/OPS/EVIDENCE/X31_CONTOUR/',
  'docs/OPS/EVIDENCE/X31_CONTOUR/TICKET_03/',
  'docs/OPS/EVIDENCE/X31_CONTOUR/TICKET_03/negative-results.json',
  'docs/OPS/EVIDENCE/X31_CONTOUR/TICKET_03/positive-results.json',
  'docs/OPS/EVIDENCE/X31_CONTOUR/TICKET_03/final-dod-acceptance-summary.json',
  'docs/OPS/EVIDENCE/X31_CONTOUR/TICKET_03/summary.json',
  'docs/OPS/EVIDENCE/X31_CONTOUR/TICKET_03/ticket-meta.json',
  'docs/OPS/STATUS/X31_WS03_EVIDENCE_MAINTENANCE_STATUS_V1.json',
  'scripts/ops/x31-ws03-evidence-maintenance-state.mjs',
  'scripts/ops/x31-ws03-evidence-maintenance-report.mjs',
  'test/contracts/x31-ws03-evidence-maintenance.contract.test.js',
]);
...
function evaluateEvidenceChain(repoRoot) {
  const nextRecordPath = path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V16.json');
  const x29SignedPath = path.join(repoRoot, 'docs/OPS/STATUS/X30_CONTOUR_CLOSEOUT_SIGNED_V1.json');
  const x29FreezePath = path.join(repoRoot, 'docs/OPS/EVIDENCE/X30_CONTOUR/x30-contour-freeze-packet-v1.json');

  const nextRecord = readJsonObject(nextRecordPath);
  const x29Signed = readJsonObject(x29SignedPath);
  const x29Freeze = readJsonObject(x29FreezePath);

  const missing = [];
  if (!nextRecord) missing.push('NEXT_RECORD_MISSING');
  if (!x29Signed) missing.push('X29_SIGNED_MISSING');
  if (!x29Freeze) missing.push('X29_FREEZE_MISSING');

  const nextRecordRefOk = normalizeString(nextRecord?.dependsOn?.x30CloseoutArtifactRef)
    === 'docs/OPS/STATUS/X30_CONTOUR_CLOSEOUT_SIGNED_V1.json';

  const signedRefOk = normalizeString(x29Signed?.evidencePacketRef)
    === 'docs/OPS/EVIDENCE/X30_CONTOUR/x30-contour-freeze-packet-v1.json';

  let hashMatch = false;
  if (x29Signed && fs.existsSync(x29FreezePath)) {
    const actual = createHash('sha256').update(fs.readFileSync(x29FreezePath)).digest('hex');
    hashMatch = normalizeString(x29Signed.evidencePacketSha256) === `sha256:${actual}`;
  }
...
function evaluateEvidenceRefCompleteness(repoRoot) {
  const requiredRefs = [
    'docs/OPS/STATUS/X31_WS01_NEXT_CONTOUR_ALIGNMENT_STATUS_V1.json',
    'docs/OPS/STATUS/X31_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json',
    'docs/OPS/EVIDENCE/X31_CONTOUR/TICKET_01/summary.json',
    'docs/OPS/EVIDENCE/X31_CONTOUR/TICKET_01/final-dod-acceptance-summary.json',
    'docs/OPS/EVIDENCE/X31_CONTOUR/TICKET_02/summary.json',
    'docs/OPS/EVIDENCE/X31_CONTOUR/TICKET_02/final-dod-acceptance-summary.json',
    'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V16.json',
    'docs/OPS/STATUS/X30_CONTOUR_CLOSEOUT_SIGNED_V1.json',
    'docs/OPS/EVIDENCE/X30_CONTOUR/x30-contour-freeze-packet-v1.json',
  ];
...
function evaluateLinkStaleness(repoRoot) {
  const ws02StatusPath = path.join(repoRoot, 'docs/OPS/STATUS/X31_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json');
...
export function evaluateX31Ws03EvidenceMaintenanceState(input = {}) {
  ...
  const nextRecord = readJsonObject(path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V16.json'));
  ...
  return {
    ...
    acceptance,
    determinism,
    finalGate: 'GO_TO_X31_WS04_ONLY_IF_ALL_DOD_AND_ACCEPTANCE_TRUE',
...
function main() {
  ...
  const state = evaluateX31Ws03EvidenceMaintenanceState({ repoRoot: process.cwd() });

  if (args.json) {
    process.stdout.write(`${stableStringify(state)}\n`);
  } else {
    process.stdout.write(`X31_WS03_EVIDENCE_MAINTENANCE_OK=${state.X31_WS03_EVIDENCE_MAINTENANCE_OK}\n`);
