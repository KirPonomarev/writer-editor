...
import { evaluateX66Ws03EvidenceMaintenanceState } from './x66-ws03-evidence-maintenance-state.mjs';

const DEFAULT_OUTPUT_DIR = 'docs/OPS/EVIDENCE/X66_CONTOUR/TICKET_03';
const DEFAULT_STATUS_PATH = 'docs/OPS/STATUS/X66_WS03_EVIDENCE_MAINTENANCE_STATUS_V1.json';
const DEFAULT_RUN_ID = 'TZ_X66_WS03_EVIDENCE_MAINTENANCE_001';
...
function main() {
  ...
  const run1 = evaluateX66Ws03EvidenceMaintenanceState({ repoRoot });
  const run2 = evaluateX66Ws03EvidenceMaintenanceState({ repoRoot });
  const run3 = evaluateX66Ws03EvidenceMaintenanceState({ repoRoot });
  ...
  const summary = {
    ...
    objective: 'MAINTAIN_EVIDENCE_CHAIN_INTEGRITY_AND_DETERMINISM_FOR_CURRENT_CONTOUR',
    ...
    evidenceMaintenanceSummary: {
      evidenceChainIntegrityTrue: run1.positiveResults.NEXT_TZ_POSITIVE_01,
      evidenceRefCompletenessTrue: run1.positiveResults.NEXT_TZ_POSITIVE_02,
      evidenceOutputDeterministic: run1.positiveResults.NEXT_TZ_POSITIVE_03,
      evidenceChainBreakCount: run1.counts.evidenceChainBreakCount,
      missingEvidenceRefCount: run1.counts.missingEvidenceRefCount,
      hashMismatchCount: run1.counts.hashMismatchCount,
      staleEvidenceLinkCount: run1.counts.staleEvidenceLinkCount,
      nonDeterministicEvidenceOutputCount: run1.counts.nonDeterministicEvidenceOutputCount,
