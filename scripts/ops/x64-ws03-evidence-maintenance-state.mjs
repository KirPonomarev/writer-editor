...
const TOKEN_NAME = 'X64_WS03_EVIDENCE_MAINTENANCE_OK';
const EXPECTED_CANON_VERSION = 'v3.13a-final';

const PACKET_FILE_ALLOWLIST = new Set([
  'docs/OPS/EVIDENCE/X64_CONTOUR/',
  'docs/OPS/EVIDENCE/X64_CONTOUR/TICKET_03/',
  'docs/OPS/EVIDENCE/X64_CONTOUR/TICKET_03/negative-results.json',
  'docs/OPS/EVIDENCE/X64_CONTOUR/TICKET_03/positive-results.json',
  'docs/OPS/EVIDENCE/X64_CONTOUR/TICKET_03/final-dod-acceptance-summary.json',
  'docs/OPS/EVIDENCE/X64_CONTOUR/TICKET_03/summary.json',
  'docs/OPS/EVIDENCE/X64_CONTOUR/TICKET_03/ticket-meta.json',
  'docs/OPS/STATUS/X64_WS03_EVIDENCE_MAINTENANCE_STATUS_V1.json',
  'scripts/ops/x64-ws03-evidence-maintenance-state.mjs',
  'scripts/ops/x64-ws03-evidence-maintenance-report.mjs',
  'test/contracts/x64-ws03-evidence-maintenance.contract.test.js',
]);
...
function evaluateEvidenceChain(repoRoot) {
  const nextRecordPath = path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V47.json');
  const x63SignedPath = path.join(repoRoot, 'docs/OPS/STATUS/X63_CONTOUR_CLOSEOUT_SIGNED_V1.json');
  const x63FreezePath = path.join(repoRoot, 'docs/OPS/EVIDENCE/X63_CONTOUR/x63-contour-freeze-packet-v1.json');

  const nextRecord = readJsonObject(nextRecordPath);
  const x63Signed = readJsonObject(x63SignedPath);
  const x63Freeze = readJsonObject(x63FreezePath);

  const missing = [];
  if (!nextRecord) missing.push('NEXT_RECORD_MISSING');
  if (!x63Signed) missing.push('X63_SIGNED_MISSING');
  if (!x63Freeze) missing.push('X63_FREEZE_MISSING');

  const nextRecordRefOk = normalizeString(nextRecord?.dependsOn?.x63CloseoutArtifactRef)
    === 'docs/OPS/STATUS/X63_CONTOUR_CLOSEOUT_SIGNED_V1.json';

  const signedRefOk = normalizeString(x63Signed?.evidencePacketRef)
    === 'docs/OPS/EVIDENCE/X63_CONTOUR/x63-contour-freeze-packet-v1.json';

  let hashMatch = false;
  if (x63Signed && fs.existsSync(x63FreezePath)) {
    const actual = createHash('sha256').update(fs.readFileSync(x63FreezePath)).digest('hex');
    hashMatch = normalizeString(x63Signed.evidencePacketSha256) === `sha256:${actual}`;
  }
...
function evaluateEvidenceRefCompleteness(repoRoot) {
  const requiredRefs = [
    ...
    'docs/OPS/EVIDENCE/X64_CONTOUR/TICKET_01/summary.json',
    'docs/OPS/EVIDENCE/X64_CONTOUR/TICKET_01/final-dod-acceptance-summary.json',
    'docs/OPS/EVIDENCE/X64_CONTOUR/TICKET_02/summary.json',
    'docs/OPS/EVIDENCE/X64_CONTOUR/TICKET_02/final-dod-acceptance-summary.json',
...
function evaluateLinkStaleness(repoRoot) {
  const ws02StatusPath = path.join(repoRoot, 'docs/OPS/STATUS/X64_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json');
...
export function evaluateX64Ws03EvidenceMaintenanceState(input = {}) {
  ...
  const nextRecord = readJsonObject(path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V47.json'));
  ...
  return {
    ...
    acceptance,
    determinism,
    finalGate: 'GO_TO_X64_WS04_ONLY_IF_ALL_DOD_AND_ACCEPTANCE_TRUE',
...
function main() {
  ...
  const state = evaluateX64Ws03EvidenceMaintenanceState({ repoRoot: process.cwd() });

  if (args.json) {
    process.stdout.write(`${stableStringify(state)}\n`);
  } else {
    process.stdout.write(`X64_WS03_EVIDENCE_MAINTENANCE_OK=${state.X64_WS03_EVIDENCE_MAINTENANCE_OK}\n`);
