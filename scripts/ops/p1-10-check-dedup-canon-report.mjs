...
const DEFAULT_OUTPUT_DIR = 'docs/OPS/EVIDENCE/P1_CONTOUR/TICKET_10';
const DEFAULT_STATUS_PATH = 'docs/OPS/STATUS/CHECK_DEDUP_CANON_v3.json';
...
function main() {
  ...
  const uniqueSignalProofOk = state.uniqueSignalMap.length > 0
    && state.duplicateChecksBeforeAfter.after.duplicateSignalCount === 0;
  const gates = {
    p1_10_duplicate_signal_reduction_check: state.duplicateChecksBeforeAfter.duplicateReductionOk ? 'PASS' : 'FAIL',
    p1_10_unique_signal_proof_check: uniqueSignalProofOk ? 'PASS' : 'FAIL',
    p1_10_risk_weighted_dedup_check: state.riskWeightedDedupProof.ok ? 'PASS' : 'FAIL',
