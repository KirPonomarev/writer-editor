...
const TOKEN_NAME = 'X59_WS03_EVIDENCE_MAINTENANCE_OK';
const EXPECTED_CANON_VERSION = 'v3.13a-final';

const PACKET_FILE_ALLOWLIST = new Set([
  'docs/OPS/EVIDENCE/X59_CONTOUR/',
  'docs/OPS/EVIDENCE/X59_CONTOUR/TICKET_03/',
  'docs/OPS/EVIDENCE/X59_CONTOUR/TICKET_03/negative-results.json',
  'docs/OPS/EVIDENCE/X59_CONTOUR/TICKET_03/positive-results.json',
  'docs/OPS/EVIDENCE/X59_CONTOUR/TICKET_03/final-dod-acceptance-summary.json',
  'docs/OPS/EVIDENCE/X59_CONTOUR/TICKET_03/summary.json',
  'docs/OPS/EVIDENCE/X59_CONTOUR/TICKET_03/ticket-meta.json',
  'docs/OPS/STATUS/X59_WS03_EVIDENCE_MAINTENANCE_STATUS_V1.json',
  'scripts/ops/x59-ws03-evidence-maintenance-state.mjs',
  'scripts/ops/x59-ws03-evidence-maintenance-report.mjs',
  'test/contracts/x59-ws03-evidence-maintenance.contract.test.js',
]);
...
function evaluateEvidenceChain(repoRoot) {
  const nextRecordPath = path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V43.json');
  const x58SignedPath = path.join(repoRoot, 'docs/OPS/STATUS/X58_CONTOUR_CLOSEOUT_SIGNED_V1.json');
  const x58FreezePath = path.join(repoRoot, 'docs/OPS/EVIDENCE/X58_CONTOUR/x58-contour-freeze-packet-v1.json');

  const nextRecord = readJsonObject(nextRecordPath);
  const x58Signed = readJsonObject(x58SignedPath);
  const x58Freeze = readJsonObject(x58FreezePath);

  const missing = [];
  if (!nextRecord) missing.push('NEXT_RECORD_MISSING');
  if (!x58Signed) missing.push('X58_SIGNED_MISSING');
  if (!x58Freeze) missing.push('X58_FREEZE_MISSING');

  const nextRecordRefOk = normalizeString(nextRecord?.dependsOn?.x58CloseoutArtifactRef)
    === 'docs/OPS/STATUS/X58_CONTOUR_CLOSEOUT_SIGNED_V1.json';

  const signedRefOk = normalizeString(x58Signed?.evidencePacketRef)
    === 'docs/OPS/EVIDENCE/X58_CONTOUR/x58-contour-freeze-packet-v1.json';

  let hashMatch = false;
  if (x58Signed && fs.existsSync(x58FreezePath)) {
    const actual = createHash('sha256').update(fs.readFileSync(x58FreezePath)).digest('hex');
    hashMatch = normalizeString(x58Signed.evidencePacketSha256) === `sha256:${actual}`;
  }
...
function evaluateEvidenceRefCompleteness(repoRoot) {
  const requiredRefs = [
    'docs/OPS/STATUS/X59_WS01_NEXT_CONTOUR_ALIGNMENT_STATUS_V1.json',
    'docs/OPS/STATUS/X59_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json',
    'docs/OPS/EVIDENCE/X59_CONTOUR/TICKET_01/summary.json',
    'docs/OPS/EVIDENCE/X59_CONTOUR/TICKET_01/final-dod-acceptance-summary.json',
    'docs/OPS/EVIDENCE/X59_CONTOUR/TICKET_02/summary.json',
    'docs/OPS/EVIDENCE/X59_CONTOUR/TICKET_02/final-dod-acceptance-summary.json',
...
function evaluateLinkStaleness(repoRoot) {
  const ws02StatusPath = path.join(repoRoot, 'docs/OPS/STATUS/X59_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json');
...
export function evaluateX59Ws03EvidenceMaintenanceState(input = {}) {
  ...
  const nextRecord = readJsonObject(path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V43.json'));
  ...
  return {
    ...
    acceptance,
    determinism,
    finalGate: 'GO_TO_X59_WS04_ONLY_IF_ALL_DOD_AND_ACCEPTANCE_TRUE',
...
function main() {
  ...
  const state = evaluateX59Ws03EvidenceMaintenanceState({ repoRoot: process.cwd() });

  if (args.json) {
    process.stdout.write(`${stableStringify(state)}\n`);
  } else {
    process.stdout.write(`X59_WS03_EVIDENCE_MAINTENANCE_OK=${state.X59_WS03_EVIDENCE_MAINTENANCE_OK}\n`);
