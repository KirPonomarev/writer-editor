...
const TOKEN_NAME = 'X29_WS03_EVIDENCE_MAINTENANCE_OK';
const EXPECTED_CANON_VERSION = 'v3.13a-final';

const PACKET_FILE_ALLOWLIST = new Set([
  'docs/OPS/EVIDENCE/X29_CONTOUR/',
  'docs/OPS/EVIDENCE/X29_CONTOUR/TICKET_03/',
  'docs/OPS/EVIDENCE/X29_CONTOUR/TICKET_03/negative-results.json',
  'docs/OPS/EVIDENCE/X29_CONTOUR/TICKET_03/positive-results.json',
  'docs/OPS/EVIDENCE/X29_CONTOUR/TICKET_03/final-dod-acceptance-summary.json',
  'docs/OPS/EVIDENCE/X29_CONTOUR/TICKET_03/summary.json',
  'docs/OPS/EVIDENCE/X29_CONTOUR/TICKET_03/ticket-meta.json',
  'docs/OPS/STATUS/X29_WS03_EVIDENCE_MAINTENANCE_STATUS_V1.json',
  'scripts/ops/x29-ws03-evidence-maintenance-state.mjs',
  'scripts/ops/x29-ws03-evidence-maintenance-report.mjs',
  'test/contracts/x29-ws03-evidence-maintenance.contract.test.js',
]);
...
function evaluateEvidenceChain(repoRoot) {
  const nextRecordPath = path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V14.json');
  const x28SignedPath = path.join(repoRoot, 'docs/OPS/STATUS/X28_CONTOUR_CLOSEOUT_SIGNED_V1.json');
  const x28FreezePath = path.join(repoRoot, 'docs/OPS/EVIDENCE/X28_CONTOUR/x28-contour-freeze-packet-v1.json');

  const nextRecord = readJsonObject(nextRecordPath);
  const x28Signed = readJsonObject(x28SignedPath);
  const x28Freeze = readJsonObject(x28FreezePath);

  const missing = [];
  if (!nextRecord) missing.push('NEXT_RECORD_MISSING');
  if (!x28Signed) missing.push('X28_SIGNED_MISSING');
  if (!x28Freeze) missing.push('X28_FREEZE_MISSING');

  const nextRecordRefOk = normalizeString(nextRecord?.dependsOn?.x28CloseoutArtifactRef)
    === 'docs/OPS/STATUS/X28_CONTOUR_CLOSEOUT_SIGNED_V1.json';

  const signedRefOk = normalizeString(x28Signed?.evidencePacketRef)
    === 'docs/OPS/EVIDENCE/X28_CONTOUR/x28-contour-freeze-packet-v1.json';

  let hashMatch = false;
  if (x28Signed && fs.existsSync(x28FreezePath)) {
    const actual = createHash('sha256').update(fs.readFileSync(x28FreezePath)).digest('hex');
    hashMatch = normalizeString(x28Signed.evidencePacketSha256) === `sha256:${actual}`;
  }
...
function evaluateEvidenceRefCompleteness(repoRoot) {
  const requiredRefs = [
    'docs/OPS/STATUS/X29_WS01_NEXT_CONTOUR_ALIGNMENT_STATUS_V1.json',
    'docs/OPS/STATUS/X29_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json',
    'docs/OPS/EVIDENCE/X29_CONTOUR/TICKET_01/summary.json',
    'docs/OPS/EVIDENCE/X29_CONTOUR/TICKET_01/final-dod-acceptance-summary.json',
    'docs/OPS/EVIDENCE/X29_CONTOUR/TICKET_02/summary.json',
    'docs/OPS/EVIDENCE/X29_CONTOUR/TICKET_02/final-dod-acceptance-summary.json',
    'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V14.json',
    'docs/OPS/STATUS/X28_CONTOUR_CLOSEOUT_SIGNED_V1.json',
    'docs/OPS/EVIDENCE/X28_CONTOUR/x28-contour-freeze-packet-v1.json',
  ];
...
function evaluateLinkStaleness(repoRoot) {
  const ws02StatusPath = path.join(repoRoot, 'docs/OPS/STATUS/X29_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json');
...
export function evaluateX29Ws03EvidenceMaintenanceState(input = {}) {
  ...
  const nextRecord = readJsonObject(path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V14.json'));
  ...
  return {
    ...
    acceptance,
    determinism,
    finalGate: 'GO_TO_X29_WS04_ONLY_IF_ALL_DOD_AND_ACCEPTANCE_TRUE',
...
function main() {
  ...
  const state = evaluateX29Ws03EvidenceMaintenanceState({ repoRoot: process.cwd() });

  if (args.json) {
    process.stdout.write(`${stableStringify(state)}\n`);
  } else {
    process.stdout.write(`X29_WS03_EVIDENCE_MAINTENANCE_OK=${state.X29_WS03_EVIDENCE_MAINTENANCE_OK}\n`);
