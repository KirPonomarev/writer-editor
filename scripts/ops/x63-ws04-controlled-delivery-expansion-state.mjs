...
const TOKEN_NAME = 'X63_WS04_CONTROLLED_DELIVERY_EXPANSION_OK';
...
const PACKET_FILE_ALLOWLIST = new Set([
  ...
  'docs/OPS/EVIDENCE/X63_CONTOUR/TICKET_04/',
  'docs/OPS/EVIDENCE/X63_CONTOUR/TICKET_04/negative-results.json',
  'docs/OPS/EVIDENCE/X63_CONTOUR/TICKET_04/positive-results.json',
  'docs/OPS/EVIDENCE/X63_CONTOUR/TICKET_04/final-dod-acceptance-summary.json',
  'docs/OPS/EVIDENCE/X63_CONTOUR/TICKET_04/summary.json',
  'docs/OPS/EVIDENCE/X63_CONTOUR/TICKET_04/ticket-meta.json',
  'docs/OPS/STATUS/X63_WS04_CONTROLLED_DELIVERY_EXPANSION_STATUS_V1.json',
  'scripts/ops/x63-ws04-controlled-delivery-expansion-state.mjs',
  'scripts/ops/x63-ws04-controlled-delivery-expansion-report.mjs',
  'test/contracts/x63-ws04-controlled-delivery-expansion.contract.test.js',
]);
...
function evaluateControlledExpansion(repoRoot) {
  ...
  const x62Signed = readJsonObject(path.join(repoRoot, 'docs/OPS/STATUS/X62_CONTOUR_CLOSEOUT_SIGNED_V1.json'));
  ...
  if (!x62Signed) missingRefs.push('X62_CONTOUR_CLOSEOUT_SIGNED_V1_JSON_MISSING');
  ...
  const blockingSurfaceExpansionFalse = nextRecord?.entryChecks?.blockingSurfaceExpansion === false
    && x62Signed?.constraints?.blockingSurfaceExpansion === false;
...
export function evaluateX63Ws04ControlledDeliveryExpansionState(input = {}) {
  ...
  return {
    ...
    acceptance,
    determinism,
    finalGate: 'GO_TO_X63_WS99_ONLY_IF_ALL_DOD_AND_ACCEPTANCE_TRUE',
...
function main() {
  ...
  const state = evaluateX63Ws04ControlledDeliveryExpansionState({ repoRoot: process.cwd() });

  if (args.json) {
    process.stdout.write(`${stableStringify(state)}\n`);
  } else {
    process.stdout.write(`X63_WS04_CONTROLLED_DELIVERY_EXPANSION_OK=${state.X63_WS04_CONTROLLED_DELIVERY_EXPANSION_OK}\n`);
