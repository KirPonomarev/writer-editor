...
const TOKEN_NAME = 'X38_WS03_EVIDENCE_MAINTENANCE_OK';
const EXPECTED_CANON_VERSION = 'v3.13a-final';

const PACKET_FILE_ALLOWLIST = new Set([
  'docs/OPS/EVIDENCE/X38_CONTOUR/',
  'docs/OPS/EVIDENCE/X38_CONTOUR/TICKET_03/',
  'docs/OPS/EVIDENCE/X38_CONTOUR/TICKET_03/negative-results.json',
  'docs/OPS/EVIDENCE/X38_CONTOUR/TICKET_03/positive-results.json',
  'docs/OPS/EVIDENCE/X38_CONTOUR/TICKET_03/final-dod-acceptance-summary.json',
  'docs/OPS/EVIDENCE/X38_CONTOUR/TICKET_03/summary.json',
  'docs/OPS/EVIDENCE/X38_CONTOUR/TICKET_03/ticket-meta.json',
  'docs/OPS/STATUS/X38_WS03_EVIDENCE_MAINTENANCE_STATUS_V1.json',
  'scripts/ops/x38-ws03-evidence-maintenance-state.mjs',
  'scripts/ops/x38-ws03-evidence-maintenance-report.mjs',
  'test/contracts/x38-ws03-evidence-maintenance.contract.test.js',
]);
...
function evaluateEvidenceChain(repoRoot) {
  const nextRecordPath = path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V23.json');
  const x37SignedPath = path.join(repoRoot, 'docs/OPS/STATUS/X37_CONTOUR_CLOSEOUT_SIGNED_V1.json');
  const x37FreezePath = path.join(repoRoot, 'docs/OPS/EVIDENCE/X37_CONTOUR/x37-contour-freeze-packet-v1.json');

  const nextRecord = readJsonObject(nextRecordPath);
  const x37Signed = readJsonObject(x37SignedPath);
  const x37Freeze = readJsonObject(x37FreezePath);

  const missing = [];
  if (!nextRecord) missing.push('NEXT_RECORD_MISSING');
  if (!x37Signed) missing.push('X37_SIGNED_MISSING');
  if (!x37Freeze) missing.push('X37_FREEZE_MISSING');

  const nextRecordRefOk = normalizeString(nextRecord?.dependsOn?.x37CloseoutArtifactRef)
    === 'docs/OPS/STATUS/X37_CONTOUR_CLOSEOUT_SIGNED_V1.json';

  const signedRefOk = normalizeString(x37Signed?.evidencePacketRef)
    === 'docs/OPS/EVIDENCE/X37_CONTOUR/x37-contour-freeze-packet-v1.json';

  let hashMatch = false;
  if (x37Signed && fs.existsSync(x37FreezePath)) {
    const actual = createHash('sha256').update(fs.readFileSync(x37FreezePath)).digest('hex');
    hashMatch = normalizeString(x37Signed.evidencePacketSha256) === `sha256:${actual}`;
  }
...
function evaluateEvidenceRefCompleteness(repoRoot) {
  const requiredRefs = [
    'docs/OPS/STATUS/X38_WS01_NEXT_CONTOUR_ALIGNMENT_STATUS_V1.json',
    'docs/OPS/STATUS/X38_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json',
    'docs/OPS/EVIDENCE/X38_CONTOUR/TICKET_01/summary.json',
    'docs/OPS/EVIDENCE/X38_CONTOUR/TICKET_01/final-dod-acceptance-summary.json',
    'docs/OPS/EVIDENCE/X38_CONTOUR/TICKET_02/summary.json',
    'docs/OPS/EVIDENCE/X38_CONTOUR/TICKET_02/final-dod-acceptance-summary.json',
    'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V23.json',
    'docs/OPS/STATUS/X37_CONTOUR_CLOSEOUT_SIGNED_V1.json',
    'docs/OPS/EVIDENCE/X37_CONTOUR/x37-contour-freeze-packet-v1.json',
  ];
...
function evaluateLinkStaleness(repoRoot) {
  const ws02StatusPath = path.join(repoRoot, 'docs/OPS/STATUS/X38_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json');
...
export function evaluateX38Ws03EvidenceMaintenanceState(input = {}) {
  ...
  const nextRecord = readJsonObject(path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V23.json'));
  ...
  return {
    ...
    acceptance,
    determinism,
    finalGate: 'GO_TO_X38_WS04_ONLY_IF_ALL_DOD_AND_ACCEPTANCE_TRUE',
...
function main() {
  ...
  const state = evaluateX38Ws03EvidenceMaintenanceState({ repoRoot: process.cwd() });

  if (args.json) {
    process.stdout.write(`${stableStringify(state)}\n`);
  } else {
    process.stdout.write(`X38_WS03_EVIDENCE_MAINTENANCE_OK=${state.X38_WS03_EVIDENCE_MAINTENANCE_OK}\n`);
