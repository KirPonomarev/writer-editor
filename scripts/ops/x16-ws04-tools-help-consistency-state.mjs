...
const TOKEN_NAME = 'X16_WS04_TOOLS_HELP_CONSISTENCY_OK';
...
const DEFAULT_CONSISTENCY_PATH = 'docs/OPS/STATUS/X16_WS04_TOOLS_HELP_CONSISTENCY_v1.json';
...
const EXPECTED_FEATURE_IDS = Object.freeze(['TOOLS_GROUP_BASE_ENTRY', 'HELP_GROUP_BASE_ENTRY']);
...
function parseCapabilityObjectFromSource(rawSource, exportName) {
  ...
  if (!match || !match[1]) return { ok: false, value: {}, reason: `${exportName}_NOT_FOUND` };

  try {
    const parsed = Function(`"use strict"; return (${match[1]});`)();
    if (!isObjectRecord(parsed)) return { ok: false, value: {}, reason: `${exportName}_NOT_OBJECT` };
...
function normalizeConsistencyDoc(doc) {
  const source = isObjectRecord(doc) ? doc : {};
  return {
    schemaVersion: Number(source.schemaVersion) || 1,
    consistencyVersion: normalizeString(source.consistencyVersion),
    ...
    requiredHelpEntries: toUniqueStrings(source.requiredHelpEntries),
...
function normalizeAllowedVisibility(entry) {
  ...
  for (const rowRaw of rows) {
    ...
    out.push({
      mode: normalizeString(rowRaw.mode),
      profiles: toUniqueStrings(rowRaw.profiles, { lower: true, sort: false }),
      channels: {
        menu: normalizeString(rowRaw.channels?.menu),
        toolbar: normalizeString(rowRaw.channels?.toolbar),
        palette: normalizeString(rowRaw.channels?.palette),
      },
    });
  }
  return out;
...
function validateConsistency({
  ...
  for (const entryRaw of consistency.entries) {
    if (!isObjectRecord(entryRaw)) continue;
  ...
  const missingFeatures = EXPECTED_FEATURE_IDS.filter((id) => !featureById.has(id));
  const unexpectedFeatures = [...featureById.keys()].filter((id) => !EXPECTED_FEATURE_IDS.includes(id));

  const policyErrors = [];
  if (consistency.nonBlockingClassification !== 'advisory') {
    policyErrors.push('NON_BLOCKING_CLASSIFICATION_MUST_BE_ADVISORY');
  }
  if (consistency.blockingSurfaceExpansion !== false) {
    policyErrors.push('BLOCKING_SURFACE_EXPANSION_MUST_BE_FALSE');
  }
  if (consistency.formalMachineBinding.status !== 'not_bound') {
    policyErrors.push('FORMAL_MACHINE_BINDING_STATUS_MUST_BE_NOT_BOUND');
  }
  if (consistency.formalMachineBinding.blockingAllowed !== false) {
    policyErrors.push('FORMAL_MACHINE_BINDING_BLOCKING_ALLOWED_MUST_BE_FALSE');
  }

  const blockingClassificationLeaks = [];
  if (consistency.nonBlockingClassification === 'blocking') {
    blockingClassificationLeaks.push({ source: 'root.nonBlockingClassification', value: 'blocking' });
  }

  const requiredHelpEntryErrors = [];
  const requiredHelpEntries = new Set(consistency.requiredHelpEntries);
  if (!requiredHelpEntries.has('HELP_GROUP_BASE_ENTRY')) {
    requiredHelpEntryErrors.push('REQUIRED_HELP_ENTRY_ID_MISSING_FROM_REQUIRED_LIST');
  }
  for (const requiredId of requiredHelpEntries) {
    if (!featureById.has(requiredId)) {
      requiredHelpEntryErrors.push(`REQUIRED_HELP_ENTRY_MISSING:${requiredId}`);
    }
  }
  ...
  const invalidModeProfileGating = [];
  const channelVisibilityMismatches = [];
  const missingMenuRows = [];
  const channelConsistencyErrors = [];
  const modeProfileCoverageGaps = [];
  ...
  for (const featureId of EXPECTED_FEATURE_IDS) {
    ...
    for (const row of allowedVisibility) {
      if (!menuGroups.modes.includes(row.mode)) {
        invalidModeProfileGating.push({ featureId, mode: row.mode, reason: 'UNKNOWN_MODE' });
      }
      for (const profile of row.profiles) {
        if (!menuGroups.profiles.includes(profile)) {
          invalidModeProfileGating.push({ featureId, mode: row.mode, profile, reason: 'UNKNOWN_PROFILE' });
        }

        const key = `${commandId}|${row.mode}|${profile}`;
        coverage.add(`${row.mode}|${profile}`);
        ...
        const states = Object.values(row.channels);
        if (new Set(states).size !== 1) {
          channelConsistencyErrors.push({ featureId, commandId, mode: row.mode, profile, channels: row.channels });
        }
        ...
        for (const channel of menuGroups.channels) {
          ...
          if (expectedState !== actualState) {
            channelVisibilityMismatches.push({
              featureId,
              commandId,
              mode: row.mode,
              profile,
    ...
    if (entryRaw.requireFullModeProfileCoverage === true) {
      for (const mode of EXPECTED_MODES) {
        for (const profile of EXPECTED_PROFILES) {
          const pairKey = `${mode}|${profile}`;
          if (!coverage.has(pairKey)) {
            modeProfileCoverageGaps.push({ featureId, mode, profile });
          }
        }
      }
    }
    ...
    enabledWhenAllowed[featureId] = stageOk && scopeOk && capabilityOk;

    if (entryRaw.machineBinding?.blocking === true && consistency.formalMachineBinding.status !== 'bound') {
  ...
  const ok = missingFeatures.length === 0
    ...
    && requiredHelpEntryErrors.length === 0
    ...
    && invalidModeProfileGating.length === 0
    && channelVisibilityMismatches.length === 0
    && missingMenuRows.length === 0
    && channelConsistencyErrors.length === 0
    && modeProfileCoverageGaps.length === 0;
  ...
  return {
    ...
    invalidModeProfileGating,
    channelVisibilityMismatches,
    missingMenuRows,
    channelConsistencyErrors,
    modeProfileCoverageGaps,
    enabledWhenAllowed,
  };
}
...
function evaluateDeterminism(validationFn) {
  ...
  return { ok: deterministic, hashes: [hashA, hashB, hashC] };
}
...
function evaluateX16Ws04ToolsHelpConsistencyState(input = {}) {
  ...
  const consistencyPath = path.resolve(repoRoot, DEFAULT_CONSISTENCY_PATH);
  ...
  const consistencyDocRaw = readJsonObject(consistencyPath);
  ...
  const consistency = normalizeConsistencyDoc(consistencyDocRaw);
  ...
  const validateBaseline = () => validateConsistency({
    consistency,
    menuGroups,
    menuProjection,
    commandCapabilityBinding,
  ...
  const negative01Doc = deepClone(consistency);
  const toolsEntry01 = (negative01Doc.entries || []).find((row) => normalizeString(row?.featureId) === 'TOOLS_GROUP_BASE_ENTRY');
  if (toolsEntry01) {
    toolsEntry01.capabilityId = 'cap.core.project.create.mismatch';
  }
  const negative01 = validateConsistency({
    consistency: normalizeConsistencyDoc(negative01Doc),
    menuGroups,
    menuProjection,
    commandCapabilityBinding,
    runtimeCapabilityBinding,
  ...
  const negative02Doc = deepClone(consistency);
  const helpEntry02 = (negative02Doc.entries || []).find((row) => normalizeString(row?.featureId) === 'HELP_GROUP_BASE_ENTRY');
  if (helpEntry02) {
    helpEntry02.allowedVisibility = [
      {
        mode: 'Write',
        profiles: ['minimal'],
  ...
  const negative02 = validateConsistency({
    consistency: normalizeConsistencyDoc(negative02Doc),
    menuGroups,
    menuProjection,
    commandCapabilityBinding,
    runtimeCapabilityBinding,
  ...
  const negative03Doc = deepClone(consistency);
  const toolsEntry03 = (negative03Doc.entries || []).find((row) => normalizeString(row?.featureId) === 'TOOLS_GROUP_BASE_ENTRY');
  if (toolsEntry03 && Array.isArray(toolsEntry03.allowedVisibility) && toolsEntry03.allowedVisibility[0]) {
    toolsEntry03.allowedVisibility[0].channels = {
      menu: 'visible',
      toolbar: 'hidden',
      palette: 'visible',
    };
  }
  const negative03 = validateConsistency({
    consistency: normalizeConsistencyDoc(negative03Doc),
    menuGroups,
    menuProjection,
    commandCapabilityBinding,
    runtimeCapabilityBinding,
  ...
  const negative04Doc = deepClone(consistency);
  negative04Doc.nonBlockingClassification = 'blocking';
  const negative04 = validateConsistency({
    consistency: normalizeConsistencyDoc(negative04Doc),
    menuGroups,
    menuProjection,
    commandCapabilityBinding,
    runtimeCapabilityBinding,
  ...
  const negative05Doc = deepClone(consistency);
  negative05Doc.entries = (negative05Doc.entries || [])
    .filter((row) => normalizeString(row?.featureId) !== 'HELP_GROUP_BASE_ENTRY');
  const negative05 = validateConsistency({
    consistency: normalizeConsistencyDoc(negative05Doc),
    menuGroups,
    menuProjection,
    commandCapabilityBinding,
    runtimeCapabilityBinding,
  ...
  const negativeResults = {
    NEXT_TZ_NEGATIVE_01: negative01.commandCapabilityMismatches.length > 0,
    NEXT_TZ_NEGATIVE_02: negative02.modeProfileCoverageGaps.length > 0,
    NEXT_TZ_NEGATIVE_03: negative03.channelConsistencyErrors.length > 0 || negative03.channelVisibilityMismatches.length > 0,
    NEXT_TZ_NEGATIVE_04: negative04.policyErrors.length > 0,
    NEXT_TZ_NEGATIVE_05: negative05.requiredHelpEntryErrors.length > 0,
  };

  const positiveResults = {
    NEXT_TZ_POSITIVE_01: baseline.ok && baseline.enabledWhenAllowed.TOOLS_GROUP_BASE_ENTRY === true,
    NEXT_TZ_POSITIVE_02: baseline.ok && baseline.enabledWhenAllowed.HELP_GROUP_BASE_ENTRY === true,
  ...
  const state = {
    ...
    objective: 'TOOLS_HELP_MENU_CONSISTENCY_MODE_PROFILE_CHANNEL_NO_CORE_LEAK',
    ...
    counts: {
      consistencyEntryCount: consistency.entries.length,
      ...
      modeProfileCoverageGapCount: baseline.modeProfileCoverageGaps.length,
      channelMismatchCount: baseline.channelVisibilityMismatches.length,
    ...
    detector: {
      detectorId: 'X16_WS04_TOOLS_HELP_CONSISTENCY_SINGLE_DETECTOR_V1',
  ...
  if (!state.ok) {
    state.failReason = !canonLock.ok
      ...
      : !stageActivationGuardCheck
        ...
        : !dod.NEXT_TZ_DOD_01
          ? 'TOOLS_HELP_CONSISTENCY_NOT_READY'
          : !dod.NEXT_TZ_DOD_02
            ...
            : !dod.NEXT_TZ_DOD_03
              ...
              : !dod.NEXT_TZ_DOD_06
                ...
                : 'X16_WS04_TOOLS_HELP_CONSISTENCY_FAIL';
...
function printHuman(state) {
  ...
  console.log(`CONSISTENCY_ENTRY_COUNT=${state.counts.consistencyEntryCount}`);
  console.log(`MENU_PROJECTION_ROW_COUNT=${state.counts.menuProjectionRowCount}`);
  console.log(`MODE_PROFILE_COVERAGE_GAP_COUNT=${state.counts.modeProfileCoverageGapCount}`);
  console.log(`CHANNEL_MISMATCH_COUNT=${state.counts.channelMismatchCount}`);
...
function main() {
  ...
  const state = evaluateX16Ws04ToolsHelpConsistencyState({
...
export {
  evaluateX16Ws04ToolsHelpConsistencyState,
  TOKEN_NAME,
};
