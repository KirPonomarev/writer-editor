...
const TOKEN_NAME = 'X40_WS03_EVIDENCE_MAINTENANCE_OK';
const EXPECTED_CANON_VERSION = 'v3.13a-final';

const PACKET_FILE_ALLOWLIST = new Set([
  'docs/OPS/EVIDENCE/X40_CONTOUR/',
  'docs/OPS/EVIDENCE/X40_CONTOUR/TICKET_03/',
  'docs/OPS/EVIDENCE/X40_CONTOUR/TICKET_03/negative-results.json',
  'docs/OPS/EVIDENCE/X40_CONTOUR/TICKET_03/positive-results.json',
  'docs/OPS/EVIDENCE/X40_CONTOUR/TICKET_03/final-dod-acceptance-summary.json',
  'docs/OPS/EVIDENCE/X40_CONTOUR/TICKET_03/summary.json',
  'docs/OPS/EVIDENCE/X40_CONTOUR/TICKET_03/ticket-meta.json',
  'docs/OPS/STATUS/X40_WS03_EVIDENCE_MAINTENANCE_STATUS_V1.json',
  'scripts/ops/x40-ws03-evidence-maintenance-state.mjs',
  'scripts/ops/x40-ws03-evidence-maintenance-report.mjs',
  'test/contracts/x40-ws03-evidence-maintenance.contract.test.js',
]);
...
function evaluateEvidenceChain(repoRoot) {
  const nextRecordPath = path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V25.json');
  const x39SignedPath = path.join(repoRoot, 'docs/OPS/STATUS/X39_CONTOUR_CLOSEOUT_SIGNED_V1.json');
  const x39FreezePath = path.join(repoRoot, 'docs/OPS/EVIDENCE/X39_CONTOUR/x39-contour-freeze-packet-v1.json');

  const nextRecord = readJsonObject(nextRecordPath);
  const x39Signed = readJsonObject(x39SignedPath);
  const x39Freeze = readJsonObject(x39FreezePath);

  const missing = [];
  if (!nextRecord) missing.push('NEXT_RECORD_MISSING');
  if (!x39Signed) missing.push('X39_SIGNED_MISSING');
  if (!x39Freeze) missing.push('X39_FREEZE_MISSING');

  const nextRecordRefOk = normalizeString(nextRecord?.dependsOn?.x39CloseoutArtifactRef)
    === 'docs/OPS/STATUS/X39_CONTOUR_CLOSEOUT_SIGNED_V1.json';

  const signedRefOk = normalizeString(x39Signed?.evidencePacketRef)
    === 'docs/OPS/EVIDENCE/X39_CONTOUR/x39-contour-freeze-packet-v1.json';

  let hashMatch = false;
  if (x39Signed && fs.existsSync(x39FreezePath)) {
    const actual = createHash('sha256').update(fs.readFileSync(x39FreezePath)).digest('hex');
    hashMatch = normalizeString(x39Signed.evidencePacketSha256) === `sha256:${actual}`;
  }
...
function evaluateEvidenceRefCompleteness(repoRoot) {
  const requiredRefs = [
    'docs/OPS/STATUS/X40_WS01_NEXT_CONTOUR_ALIGNMENT_STATUS_V1.json',
    'docs/OPS/STATUS/X40_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json',
    'docs/OPS/EVIDENCE/X40_CONTOUR/TICKET_01/summary.json',
    'docs/OPS/EVIDENCE/X40_CONTOUR/TICKET_01/final-dod-acceptance-summary.json',
    'docs/OPS/EVIDENCE/X40_CONTOUR/TICKET_02/summary.json',
    'docs/OPS/EVIDENCE/X40_CONTOUR/TICKET_02/final-dod-acceptance-summary.json',
    'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V25.json',
    'docs/OPS/STATUS/X39_CONTOUR_CLOSEOUT_SIGNED_V1.json',
    'docs/OPS/EVIDENCE/X39_CONTOUR/x39-contour-freeze-packet-v1.json',
  ];
...
function evaluateLinkStaleness(repoRoot) {
  const ws02StatusPath = path.join(repoRoot, 'docs/OPS/STATUS/X40_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json');
...
export function evaluateX40Ws03EvidenceMaintenanceState(input = {}) {
  ...
  const nextRecord = readJsonObject(path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V25.json'));
  ...
  return {
    ...
    acceptance,
    determinism,
    finalGate: 'GO_TO_X40_WS04_ONLY_IF_ALL_DOD_AND_ACCEPTANCE_TRUE',
...
function main() {
  ...
  const state = evaluateX40Ws03EvidenceMaintenanceState({ repoRoot: process.cwd() });

  if (args.json) {
    process.stdout.write(`${stableStringify(state)}\n`);
  } else {
    process.stdout.write(`X40_WS03_EVIDENCE_MAINTENANCE_OK=${state.X40_WS03_EVIDENCE_MAINTENANCE_OK}\n`);
