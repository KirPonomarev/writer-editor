...
const TOKEN_NAME = 'X47_WS03_EVIDENCE_MAINTENANCE_OK';
const EXPECTED_CANON_VERSION = 'v3.13a-final';

const PACKET_FILE_ALLOWLIST = new Set([
  'docs/OPS/EVIDENCE/X47_CONTOUR/',
  'docs/OPS/EVIDENCE/X47_CONTOUR/TICKET_03/',
  'docs/OPS/EVIDENCE/X47_CONTOUR/TICKET_03/negative-results.json',
  'docs/OPS/EVIDENCE/X47_CONTOUR/TICKET_03/positive-results.json',
  'docs/OPS/EVIDENCE/X47_CONTOUR/TICKET_03/final-dod-acceptance-summary.json',
  'docs/OPS/EVIDENCE/X47_CONTOUR/TICKET_03/summary.json',
  'docs/OPS/EVIDENCE/X47_CONTOUR/TICKET_03/ticket-meta.json',
  'docs/OPS/STATUS/X47_WS03_EVIDENCE_MAINTENANCE_STATUS_V1.json',
  'scripts/ops/x47-ws03-evidence-maintenance-state.mjs',
  'scripts/ops/x47-ws03-evidence-maintenance-report.mjs',
  'test/contracts/x47-ws03-evidence-maintenance.contract.test.js',
]);
...
function evaluateEvidenceChain(repoRoot) {
  const nextRecordPath = path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V32.json');
  const x41SignedPath = path.join(repoRoot, 'docs/OPS/STATUS/X46_CONTOUR_CLOSEOUT_SIGNED_V1.json');
  const x41FreezePath = path.join(repoRoot, 'docs/OPS/EVIDENCE/X46_CONTOUR/x46-contour-freeze-packet-v1.json');
  ...
  if (!x41Signed) missing.push('X46_SIGNED_MISSING');
  if (!x41Freeze) missing.push('X46_FREEZE_MISSING');

  const nextRecordRefOk = normalizeString(nextRecord?.dependsOn?.x46CloseoutArtifactRef)
    === 'docs/OPS/STATUS/X46_CONTOUR_CLOSEOUT_SIGNED_V1.json';

  const signedRefOk = normalizeString(x41Signed?.evidencePacketRef)
    === 'docs/OPS/EVIDENCE/X46_CONTOUR/x46-contour-freeze-packet-v1.json';
...
function evaluateEvidenceRefCompleteness(repoRoot) {
  const requiredRefs = [
    'docs/OPS/STATUS/X47_WS01_NEXT_CONTOUR_ALIGNMENT_STATUS_V1.json',
    'docs/OPS/STATUS/X47_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json',
    'docs/OPS/EVIDENCE/X47_CONTOUR/TICKET_01/summary.json',
    'docs/OPS/EVIDENCE/X47_CONTOUR/TICKET_01/final-dod-acceptance-summary.json',
    'docs/OPS/EVIDENCE/X47_CONTOUR/TICKET_02/summary.json',
    'docs/OPS/EVIDENCE/X47_CONTOUR/TICKET_02/final-dod-acceptance-summary.json',
    'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V32.json',
    'docs/OPS/STATUS/X46_CONTOUR_CLOSEOUT_SIGNED_V1.json',
    'docs/OPS/EVIDENCE/X46_CONTOUR/x46-contour-freeze-packet-v1.json',
  ];
...
function evaluateLinkStaleness(repoRoot) {
  const ws02StatusPath = path.join(repoRoot, 'docs/OPS/STATUS/X47_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json');
...
export function evaluateX47Ws03EvidenceMaintenanceState(input = {}) {
  ...
  const nextRecord = readJsonObject(path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V32.json'));
  ...
  return {
    ...
    acceptance,
    determinism,
    finalGate: 'GO_TO_X47_WS04_ONLY_IF_ALL_DOD_AND_ACCEPTANCE_TRUE',
...
function main() {
  ...
  const state = evaluateX47Ws03EvidenceMaintenanceState({ repoRoot: process.cwd() });

  if (args.json) {
    process.stdout.write(`${stableStringify(state)}\n`);
  } else {
    process.stdout.write(`X47_WS03_EVIDENCE_MAINTENANCE_OK=${state.X47_WS03_EVIDENCE_MAINTENANCE_OK}\n`);
