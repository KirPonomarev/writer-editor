...
const TOKEN_NAME = 'X56_WS03_EVIDENCE_MAINTENANCE_OK';
const EXPECTED_CANON_VERSION = 'v3.13a-final';

const PACKET_FILE_ALLOWLIST = new Set([
  'docs/OPS/EVIDENCE/X56_CONTOUR/',
  'docs/OPS/EVIDENCE/X56_CONTOUR/TICKET_03/',
  'docs/OPS/EVIDENCE/X56_CONTOUR/TICKET_03/negative-results.json',
  'docs/OPS/EVIDENCE/X56_CONTOUR/TICKET_03/positive-results.json',
  'docs/OPS/EVIDENCE/X56_CONTOUR/TICKET_03/final-dod-acceptance-summary.json',
  'docs/OPS/EVIDENCE/X56_CONTOUR/TICKET_03/summary.json',
  'docs/OPS/EVIDENCE/X56_CONTOUR/TICKET_03/ticket-meta.json',
  'docs/OPS/STATUS/X56_WS03_EVIDENCE_MAINTENANCE_STATUS_V1.json',
  'scripts/ops/x56-ws03-evidence-maintenance-state.mjs',
  'scripts/ops/x56-ws03-evidence-maintenance-report.mjs',
  'test/contracts/x56-ws03-evidence-maintenance.contract.test.js',
]);
...
function evaluateEvidenceChain(repoRoot) {
  ...
  const x55SignedPath = path.join(repoRoot, 'docs/OPS/STATUS/X55_CONTOUR_CLOSEOUT_SIGNED_V1.json');
  const x55FreezePath = path.join(repoRoot, 'docs/OPS/EVIDENCE/X55_CONTOUR/x55-contour-freeze-packet-v1.json');

  const nextRecord = readJsonObject(nextRecordPath);
  const x55Signed = readJsonObject(x55SignedPath);
  const x55Freeze = readJsonObject(x55FreezePath);

  const missing = [];
  if (!nextRecord) missing.push('NEXT_RECORD_MISSING');
  if (!x55Signed) missing.push('X55_SIGNED_MISSING');
  if (!x55Freeze) missing.push('X55_FREEZE_MISSING');

  const nextRecordRefOk = normalizeString(nextRecord?.dependsOn?.x55CloseoutArtifactRef)
    === 'docs/OPS/STATUS/X55_CONTOUR_CLOSEOUT_SIGNED_V1.json';

  const signedRefOk = normalizeString(x55Signed?.evidencePacketRef)
    === 'docs/OPS/EVIDENCE/X55_CONTOUR/x55-contour-freeze-packet-v1.json';

  let hashMatch = false;
  if (x55Signed && fs.existsSync(x55FreezePath)) {
    const actual = createHash('sha256').update(fs.readFileSync(x55FreezePath)).digest('hex');
    hashMatch = normalizeString(x55Signed.evidencePacketSha256) === `sha256:${actual}`;
  }
...
function evaluateEvidenceRefCompleteness(repoRoot) {
  const requiredRefs = [
    'docs/OPS/STATUS/X56_WS01_NEXT_CONTOUR_ALIGNMENT_STATUS_V1.json',
    'docs/OPS/STATUS/X56_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json',
    'docs/OPS/EVIDENCE/X56_CONTOUR/TICKET_01/summary.json',
    'docs/OPS/EVIDENCE/X56_CONTOUR/TICKET_01/final-dod-acceptance-summary.json',
    'docs/OPS/EVIDENCE/X56_CONTOUR/TICKET_02/summary.json',
    'docs/OPS/EVIDENCE/X56_CONTOUR/TICKET_02/final-dod-acceptance-summary.json',
...
function evaluateLinkStaleness(repoRoot) {
  const ws02StatusPath = path.join(repoRoot, 'docs/OPS/STATUS/X56_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json');
...
export function evaluateX56Ws03EvidenceMaintenanceState(input = {}) {
  ...
  return {
    ...
    acceptance,
    determinism,
    finalGate: 'GO_TO_X56_WS04_ONLY_IF_ALL_DOD_AND_ACCEPTANCE_TRUE',
...
function main() {
  ...
  const state = evaluateX56Ws03EvidenceMaintenanceState({ repoRoot: process.cwd() });

  if (args.json) {
    process.stdout.write(`${stableStringify(state)}\n`);
  } else {
    process.stdout.write(`X56_WS03_EVIDENCE_MAINTENANCE_OK=${state.X56_WS03_EVIDENCE_MAINTENANCE_OK}\n`);
