...
import { evaluateX56Ws04ControlledDeliveryExpansionState } from './x56-ws04-controlled-delivery-expansion-state.mjs';

const DEFAULT_OUTPUT_DIR = 'docs/OPS/EVIDENCE/X56_CONTOUR/TICKET_04';
const DEFAULT_STATUS_PATH = 'docs/OPS/STATUS/X56_WS04_CONTROLLED_DELIVERY_EXPANSION_STATUS_V1.json';
const DEFAULT_RUN_ID = 'TZ_X56_WS04_CONTROLLED_DELIVERY_EXPANSION_001';
...
function main() {
  ...
  const run1 = evaluateX56Ws04ControlledDeliveryExpansionState({ repoRoot });
  const run2 = evaluateX56Ws04ControlledDeliveryExpansionState({ repoRoot });
  const run3 = evaluateX56Ws04ControlledDeliveryExpansionState({ repoRoot });
