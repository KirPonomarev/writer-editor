...
const TOKEN_NAME = 'X51_WS03_EVIDENCE_MAINTENANCE_OK';
const EXPECTED_CANON_VERSION = 'v3.13a-final';

const PACKET_FILE_ALLOWLIST = new Set([
  'docs/OPS/EVIDENCE/X51_CONTOUR/',
  'docs/OPS/EVIDENCE/X51_CONTOUR/TICKET_03/',
  'docs/OPS/EVIDENCE/X51_CONTOUR/TICKET_03/negative-results.json',
  'docs/OPS/EVIDENCE/X51_CONTOUR/TICKET_03/positive-results.json',
  'docs/OPS/EVIDENCE/X51_CONTOUR/TICKET_03/final-dod-acceptance-summary.json',
  'docs/OPS/EVIDENCE/X51_CONTOUR/TICKET_03/summary.json',
  'docs/OPS/EVIDENCE/X51_CONTOUR/TICKET_03/ticket-meta.json',
  'docs/OPS/STATUS/X51_WS03_EVIDENCE_MAINTENANCE_STATUS_V1.json',
  'scripts/ops/x51-ws03-evidence-maintenance-state.mjs',
  'scripts/ops/x51-ws03-evidence-maintenance-report.mjs',
  'test/contracts/x51-ws03-evidence-maintenance.contract.test.js',
]);
...
function evaluateEvidenceChain(repoRoot) {
  const nextRecordPath = path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V36.json');
  const x50SignedPath = path.join(repoRoot, 'docs/OPS/STATUS/X50_CONTOUR_CLOSEOUT_SIGNED_V1.json');
  const x50FreezePath = path.join(repoRoot, 'docs/OPS/EVIDENCE/X50_CONTOUR/x50-contour-freeze-packet-v1.json');

  const nextRecord = readJsonObject(nextRecordPath);
  const x50Signed = readJsonObject(x50SignedPath);
  const x50Freeze = readJsonObject(x50FreezePath);

  const missing = [];
  if (!nextRecord) missing.push('NEXT_RECORD_MISSING');
  if (!x50Signed) missing.push('X50_SIGNED_MISSING');
  if (!x50Freeze) missing.push('X50_FREEZE_MISSING');

  const nextRecordRefOk = normalizeString(nextRecord?.dependsOn?.x50CloseoutArtifactRef)
    === 'docs/OPS/STATUS/X50_CONTOUR_CLOSEOUT_SIGNED_V1.json';

  const signedRefOk = normalizeString(x50Signed?.evidencePacketRef)
    === 'docs/OPS/EVIDENCE/X50_CONTOUR/x50-contour-freeze-packet-v1.json';

  let hashMatch = false;
  if (x50Signed && fs.existsSync(x50FreezePath)) {
    const actual = createHash('sha256').update(fs.readFileSync(x50FreezePath)).digest('hex');
    hashMatch = normalizeString(x50Signed.evidencePacketSha256) === `sha256:${actual}`;
  }
...
function evaluateEvidenceRefCompleteness(repoRoot) {
  const requiredRefs = [
    'docs/OPS/STATUS/X51_WS01_NEXT_CONTOUR_ALIGNMENT_STATUS_V1.json',
    'docs/OPS/STATUS/X51_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json',
    'docs/OPS/EVIDENCE/X51_CONTOUR/TICKET_01/summary.json',
    'docs/OPS/EVIDENCE/X51_CONTOUR/TICKET_01/final-dod-acceptance-summary.json',
    'docs/OPS/EVIDENCE/X51_CONTOUR/TICKET_02/summary.json',
    'docs/OPS/EVIDENCE/X51_CONTOUR/TICKET_02/final-dod-acceptance-summary.json',
    'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V36.json',
    'docs/OPS/STATUS/X50_CONTOUR_CLOSEOUT_SIGNED_V1.json',
    'docs/OPS/EVIDENCE/X50_CONTOUR/x50-contour-freeze-packet-v1.json',
  ];
...
function evaluateLinkStaleness(repoRoot) {
  const ws02StatusPath = path.join(repoRoot, 'docs/OPS/STATUS/X51_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json');
...
export function evaluateX51Ws03EvidenceMaintenanceState(input = {}) {
  ...
  const nextRecord = readJsonObject(path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V36.json'));
  ...
  return {
    ...
    acceptance,
    determinism,
    finalGate: 'GO_TO_X51_WS04_ONLY_IF_ALL_DOD_AND_ACCEPTANCE_TRUE',
...
function main() {
  ...
  const state = evaluateX51Ws03EvidenceMaintenanceState({ repoRoot: process.cwd() });

  if (args.json) {
    process.stdout.write(`${stableStringify(state)}\n`);
  } else {
    process.stdout.write(`X51_WS03_EVIDENCE_MAINTENANCE_OK=${state.X51_WS03_EVIDENCE_MAINTENANCE_OK}\n`);
