...
import { evaluateX51Ws04ControlledDeliveryExpansionState } from './x51-ws04-controlled-delivery-expansion-state.mjs';

const DEFAULT_OUTPUT_DIR = 'docs/OPS/EVIDENCE/X51_CONTOUR/TICKET_04';
const DEFAULT_STATUS_PATH = 'docs/OPS/STATUS/X51_WS04_CONTROLLED_DELIVERY_EXPANSION_STATUS_V1.json';
const DEFAULT_RUN_ID = 'TZ_X51_WS04_CONTROLLED_DELIVERY_EXPANSION_001';
...
function main() {
  ...
  const run1 = evaluateX51Ws04ControlledDeliveryExpansionState({ repoRoot });
  const run2 = evaluateX51Ws04ControlledDeliveryExpansionState({ repoRoot });
  const run3 = evaluateX51Ws04ControlledDeliveryExpansionState({ repoRoot });
