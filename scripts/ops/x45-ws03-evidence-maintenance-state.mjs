...
const TOKEN_NAME = 'X45_WS03_EVIDENCE_MAINTENANCE_OK';
const EXPECTED_CANON_VERSION = 'v3.13a-final';

const PACKET_FILE_ALLOWLIST = new Set([
  'docs/OPS/EVIDENCE/X45_CONTOUR/',
  'docs/OPS/EVIDENCE/X45_CONTOUR/TICKET_03/',
  'docs/OPS/EVIDENCE/X45_CONTOUR/TICKET_03/negative-results.json',
  'docs/OPS/EVIDENCE/X45_CONTOUR/TICKET_03/positive-results.json',
  'docs/OPS/EVIDENCE/X45_CONTOUR/TICKET_03/final-dod-acceptance-summary.json',
  'docs/OPS/EVIDENCE/X45_CONTOUR/TICKET_03/summary.json',
  'docs/OPS/EVIDENCE/X45_CONTOUR/TICKET_03/ticket-meta.json',
  'docs/OPS/STATUS/X45_WS03_EVIDENCE_MAINTENANCE_STATUS_V1.json',
  'scripts/ops/x45-ws03-evidence-maintenance-state.mjs',
  'scripts/ops/x45-ws03-evidence-maintenance-report.mjs',
  'test/contracts/x45-ws03-evidence-maintenance.contract.test.js',
]);
...
function evaluateEvidenceChain(repoRoot) {
  const nextRecordPath = path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V30.json');
  const x41SignedPath = path.join(repoRoot, 'docs/OPS/STATUS/X44_CONTOUR_CLOSEOUT_SIGNED_V1.json');
  const x41FreezePath = path.join(repoRoot, 'docs/OPS/EVIDENCE/X44_CONTOUR/x44-contour-freeze-packet-v1.json');

  const nextRecord = readJsonObject(nextRecordPath);
  const x41Signed = readJsonObject(x41SignedPath);
  const x41Freeze = readJsonObject(x41FreezePath);

  const missing = [];
  if (!nextRecord) missing.push('NEXT_RECORD_MISSING');
  if (!x41Signed) missing.push('X44_SIGNED_MISSING');
  if (!x41Freeze) missing.push('X44_FREEZE_MISSING');

  const nextRecordRefOk = normalizeString(nextRecord?.dependsOn?.x44CloseoutArtifactRef)
    === 'docs/OPS/STATUS/X44_CONTOUR_CLOSEOUT_SIGNED_V1.json';

  const signedRefOk = normalizeString(x41Signed?.evidencePacketRef)
    === 'docs/OPS/EVIDENCE/X44_CONTOUR/x44-contour-freeze-packet-v1.json';

  let hashMatch = false;
  if (x41Signed && fs.existsSync(x41FreezePath)) {
    const actual = createHash('sha256').update(fs.readFileSync(x41FreezePath)).digest('hex');
    hashMatch = normalizeString(x41Signed.evidencePacketSha256) === `sha256:${actual}`;
  }
...
function evaluateEvidenceRefCompleteness(repoRoot) {
  const requiredRefs = [
    'docs/OPS/STATUS/X45_WS01_NEXT_CONTOUR_ALIGNMENT_STATUS_V1.json',
    'docs/OPS/STATUS/X45_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json',
    'docs/OPS/EVIDENCE/X45_CONTOUR/TICKET_01/summary.json',
    'docs/OPS/EVIDENCE/X45_CONTOUR/TICKET_01/final-dod-acceptance-summary.json',
    'docs/OPS/EVIDENCE/X45_CONTOUR/TICKET_02/summary.json',
    'docs/OPS/EVIDENCE/X45_CONTOUR/TICKET_02/final-dod-acceptance-summary.json',
    'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V30.json',
    'docs/OPS/STATUS/X44_CONTOUR_CLOSEOUT_SIGNED_V1.json',
    'docs/OPS/EVIDENCE/X44_CONTOUR/x44-contour-freeze-packet-v1.json',
  ];
...
function evaluateLinkStaleness(repoRoot) {
  const ws02StatusPath = path.join(repoRoot, 'docs/OPS/STATUS/X45_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json');
...
export function evaluateX45Ws03EvidenceMaintenanceState(input = {}) {
  ...
  const nextRecord = readJsonObject(path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V30.json'));
  ...
  return {
    ...
    acceptance,
    determinism,
    finalGate: 'GO_TO_X45_WS04_ONLY_IF_ALL_DOD_AND_ACCEPTANCE_TRUE',
...
function main() {
  ...
  const state = evaluateX45Ws03EvidenceMaintenanceState({ repoRoot: process.cwd() });

  if (args.json) {
    process.stdout.write(`${stableStringify(state)}\n`);
  } else {
    process.stdout.write(`X45_WS03_EVIDENCE_MAINTENANCE_OK=${state.X45_WS03_EVIDENCE_MAINTENANCE_OK}\n`);
