...
const TOKEN_NAME = 'X28_WS03_EVIDENCE_MAINTENANCE_OK';
const EXPECTED_CANON_VERSION = 'v3.13a-final';

const PACKET_FILE_ALLOWLIST = new Set([
  'docs/OPS/EVIDENCE/X28_CONTOUR/',
  'docs/OPS/EVIDENCE/X28_CONTOUR/TICKET_03/',
  'docs/OPS/EVIDENCE/X28_CONTOUR/TICKET_03/negative-results.json',
  'docs/OPS/EVIDENCE/X28_CONTOUR/TICKET_03/positive-results.json',
  'docs/OPS/EVIDENCE/X28_CONTOUR/TICKET_03/final-dod-acceptance-summary.json',
  'docs/OPS/EVIDENCE/X28_CONTOUR/TICKET_03/summary.json',
  'docs/OPS/EVIDENCE/X28_CONTOUR/TICKET_03/ticket-meta.json',
  'docs/OPS/STATUS/X28_WS03_EVIDENCE_MAINTENANCE_STATUS_V1.json',
  'scripts/ops/x28-ws03-evidence-maintenance-state.mjs',
  'scripts/ops/x28-ws03-evidence-maintenance-report.mjs',
  'test/contracts/x28-ws03-evidence-maintenance.contract.test.js',
]);
...
function evaluateEvidenceChain(repoRoot) {
  const nextRecordPath = path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V13.json');
  const x27SignedPath = path.join(repoRoot, 'docs/OPS/STATUS/X27_CONTOUR_CLOSEOUT_SIGNED_V1.json');
  const x27FreezePath = path.join(repoRoot, 'docs/OPS/EVIDENCE/X27_CONTOUR/x27-contour-freeze-packet-v1.json');

  const nextRecord = readJsonObject(nextRecordPath);
  const x27Signed = readJsonObject(x27SignedPath);
  const x27Freeze = readJsonObject(x27FreezePath);

  const missing = [];
  if (!nextRecord) missing.push('NEXT_RECORD_MISSING');
  if (!x27Signed) missing.push('X27_SIGNED_MISSING');
  if (!x27Freeze) missing.push('X27_FREEZE_MISSING');

  const nextRecordRefOk = normalizeString(nextRecord?.dependsOn?.x27CloseoutArtifactRef)
    === 'docs/OPS/STATUS/X27_CONTOUR_CLOSEOUT_SIGNED_V1.json';

  const signedRefOk = normalizeString(x27Signed?.evidencePacketRef)
    === 'docs/OPS/EVIDENCE/X27_CONTOUR/x27-contour-freeze-packet-v1.json';

  let hashMatch = false;
  if (x27Signed && fs.existsSync(x27FreezePath)) {
    const actual = createHash('sha256').update(fs.readFileSync(x27FreezePath)).digest('hex');
    hashMatch = normalizeString(x27Signed.evidencePacketSha256) === `sha256:${actual}`;
  }
...
function evaluateEvidenceRefCompleteness(repoRoot) {
  const requiredRefs = [
    'docs/OPS/STATUS/X28_WS01_NEXT_CONTOUR_ALIGNMENT_STATUS_V1.json',
    'docs/OPS/STATUS/X28_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json',
    'docs/OPS/EVIDENCE/X28_CONTOUR/TICKET_01/summary.json',
    'docs/OPS/EVIDENCE/X28_CONTOUR/TICKET_01/final-dod-acceptance-summary.json',
    'docs/OPS/EVIDENCE/X28_CONTOUR/TICKET_02/summary.json',
    'docs/OPS/EVIDENCE/X28_CONTOUR/TICKET_02/final-dod-acceptance-summary.json',
    'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V13.json',
    'docs/OPS/STATUS/X27_CONTOUR_CLOSEOUT_SIGNED_V1.json',
    'docs/OPS/EVIDENCE/X27_CONTOUR/x27-contour-freeze-packet-v1.json',
  ];
...
function evaluateLinkStaleness(repoRoot) {
  const ws02StatusPath = path.join(repoRoot, 'docs/OPS/STATUS/X28_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json');
...
export function evaluateX28Ws03EvidenceMaintenanceState(input = {}) {
  ...
  const nextRecord = readJsonObject(path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V13.json'));
  ...
  return {
    ...
    acceptance,
    determinism,
    finalGate: 'GO_TO_X28_WS04_ONLY_IF_ALL_DOD_AND_ACCEPTANCE_TRUE',
...
function main() {
  ...
  const state = evaluateX28Ws03EvidenceMaintenanceState({ repoRoot: process.cwd() });

  if (args.json) {
    process.stdout.write(`${stableStringify(state)}\n`);
  } else {
    process.stdout.write(`X28_WS03_EVIDENCE_MAINTENANCE_OK=${state.X28_WS03_EVIDENCE_MAINTENANCE_OK}\n`);
