...
const TOKEN_NAME = 'X63_WS03_EVIDENCE_MAINTENANCE_OK';
const EXPECTED_CANON_VERSION = 'v3.13a-final';

const PACKET_FILE_ALLOWLIST = new Set([
  'docs/OPS/EVIDENCE/X63_CONTOUR/',
  'docs/OPS/EVIDENCE/X63_CONTOUR/TICKET_03/',
  'docs/OPS/EVIDENCE/X63_CONTOUR/TICKET_03/negative-results.json',
  'docs/OPS/EVIDENCE/X63_CONTOUR/TICKET_03/positive-results.json',
  'docs/OPS/EVIDENCE/X63_CONTOUR/TICKET_03/final-dod-acceptance-summary.json',
  'docs/OPS/EVIDENCE/X63_CONTOUR/TICKET_03/summary.json',
  'docs/OPS/EVIDENCE/X63_CONTOUR/TICKET_03/ticket-meta.json',
  'docs/OPS/STATUS/X63_WS03_EVIDENCE_MAINTENANCE_STATUS_V1.json',
  'scripts/ops/x63-ws03-evidence-maintenance-state.mjs',
  'scripts/ops/x63-ws03-evidence-maintenance-report.mjs',
  'test/contracts/x63-ws03-evidence-maintenance.contract.test.js',
]);
...
function evaluateEvidenceChain(repoRoot) {
  const nextRecordPath = path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V46.json');
  const x62SignedPath = path.join(repoRoot, 'docs/OPS/STATUS/X62_CONTOUR_CLOSEOUT_SIGNED_V1.json');
  const x62FreezePath = path.join(repoRoot, 'docs/OPS/EVIDENCE/X62_CONTOUR/x62-contour-freeze-packet-v1.json');

  const nextRecord = readJsonObject(nextRecordPath);
  const x62Signed = readJsonObject(x62SignedPath);
  const x62Freeze = readJsonObject(x62FreezePath);

  const missing = [];
  if (!nextRecord) missing.push('NEXT_RECORD_MISSING');
  if (!x62Signed) missing.push('X62_SIGNED_MISSING');
  if (!x62Freeze) missing.push('X62_FREEZE_MISSING');

  const nextRecordRefOk = normalizeString(nextRecord?.dependsOn?.x62CloseoutArtifactRef)
    === 'docs/OPS/STATUS/X62_CONTOUR_CLOSEOUT_SIGNED_V1.json';

  const signedRefOk = normalizeString(x62Signed?.evidencePacketRef)
    === 'docs/OPS/EVIDENCE/X62_CONTOUR/x62-contour-freeze-packet-v1.json';

  let hashMatch = false;
  if (x62Signed && fs.existsSync(x62FreezePath)) {
    const actual = createHash('sha256').update(fs.readFileSync(x62FreezePath)).digest('hex');
    hashMatch = normalizeString(x62Signed.evidencePacketSha256) === `sha256:${actual}`;
  }
...
function evaluateEvidenceRefCompleteness(repoRoot) {
  const requiredRefs = [
    'docs/OPS/STATUS/X63_WS01_NEXT_CONTOUR_ALIGNMENT_STATUS_V1.json',
    'docs/OPS/STATUS/X63_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json',
    'docs/OPS/EVIDENCE/X63_CONTOUR/TICKET_01/summary.json',
    'docs/OPS/EVIDENCE/X63_CONTOUR/TICKET_01/final-dod-acceptance-summary.json',
    'docs/OPS/EVIDENCE/X63_CONTOUR/TICKET_02/summary.json',
    'docs/OPS/EVIDENCE/X63_CONTOUR/TICKET_02/final-dod-acceptance-summary.json',
...
function evaluateLinkStaleness(repoRoot) {
  const ws02StatusPath = path.join(repoRoot, 'docs/OPS/STATUS/X63_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json');
...
export function evaluateX63Ws03EvidenceMaintenanceState(input = {}) {
  ...
  const nextRecord = readJsonObject(path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V46.json'));
  ...
  return {
    ...
    acceptance,
    determinism,
    finalGate: 'GO_TO_X63_WS04_ONLY_IF_ALL_DOD_AND_ACCEPTANCE_TRUE',
...
function main() {
  ...
  const state = evaluateX63Ws03EvidenceMaintenanceState({ repoRoot: process.cwd() });

  if (args.json) {
    process.stdout.write(`${stableStringify(state)}\n`);
  } else {
    process.stdout.write(`X63_WS03_EVIDENCE_MAINTENANCE_OK=${state.X63_WS03_EVIDENCE_MAINTENANCE_OK}\n`);
