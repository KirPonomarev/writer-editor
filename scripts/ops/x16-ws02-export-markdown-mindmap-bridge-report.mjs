...
import { evaluateX16Ws02ExportMarkdownMindmapBridgeState } from './x16-ws02-export-markdown-mindmap-bridge-state.mjs';

const DEFAULT_OUTPUT_DIR = 'docs/OPS/EVIDENCE/X16_CONTOUR/TICKET_02';
const DEFAULT_STATUS_PATH = 'docs/OPS/STATUS/X16_WS02_EXPORT_MARKDOWN_MINDMAP_BRIDGE_STATUS_v1.json';
...
function comparableState(state) {
  return {
    ok: state.ok,
    token: state.X16_WS02_EXPORT_MARKDOWN_MINDMAP_BRIDGE_OK,
    counts: state.counts,
...
function main() {
  ...
  const run1 = evaluateX16Ws02ExportMarkdownMindmapBridgeState({ repoRoot });
  const run2 = evaluateX16Ws02ExportMarkdownMindmapBridgeState({ repoRoot });
  const run3 = evaluateX16Ws02ExportMarkdownMindmapBridgeState({ repoRoot });
  ...
  const summary = {
    ...
    objective: 'ENABLE_EXPORT_MARKDOWN_AND_MINDMAP_BRIDGE_STAGE_GATED_NO_CORE_LEAK',
  ...
  const statusDoc = {
    version: 1,
    token: 'X16_WS02_EXPORT_MARKDOWN_MINDMAP_BRIDGE_OK',
    bridgeEntryCount: summary.counts.bridgeEntryCount,
    ...
    visibilityConflictCount: summary.counts.visibilityConflictCount,
    missingStageGateCount: summary.counts.missingStageGateCount,
  ...
  writeJson(path.join(outputDir, 'negative-results.json'), {
    target: {
      NEXT_TZ_NEGATIVE_01: 'EXPORT_MARKDOWN_COMMAND_WITHOUT_STAGE_FLAG_EXPECT_REJECT',
      NEXT_TZ_NEGATIVE_02: 'MINDMAP_COMMAND_WITHOUT_DERIVED_GUARD_EXPECT_REJECT',
      NEXT_TZ_NEGATIVE_03: 'MODE_PROFILE_VISIBILITY_CONFLICT_EXPECT_REJECT',
      NEXT_TZ_NEGATIVE_04: 'COMMAND_BOUND_AS_BLOCKING_WITHOUT_MACHINE_BINDING_EXPECT_REJECT',
      NEXT_TZ_NEGATIVE_05: 'CAPABILITY_MISMATCH_EXPECT_REJECT',
    },
  ...
  writeJson(path.join(outputDir, 'positive-results.json'), {
    target: {
      NEXT_TZ_POSITIVE_01: 'EXPORT_MARKDOWN_MENU_PATH_ENABLED_WHEN_ALLOWED',
      NEXT_TZ_POSITIVE_02: 'MINDMAP_BRIDGE_MENU_PATH_ENABLED_WHEN_ALLOWED',
      NEXT_TZ_POSITIVE_03: 'STAGE_GATED_POLICY_DETERMINISTIC_AND_STABLE',
    },
    results: run1.positiveResults,
    details: {
      enabledWhenAllowed: run1.baseline.enabledWhenAllowed,
      determinism: run1.determinism,
      stageActivation: run1.stageActivation,
    },
  });

  writeJson(path.join(outputDir, 'export-mindmap-bridge-summary.json'), {
    bridgeEntryCount: run1.counts.bridgeEntryCount,
  ...
  writeJson(path.join(outputDir, 'final-dod-acceptance-summary.json'), {
    ...
    finalGate: summary.status === 'PASS' ? 'GO_TO_X16_WS03' : 'HOLD',
  });
