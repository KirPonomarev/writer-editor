...
const TOKEN_NAME = 'X30_WS03_EVIDENCE_MAINTENANCE_OK';
const EXPECTED_CANON_VERSION = 'v3.13a-final';

const PACKET_FILE_ALLOWLIST = new Set([
  'docs/OPS/EVIDENCE/X30_CONTOUR/',
  'docs/OPS/EVIDENCE/X30_CONTOUR/TICKET_03/',
  'docs/OPS/EVIDENCE/X30_CONTOUR/TICKET_03/negative-results.json',
  'docs/OPS/EVIDENCE/X30_CONTOUR/TICKET_03/positive-results.json',
  'docs/OPS/EVIDENCE/X30_CONTOUR/TICKET_03/final-dod-acceptance-summary.json',
  'docs/OPS/EVIDENCE/X30_CONTOUR/TICKET_03/summary.json',
  'docs/OPS/EVIDENCE/X30_CONTOUR/TICKET_03/ticket-meta.json',
  'docs/OPS/STATUS/X30_WS03_EVIDENCE_MAINTENANCE_STATUS_V1.json',
  'scripts/ops/x30-ws03-evidence-maintenance-state.mjs',
  'scripts/ops/x30-ws03-evidence-maintenance-report.mjs',
  'test/contracts/x30-ws03-evidence-maintenance.contract.test.js',
]);
...
function evaluateEvidenceChain(repoRoot) {
  const nextRecordPath = path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V15.json');
  const x29SignedPath = path.join(repoRoot, 'docs/OPS/STATUS/X29_CONTOUR_CLOSEOUT_SIGNED_V1.json');
  const x29FreezePath = path.join(repoRoot, 'docs/OPS/EVIDENCE/X29_CONTOUR/x29-contour-freeze-packet-v1.json');
  ...
  const nextRecordRefOk = normalizeString(nextRecord?.dependsOn?.x29CloseoutArtifactRef)
    === 'docs/OPS/STATUS/X29_CONTOUR_CLOSEOUT_SIGNED_V1.json';

  const signedRefOk = normalizeString(x29Signed?.evidencePacketRef)
    === 'docs/OPS/EVIDENCE/X29_CONTOUR/x29-contour-freeze-packet-v1.json';
...
function evaluateEvidenceRefCompleteness(repoRoot) {
  const requiredRefs = [
    'docs/OPS/STATUS/X30_WS01_NEXT_CONTOUR_ALIGNMENT_STATUS_V1.json',
    'docs/OPS/STATUS/X30_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json',
    'docs/OPS/EVIDENCE/X30_CONTOUR/TICKET_01/summary.json',
    'docs/OPS/EVIDENCE/X30_CONTOUR/TICKET_01/final-dod-acceptance-summary.json',
    'docs/OPS/EVIDENCE/X30_CONTOUR/TICKET_02/summary.json',
    'docs/OPS/EVIDENCE/X30_CONTOUR/TICKET_02/final-dod-acceptance-summary.json',
    'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V15.json',
    'docs/OPS/STATUS/X29_CONTOUR_CLOSEOUT_SIGNED_V1.json',
    'docs/OPS/EVIDENCE/X29_CONTOUR/x29-contour-freeze-packet-v1.json',
  ];
...
function evaluateLinkStaleness(repoRoot) {
  const ws02StatusPath = path.join(repoRoot, 'docs/OPS/STATUS/X30_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json');
...
export function evaluateX30Ws03EvidenceMaintenanceState(input = {}) {
  ...
  const nextRecord = readJsonObject(path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V15.json'));
  ...
  return {
    ...
    acceptance,
    determinism,
    finalGate: 'GO_TO_X30_WS04_ONLY_IF_ALL_DOD_AND_ACCEPTANCE_TRUE',
...
function main() {
  ...
  const state = evaluateX30Ws03EvidenceMaintenanceState({ repoRoot: process.cwd() });

  if (args.json) {
    process.stdout.write(`${stableStringify(state)}\n`);
  } else {
    process.stdout.write(`X30_WS03_EVIDENCE_MAINTENANCE_OK=${state.X30_WS03_EVIDENCE_MAINTENANCE_OK}\n`);
