...
const TOKEN_NAME = 'X62_WS03_EVIDENCE_MAINTENANCE_OK';
const EXPECTED_CANON_VERSION = 'v3.13a-final';

const PACKET_FILE_ALLOWLIST = new Set([
  'docs/OPS/EVIDENCE/X62_CONTOUR/',
  'docs/OPS/EVIDENCE/X62_CONTOUR/TICKET_03/',
  'docs/OPS/EVIDENCE/X62_CONTOUR/TICKET_03/negative-results.json',
  'docs/OPS/EVIDENCE/X62_CONTOUR/TICKET_03/positive-results.json',
  'docs/OPS/EVIDENCE/X62_CONTOUR/TICKET_03/final-dod-acceptance-summary.json',
  'docs/OPS/EVIDENCE/X62_CONTOUR/TICKET_03/summary.json',
  'docs/OPS/EVIDENCE/X62_CONTOUR/TICKET_03/ticket-meta.json',
  'docs/OPS/STATUS/X62_WS03_EVIDENCE_MAINTENANCE_STATUS_V1.json',
  'scripts/ops/x62-ws03-evidence-maintenance-state.mjs',
  'scripts/ops/x62-ws03-evidence-maintenance-report.mjs',
  'test/contracts/x62-ws03-evidence-maintenance.contract.test.js',
]);
...
function evaluateEvidenceChain(repoRoot) {
  ...
  const x61SignedPath = path.join(repoRoot, 'docs/OPS/STATUS/X61_CONTOUR_CLOSEOUT_SIGNED_V1.json');
  const x61FreezePath = path.join(repoRoot, 'docs/OPS/EVIDENCE/X61_CONTOUR/x61-contour-freeze-packet-v1.json');

  const nextRecord = readJsonObject(nextRecordPath);
  const x61Signed = readJsonObject(x61SignedPath);
  const x61Freeze = readJsonObject(x61FreezePath);

  const missing = [];
  if (!nextRecord) missing.push('NEXT_RECORD_MISSING');
  if (!x61Signed) missing.push('X61_SIGNED_MISSING');
  if (!x61Freeze) missing.push('X61_FREEZE_MISSING');

  const nextRecordRefOk = normalizeString(nextRecord?.dependsOn?.x61CloseoutArtifactRef)
    === 'docs/OPS/STATUS/X61_CONTOUR_CLOSEOUT_SIGNED_V1.json';

  const signedRefOk = normalizeString(x61Signed?.evidencePacketRef)
    === 'docs/OPS/EVIDENCE/X61_CONTOUR/x61-contour-freeze-packet-v1.json';

  let hashMatch = false;
  if (x61Signed && fs.existsSync(x61FreezePath)) {
    const actual = createHash('sha256').update(fs.readFileSync(x61FreezePath)).digest('hex');
    hashMatch = normalizeString(x61Signed.evidencePacketSha256) === `sha256:${actual}`;
  }
...
function evaluateEvidenceRefCompleteness(repoRoot) {
  const requiredRefs = [
    'docs/OPS/STATUS/X62_WS01_NEXT_CONTOUR_ALIGNMENT_STATUS_V1.json',
    'docs/OPS/STATUS/X62_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json',
    'docs/OPS/EVIDENCE/X62_CONTOUR/TICKET_01/summary.json',
    'docs/OPS/EVIDENCE/X62_CONTOUR/TICKET_01/final-dod-acceptance-summary.json',
    'docs/OPS/EVIDENCE/X62_CONTOUR/TICKET_02/summary.json',
    'docs/OPS/EVIDENCE/X62_CONTOUR/TICKET_02/final-dod-acceptance-summary.json',
...
function evaluateLinkStaleness(repoRoot) {
  const ws02StatusPath = path.join(repoRoot, 'docs/OPS/STATUS/X62_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json');
...
export function evaluateX62Ws03EvidenceMaintenanceState(input = {}) {
  ...
  return {
    ...
    acceptance,
    determinism,
    finalGate: 'GO_TO_X62_WS04_ONLY_IF_ALL_DOD_AND_ACCEPTANCE_TRUE',
...
function main() {
  ...
  const state = evaluateX62Ws03EvidenceMaintenanceState({ repoRoot: process.cwd() });

  if (args.json) {
    process.stdout.write(`${stableStringify(state)}\n`);
  } else {
    process.stdout.write(`X62_WS03_EVIDENCE_MAINTENANCE_OK=${state.X62_WS03_EVIDENCE_MAINTENANCE_OK}\n`);
