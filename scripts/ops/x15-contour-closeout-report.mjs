...
const DEFAULT_FREEZE_PATH = 'docs/OPS/EVIDENCE/X15_CONTOUR/x15-contour-freeze-packet-v1.json';
const DEFAULT_CLOSE_SUMMARY_PATH = 'docs/OPS/STATUS/X15_CONTOUR_CLOSE_SUMMARY_V1.json';
const DEFAULT_SIGNED_PATH = 'docs/OPS/STATUS/X15_CONTOUR_CLOSEOUT_SIGNED_V1.json';
const DEFAULT_NEXT_RECORD_PATH = 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V2.json';
...
const WS_MAP = [
  {
    id: 'WS01',
    ticketId: 'X15_WS01',
    statusRef: 'docs/OPS/STATUS/X15_WS01_UI_MENU_EXPANSION_FOUNDATION_v1.json',
    summaryRef: 'docs/OPS/EVIDENCE/X15_CONTOUR/TICKET_01/summary.json',
  },
  {
    id: 'WS02',
    ticketId: 'X15_WS02',
    statusRef: 'docs/OPS/STATUS/X15_WS02_MENU_COMPOSER_LEVELS_v1.json',
    summaryRef: 'docs/OPS/EVIDENCE/X15_CONTOUR/TICKET_02/summary.json',
  },
  {
    id: 'WS03',
    ticketId: 'X15_WS03',
    statusRef: 'docs/OPS/STATUS/X15_WS03_PROFILE_PRESETS_v1.json',
    summaryRef: 'docs/OPS/EVIDENCE/X15_CONTOUR/TICKET_03/summary.json',
  },
  {
    id: 'WS04',
    ticketId: 'X15_WS04',
    statusRef: 'docs/OPS/STATUS/X15_WS04_WRITE_PLAN_REVIEW_SHELLS_v1.json',
    summaryRef: 'docs/OPS/EVIDENCE/X15_CONTOUR/TICKET_04/summary.json',
  },
  {
    id: 'WS05',
    ticketId: 'X15_WS05',
    statusRef: 'docs/OPS/STATUS/X15_WS05_COMMAND_VISIBILITY_REFINEMENT_v1.json',
    summaryRef: 'docs/OPS/EVIDENCE/X15_CONTOUR/TICKET_05/summary.json',
  },
];
...
function parseArgs(argv = process.argv.slice(2)) {
  const out = {
    ...
    runId: 'TZ_X15_CONTOUR_CLOSEOUT_001',
  };
...
function main() {
  ...
  const freezePacket = {
    artifactId: 'X15_CONTOUR_EVIDENCE_FREEZE_PACKET_V1',
    schemaVersion: 1,
    generatedAtUtc,
    owner: 'KIRILL_PLUS_CODEX',
    gate: {
      name: 'GO_IF_X15_WS01_TO_WS05_ALL_ACCEPTED',
    ...
    requiredOutputs: [
      'X15_CONTOUR_FREEZE_PACKET_JSON',
      'X15_CONTOUR_CLOSE_SUMMARY_JSON',
      'X15_CONTOUR_CLOSEOUT_SIGNED_JSON',
  ...
  // SHA lock is finalized against the final serialized freeze packet bytes.
  ...
  const signedDoc = {
    artifactId: 'X15_CONTOUR_CLOSEOUT_SIGNED_V1',
    schemaVersion: 1,
    taskId: args.runId,
    status: 'SIGNED',
    ...
    gate: {
      name: 'GO_IF_X15_CLOSEOUT_ARTIFACT_SIGNED',
      isOpen: true,
  ...
  const shaLockMatchPass = normalizeString(readJsonOrNull(signedPath)?.evidencePacketSha256) === `sha256:${computeSha256OfFile(freezePath)}`;

  const closeSummary = {
    artifactId: 'X15_CONTOUR_CLOSE_SUMMARY_V1',
    schemaVersion: 1,
    taskId: args.runId,
    ...
    closeout: {
      action01_freezeX15EvidencePacket: 'DONE',
      action02_recordX15ContourCloseSummary: 'DONE',
      action03_openNextContourRecord: 'DONE',
    },
    nextTicket: {
      tzId: 'TZ_X16_CONTOUR_OPENING_001',
      priority: 'P2',
      objective: 'OPEN_NEXT_CONTOUR_WITH_CANON_LOCK_AND_NON_EXPANDING_BLOCKING_SURFACE',
      gate: 'GO_IF_X15_CLOSEOUT_ARTIFACT_SIGNED',
    },
  };
  ...
  const nextRecord = {
    ticketId: 'TZ_X16_CONTOUR_OPENING_001',
    schemaVersion: 1,
    status: 'READY',
    priority: 'P2',
    mode: 'IMPLEMENTATION',
    ...
    dependsOn: {
      x15CloseoutArtifactRef: path.relative(repoRoot, signedPath).replaceAll(path.sep, '/'),
      requiredGate: 'GO_IF_X15_CLOSEOUT_ARTIFACT_SIGNED',
      gateSatisfied: true,
    },
    ...
    entryChecks: {
      activeCanonLockCheckPass: activeCanonLockCheckPass,
      stageActivationGuardCheckPass: stageActivationGuardCheckPass,
      blockingSurfaceExpansion: false,
      advisoryAsBlockingDrift: advisoryAsBlockingDrift ? 'NON_ZERO' : 'ZERO',
      offlineReleaseIntegrityLocallyVerifiable: offlineReleaseIntegrityLocallyVerifiable,
      x15ContourClosed: allWsAccepted,
    },
