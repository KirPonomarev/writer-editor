...
const TOKEN_NAME = 'X26_WS03_EVIDENCE_MAINTENANCE_OK';
const EXPECTED_CANON_VERSION = 'v3.13a-final';

const PACKET_FILE_ALLOWLIST = new Set([
  'docs/OPS/EVIDENCE/X26_CONTOUR/',
  'docs/OPS/EVIDENCE/X26_CONTOUR/TICKET_03/',
  'docs/OPS/EVIDENCE/X26_CONTOUR/TICKET_03/negative-results.json',
  'docs/OPS/EVIDENCE/X26_CONTOUR/TICKET_03/positive-results.json',
  'docs/OPS/EVIDENCE/X26_CONTOUR/TICKET_03/final-dod-acceptance-summary.json',
  'docs/OPS/EVIDENCE/X26_CONTOUR/TICKET_03/summary.json',
  'docs/OPS/EVIDENCE/X26_CONTOUR/TICKET_03/ticket-meta.json',
  'docs/OPS/STATUS/X26_WS03_EVIDENCE_MAINTENANCE_STATUS_V1.json',
  'scripts/ops/x26-ws03-evidence-maintenance-state.mjs',
  'scripts/ops/x26-ws03-evidence-maintenance-report.mjs',
  'test/contracts/x26-ws03-evidence-maintenance.contract.test.js',
]);
...
function evaluateEvidenceChain(repoRoot) {
  const nextRecordPath = path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V11.json');
  const x25SignedPath = path.join(repoRoot, 'docs/OPS/STATUS/X25_CONTOUR_CLOSEOUT_SIGNED_V1.json');
  const x25FreezePath = path.join(repoRoot, 'docs/OPS/EVIDENCE/X25_CONTOUR/x25-contour-freeze-packet-v1.json');

  const nextRecord = readJsonObject(nextRecordPath);
  const x25Signed = readJsonObject(x25SignedPath);
  const x25Freeze = readJsonObject(x25FreezePath);

  const missing = [];
  if (!nextRecord) missing.push('NEXT_RECORD_MISSING');
  if (!x25Signed) missing.push('X25_SIGNED_MISSING');
  if (!x25Freeze) missing.push('X25_FREEZE_MISSING');

  const nextRecordRefOk = normalizeString(nextRecord?.dependsOn?.x25CloseoutArtifactRef)
    === 'docs/OPS/STATUS/X25_CONTOUR_CLOSEOUT_SIGNED_V1.json';

  const signedRefOk = normalizeString(x25Signed?.evidencePacketRef)
    === 'docs/OPS/EVIDENCE/X25_CONTOUR/x25-contour-freeze-packet-v1.json';

  let hashMatch = false;
  if (x25Signed && fs.existsSync(x25FreezePath)) {
    const actual = createHash('sha256').update(fs.readFileSync(x25FreezePath)).digest('hex');
    hashMatch = normalizeString(x25Signed.evidencePacketSha256) === `sha256:${actual}`;
  }
...
function evaluateEvidenceRefCompleteness(repoRoot) {
  const requiredRefs = [
    'docs/OPS/STATUS/X26_WS01_NEXT_CONTOUR_ALIGNMENT_STATUS_V1.json',
    'docs/OPS/STATUS/X26_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json',
    'docs/OPS/EVIDENCE/X26_CONTOUR/TICKET_01/summary.json',
    'docs/OPS/EVIDENCE/X26_CONTOUR/TICKET_01/final-dod-acceptance-summary.json',
    'docs/OPS/EVIDENCE/X26_CONTOUR/TICKET_02/summary.json',
    'docs/OPS/EVIDENCE/X26_CONTOUR/TICKET_02/final-dod-acceptance-summary.json',
    'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V11.json',
    'docs/OPS/STATUS/X25_CONTOUR_CLOSEOUT_SIGNED_V1.json',
    'docs/OPS/EVIDENCE/X25_CONTOUR/x25-contour-freeze-packet-v1.json',
  ];
...
function evaluateLinkStaleness(repoRoot) {
  const ws02StatusPath = path.join(repoRoot, 'docs/OPS/STATUS/X26_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json');
...
export function evaluateX26Ws03EvidenceMaintenanceState(input = {}) {
  ...
  const nextRecord = readJsonObject(path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V11.json'));
  ...
  return {
    ...
    acceptance,
    determinism,
    finalGate: 'GO_TO_X26_WS04_ONLY_IF_ALL_DOD_AND_ACCEPTANCE_TRUE',
...
function main() {
  ...
  const state = evaluateX26Ws03EvidenceMaintenanceState({ repoRoot: process.cwd() });

  if (args.json) {
    process.stdout.write(`${stableStringify(state)}\n`);
  } else {
    process.stdout.write(`X26_WS03_EVIDENCE_MAINTENANCE_OK=${state.X26_WS03_EVIDENCE_MAINTENANCE_OK}\n`);
