...
import { evaluateX17Ws01FileEditCoreActionsState } from './x17-ws01-file-edit-core-actions-state.mjs';

const DEFAULT_OUTPUT_DIR = 'docs/OPS/EVIDENCE/X17_CONTOUR/TICKET_01';
const DEFAULT_STATUS_PATH = 'docs/OPS/STATUS/X17_WS01_FILE_EDIT_CORE_ACTIONS_v1.json';
...
function main() {
  ...
  const run1 = evaluateX17Ws01FileEditCoreActionsState({ repoRoot });
  const run2 = evaluateX17Ws01FileEditCoreActionsState({ repoRoot });
  const run3 = evaluateX17Ws01FileEditCoreActionsState({ repoRoot });
  ...
  const summary = {
    ...
    objective: 'ENABLE_FILE_EDIT_CORE_ACTIONS_COMMAND_BUS_DETERMINISTIC_CAPABILITY_POLICY',
  ...
  const statusDoc = {
    version: 1,
    token: 'X17_WS01_FILE_EDIT_CORE_ACTIONS_OK',
  ...
  writeJson(path.join(outputDir, 'negative-results.json'), {
    target: {
      ...
      NEXT_TZ_NEGATIVE_04: 'REQUIRED_CORE_ACTION_HIDDEN_EXPECT_REJECT',
  ...
  writeJson(path.join(outputDir, 'positive-results.json'), {
    target: {
      NEXT_TZ_POSITIVE_01: 'FILE_EDIT_ACTIONS_EXECUTE_VIA_COMMAND_BUS',
  ...
  writeJson(path.join(outputDir, 'file-edit-action-summary.json'), {
  ...
  writeJson(path.join(outputDir, 'final-dod-acceptance-summary.json'), {
    ...
    finalGate: summary.status === 'PASS' ? 'GO_TO_X17_WS02' : 'HOLD',
  });
