...
const TOKEN_NAME = 'X49_WS03_EVIDENCE_MAINTENANCE_OK';
const EXPECTED_CANON_VERSION = 'v3.13a-final';

const PACKET_FILE_ALLOWLIST = new Set([
  'docs/OPS/EVIDENCE/X49_CONTOUR/',
  'docs/OPS/EVIDENCE/X49_CONTOUR/TICKET_03/',
  'docs/OPS/EVIDENCE/X49_CONTOUR/TICKET_03/negative-results.json',
  'docs/OPS/EVIDENCE/X49_CONTOUR/TICKET_03/positive-results.json',
  'docs/OPS/EVIDENCE/X49_CONTOUR/TICKET_03/final-dod-acceptance-summary.json',
  'docs/OPS/EVIDENCE/X49_CONTOUR/TICKET_03/summary.json',
  'docs/OPS/EVIDENCE/X49_CONTOUR/TICKET_03/ticket-meta.json',
  'docs/OPS/STATUS/X49_WS03_EVIDENCE_MAINTENANCE_STATUS_V1.json',
  'scripts/ops/x49-ws03-evidence-maintenance-state.mjs',
  'scripts/ops/x49-ws03-evidence-maintenance-report.mjs',
  'test/contracts/x49-ws03-evidence-maintenance.contract.test.js',
]);
...
function evaluateEvidenceChain(repoRoot) {
  const nextRecordPath = path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V34.json');
  const x48SignedPath = path.join(repoRoot, 'docs/OPS/STATUS/X48_CONTOUR_CLOSEOUT_SIGNED_V1.json');
  const x48FreezePath = path.join(repoRoot, 'docs/OPS/EVIDENCE/X48_CONTOUR/x48-contour-freeze-packet-v1.json');

  const nextRecord = readJsonObject(nextRecordPath);
  const x48Signed = readJsonObject(x48SignedPath);
  const x48Freeze = readJsonObject(x48FreezePath);

  const missing = [];
  if (!nextRecord) missing.push('NEXT_RECORD_MISSING');
  if (!x48Signed) missing.push('X48_SIGNED_MISSING');
  if (!x48Freeze) missing.push('X48_FREEZE_MISSING');

  const nextRecordRefOk = normalizeString(nextRecord?.dependsOn?.x48CloseoutArtifactRef)
    === 'docs/OPS/STATUS/X48_CONTOUR_CLOSEOUT_SIGNED_V1.json';

  const signedRefOk = normalizeString(x48Signed?.evidencePacketRef)
    === 'docs/OPS/EVIDENCE/X48_CONTOUR/x48-contour-freeze-packet-v1.json';

  let hashMatch = false;
  if (x48Signed && fs.existsSync(x48FreezePath)) {
    const actual = createHash('sha256').update(fs.readFileSync(x48FreezePath)).digest('hex');
    hashMatch = normalizeString(x48Signed.evidencePacketSha256) === `sha256:${actual}`;
  }
...
function evaluateEvidenceRefCompleteness(repoRoot) {
  const requiredRefs = [
    'docs/OPS/STATUS/X49_WS01_NEXT_CONTOUR_ALIGNMENT_STATUS_V1.json',
    'docs/OPS/STATUS/X49_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json',
    'docs/OPS/EVIDENCE/X49_CONTOUR/TICKET_01/summary.json',
    'docs/OPS/EVIDENCE/X49_CONTOUR/TICKET_01/final-dod-acceptance-summary.json',
    'docs/OPS/EVIDENCE/X49_CONTOUR/TICKET_02/summary.json',
    'docs/OPS/EVIDENCE/X49_CONTOUR/TICKET_02/final-dod-acceptance-summary.json',
    'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V34.json',
    'docs/OPS/STATUS/X48_CONTOUR_CLOSEOUT_SIGNED_V1.json',
    'docs/OPS/EVIDENCE/X48_CONTOUR/x48-contour-freeze-packet-v1.json',
  ];
...
function evaluateLinkStaleness(repoRoot) {
  const ws02StatusPath = path.join(repoRoot, 'docs/OPS/STATUS/X49_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json');
...
export function evaluateX49Ws03EvidenceMaintenanceState(input = {}) {
  ...
  const nextRecord = readJsonObject(path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V34.json'));
  ...
  return {
    ...
    acceptance,
    determinism,
    finalGate: 'GO_TO_X49_WS04_ONLY_IF_ALL_DOD_AND_ACCEPTANCE_TRUE',
...
function main() {
  ...
  const state = evaluateX49Ws03EvidenceMaintenanceState({ repoRoot: process.cwd() });

  if (args.json) {
    process.stdout.write(`${stableStringify(state)}\n`);
  } else {
    process.stdout.write(`X49_WS03_EVIDENCE_MAINTENANCE_OK=${state.X49_WS03_EVIDENCE_MAINTENANCE_OK}\n`);
