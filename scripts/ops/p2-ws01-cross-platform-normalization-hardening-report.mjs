...
import { evaluateP2Ws01CrossPlatformNormalizationHardeningState } from './p2-ws01-cross-platform-normalization-hardening-state.mjs';

const DEFAULT_OUTPUT_DIR = 'docs/OPS/EVIDENCE/P2_CONTOUR/TICKET_01';
const DEFAULT_STATUS_PATH = 'docs/OPS/STATUS/P2_WS01_CROSS_PLATFORM_NORMALIZATION_HARDENING_v1.json';
...
function comparableState(state) {
  return {
    ...
    platformGapEntries: state.platformGapEntries,
...
function main() {
  ...
  const run1 = evaluateP2Ws01CrossPlatformNormalizationHardeningState({ repoRoot });
  const run2 = evaluateP2Ws01CrossPlatformNormalizationHardeningState({ repoRoot });
  const run3 = evaluateP2Ws01CrossPlatformNormalizationHardeningState({ repoRoot });
  ...
  const summary = {
    ...
    counts: run1.counts,
    platformGapEntries: run1.platformGapEntries,
  ...
  const statusDoc = {
    version: 1,
    token: 'P2_WS01_CROSS_PLATFORM_NORMALIZATION_HARDENING_OK',
    platformGapCount: summary.counts.platformGapCount,
  ...
  writeJson(path.join(outputDir, 'platform-gap-summary.json'), {
    platformGapCount: run1.counts.platformGapCount,
    platformGapEntries: run1.platformGapEntries,
    scopeChecks: run1.scopeChecks,
    counts: run1.counts,
  });

  writeJson(path.join(outputDir, 'negative-results.json'), {
    target: {
      NEXT_TZ_NEGATIVE_01: 'CASE_COLLISION_EXPECT_FAIL',
      NEXT_TZ_NEGATIVE_02: 'RESERVED_NAME_EXPECT_FAIL',
      NEXT_TZ_NEGATIVE_03: 'INVALID_SEPARATOR_MIX_EXPECT_FAIL',
      NEXT_TZ_NEGATIVE_04: 'NEWLINE_MISMATCH_EXPECT_FAIL',
      NEXT_TZ_NEGATIVE_05: 'UNICODE_NORMALIZATION_MISMATCH_EXPECT_FAIL',
    },
  ...
  writeJson(path.join(outputDir, 'positive-results.json'), {
    target: {
      NEXT_TZ_POSITIVE_01: 'PLATFORM_MATRIX_NORMALIZATION_PASS',
      NEXT_TZ_POSITIVE_02: 'DETERMINISTIC_OUTPUT_ACROSS_PLATFORMS_PASS',
  ...
  writeJson(path.join(outputDir, 'final-dod-acceptance-summary.json'), {
    ...
    finalGate: summary.status === 'PASS' ? 'GO_TO_P2_WS02' : 'HOLD',
  });
