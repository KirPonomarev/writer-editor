...
const TOKEN_NAME = 'X34_WS03_EVIDENCE_MAINTENANCE_OK';
const EXPECTED_CANON_VERSION = 'v3.13a-final';

const PACKET_FILE_ALLOWLIST = new Set([
  'docs/OPS/EVIDENCE/X34_CONTOUR/',
  'docs/OPS/EVIDENCE/X34_CONTOUR/TICKET_03/',
  'docs/OPS/EVIDENCE/X34_CONTOUR/TICKET_03/negative-results.json',
  'docs/OPS/EVIDENCE/X34_CONTOUR/TICKET_03/positive-results.json',
  'docs/OPS/EVIDENCE/X34_CONTOUR/TICKET_03/final-dod-acceptance-summary.json',
  'docs/OPS/EVIDENCE/X34_CONTOUR/TICKET_03/summary.json',
  'docs/OPS/EVIDENCE/X34_CONTOUR/TICKET_03/ticket-meta.json',
  'docs/OPS/STATUS/X34_WS03_EVIDENCE_MAINTENANCE_STATUS_V1.json',
  'scripts/ops/x34-ws03-evidence-maintenance-state.mjs',
  'scripts/ops/x34-ws03-evidence-maintenance-report.mjs',
  'test/contracts/x34-ws03-evidence-maintenance.contract.test.js',
]);
...
function evaluateEvidenceChain(repoRoot) {
  const nextRecordPath = path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V19.json');
  const x33SignedPath = path.join(repoRoot, 'docs/OPS/STATUS/X33_CONTOUR_CLOSEOUT_SIGNED_V1.json');
  const x33FreezePath = path.join(repoRoot, 'docs/OPS/EVIDENCE/X33_CONTOUR/x33-contour-freeze-packet-v1.json');

  const nextRecord = readJsonObject(nextRecordPath);
  const x33Signed = readJsonObject(x33SignedPath);
  const x33Freeze = readJsonObject(x33FreezePath);

  const missing = [];
  if (!nextRecord) missing.push('NEXT_RECORD_MISSING');
  if (!x33Signed) missing.push('X33_SIGNED_MISSING');
  if (!x33Freeze) missing.push('X33_FREEZE_MISSING');

  const nextRecordRefOk = normalizeString(nextRecord?.dependsOn?.x33CloseoutArtifactRef)
    === 'docs/OPS/STATUS/X33_CONTOUR_CLOSEOUT_SIGNED_V1.json';

  const signedRefOk = normalizeString(x33Signed?.evidencePacketRef)
    === 'docs/OPS/EVIDENCE/X33_CONTOUR/x33-contour-freeze-packet-v1.json';

  let hashMatch = false;
  if (x33Signed && fs.existsSync(x33FreezePath)) {
    const actual = createHash('sha256').update(fs.readFileSync(x33FreezePath)).digest('hex');
    hashMatch = normalizeString(x33Signed.evidencePacketSha256) === `sha256:${actual}`;
  }
...
function evaluateEvidenceRefCompleteness(repoRoot) {
  const requiredRefs = [
    'docs/OPS/STATUS/X34_WS01_NEXT_CONTOUR_ALIGNMENT_STATUS_V1.json',
    'docs/OPS/STATUS/X34_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json',
    'docs/OPS/EVIDENCE/X34_CONTOUR/TICKET_01/summary.json',
    'docs/OPS/EVIDENCE/X34_CONTOUR/TICKET_01/final-dod-acceptance-summary.json',
    'docs/OPS/EVIDENCE/X34_CONTOUR/TICKET_02/summary.json',
    'docs/OPS/EVIDENCE/X34_CONTOUR/TICKET_02/final-dod-acceptance-summary.json',
    'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V19.json',
    'docs/OPS/STATUS/X33_CONTOUR_CLOSEOUT_SIGNED_V1.json',
    'docs/OPS/EVIDENCE/X33_CONTOUR/x33-contour-freeze-packet-v1.json',
  ];
...
function evaluateLinkStaleness(repoRoot) {
  const ws02StatusPath = path.join(repoRoot, 'docs/OPS/STATUS/X34_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json');
...
export function evaluateX34Ws03EvidenceMaintenanceState(input = {}) {
  ...
  const nextRecord = readJsonObject(path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V19.json'));
  ...
  return {
    ...
    acceptance,
    determinism,
    finalGate: 'GO_TO_X34_WS04_ONLY_IF_ALL_DOD_AND_ACCEPTANCE_TRUE',
...
function main() {
  ...
  const state = evaluateX34Ws03EvidenceMaintenanceState({ repoRoot: process.cwd() });

  if (args.json) {
    process.stdout.write(`${stableStringify(state)}\n`);
  } else {
    process.stdout.write(`X34_WS03_EVIDENCE_MAINTENANCE_OK=${state.X34_WS03_EVIDENCE_MAINTENANCE_OK}\n`);
