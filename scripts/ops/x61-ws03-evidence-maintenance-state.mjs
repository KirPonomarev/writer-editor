...
const TOKEN_NAME = 'X61_WS03_EVIDENCE_MAINTENANCE_OK';
const EXPECTED_CANON_VERSION = 'v3.13a-final';

const PACKET_FILE_ALLOWLIST = new Set([
  'docs/OPS/EVIDENCE/X61_CONTOUR/',
  'docs/OPS/EVIDENCE/X61_CONTOUR/TICKET_03/',
  'docs/OPS/EVIDENCE/X61_CONTOUR/TICKET_03/negative-results.json',
  'docs/OPS/EVIDENCE/X61_CONTOUR/TICKET_03/positive-results.json',
  'docs/OPS/EVIDENCE/X61_CONTOUR/TICKET_03/final-dod-acceptance-summary.json',
  'docs/OPS/EVIDENCE/X61_CONTOUR/TICKET_03/summary.json',
  'docs/OPS/EVIDENCE/X61_CONTOUR/TICKET_03/ticket-meta.json',
  'docs/OPS/STATUS/X61_WS03_EVIDENCE_MAINTENANCE_STATUS_V1.json',
  'scripts/ops/x61-ws03-evidence-maintenance-state.mjs',
  'scripts/ops/x61-ws03-evidence-maintenance-report.mjs',
  'test/contracts/x61-ws03-evidence-maintenance.contract.test.js',
]);
...
function evaluateEvidenceChain(repoRoot) {
  const nextRecordPath = path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V45.json');
  const x60SignedPath = path.join(repoRoot, 'docs/OPS/STATUS/X60_CONTOUR_CLOSEOUT_SIGNED_V1.json');
  const x60FreezePath = path.join(repoRoot, 'docs/OPS/EVIDENCE/X60_CONTOUR/x60-contour-freeze-packet-v1.json');

  const nextRecord = readJsonObject(nextRecordPath);
  const x60Signed = readJsonObject(x60SignedPath);
  const x60Freeze = readJsonObject(x60FreezePath);

  const missing = [];
  if (!nextRecord) missing.push('NEXT_RECORD_MISSING');
  if (!x60Signed) missing.push('X60_SIGNED_MISSING');
  if (!x60Freeze) missing.push('X60_FREEZE_MISSING');

  const nextRecordRefOk = normalizeString(nextRecord?.dependsOn?.x60CloseoutArtifactRef)
    === 'docs/OPS/STATUS/X60_CONTOUR_CLOSEOUT_SIGNED_V1.json';

  const signedRefOk = normalizeString(x60Signed?.evidencePacketRef)
    === 'docs/OPS/EVIDENCE/X60_CONTOUR/x60-contour-freeze-packet-v1.json';

  let hashMatch = false;
  if (x60Signed && fs.existsSync(x60FreezePath)) {
    const actual = createHash('sha256').update(fs.readFileSync(x60FreezePath)).digest('hex');
    hashMatch = normalizeString(x60Signed.evidencePacketSha256) === `sha256:${actual}`;
  }
...
function evaluateEvidenceRefCompleteness(repoRoot) {
  const requiredRefs = [
    'docs/OPS/STATUS/X61_WS01_NEXT_CONTOUR_ALIGNMENT_STATUS_V1.json',
    'docs/OPS/STATUS/X61_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json',
    'docs/OPS/EVIDENCE/X61_CONTOUR/TICKET_01/summary.json',
    'docs/OPS/EVIDENCE/X61_CONTOUR/TICKET_01/final-dod-acceptance-summary.json',
    'docs/OPS/EVIDENCE/X61_CONTOUR/TICKET_02/summary.json',
    'docs/OPS/EVIDENCE/X61_CONTOUR/TICKET_02/final-dod-acceptance-summary.json',
...
function evaluateLinkStaleness(repoRoot) {
  const ws02StatusPath = path.join(repoRoot, 'docs/OPS/STATUS/X61_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json');
...
export function evaluateX61Ws03EvidenceMaintenanceState(input = {}) {
  ...
  const nextRecord = readJsonObject(path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V45.json'));
  ...
  return {
    ...
    acceptance,
    determinism,
    finalGate: 'GO_TO_X61_WS04_ONLY_IF_ALL_DOD_AND_ACCEPTANCE_TRUE',
...
function main() {
  ...
  const state = evaluateX61Ws03EvidenceMaintenanceState({ repoRoot: process.cwd() });

  if (args.json) {
    process.stdout.write(`${stableStringify(state)}\n`);
  } else {
    process.stdout.write(`X61_WS03_EVIDENCE_MAINTENANCE_OK=${state.X61_WS03_EVIDENCE_MAINTENANCE_OK}\n`);
