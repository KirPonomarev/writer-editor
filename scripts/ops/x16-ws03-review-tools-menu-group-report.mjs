...
import { evaluateX16Ws03ReviewToolsMenuGroupState } from './x16-ws03-review-tools-menu-group-state.mjs';

const DEFAULT_OUTPUT_DIR = 'docs/OPS/EVIDENCE/X16_CONTOUR/TICKET_03';
const DEFAULT_STATUS_PATH = 'docs/OPS/STATUS/X16_WS03_REVIEW_TOOLS_MENU_GROUP_STATUS_v1.json';
...
function main() {
  ...
  const run1 = evaluateX16Ws03ReviewToolsMenuGroupState({ repoRoot });
  const run2 = evaluateX16Ws03ReviewToolsMenuGroupState({ repoRoot });
  const run3 = evaluateX16Ws03ReviewToolsMenuGroupState({ repoRoot });
  ...
  const summary = {
    ...
    objective: 'ENABLE_REVIEW_TOOLS_MENU_GROUP_LOCAL_FIRST_STAGE_GATED_NO_CORE_LEAK',
  ...
  const statusDoc = {
    version: 1,
    token: 'X16_WS03_REVIEW_TOOLS_MENU_GROUP_OK',
    reviewEntryCount: summary.counts.reviewEntryCount,
    ...
    missingReviewGroupBindingsCount: summary.counts.missingReviewGroupBindingsCount,
    localFirstPolicyErrorCount: summary.counts.localFirstPolicyErrorCount,
  ...
  writeJson(path.join(outputDir, 'negative-results.json'), {
    target: {
      NEXT_TZ_NEGATIVE_01: 'REVIEW_COMMAND_WITHOUT_LOCAL_FIRST_FLAG_EXPECT_REJECT',
      NEXT_TZ_NEGATIVE_02: 'REVIEW_GROUP_VISIBLE_IN_INVALID_MODE_EXPECT_REJECT',
      NEXT_TZ_NEGATIVE_03: 'REVIEW_GROUP_WITH_CAPABILITY_MISMATCH_EXPECT_REJECT',
      NEXT_TZ_NEGATIVE_04: 'REVIEW_GROUP_BOUND_AS_BLOCKING_WITHOUT_MACHINE_BINDING_EXPECT_REJECT',
      NEXT_TZ_NEGATIVE_05: 'HISTORY_COMPARE_WITHOUT_REQUIRED_GUARD_EXPECT_REJECT',
    },
  ...
  writeJson(path.join(outputDir, 'positive-results.json'), {
    target: {
      NEXT_TZ_POSITIVE_01: 'REVIEW_GROUP_COMMANDS_VISIBLE_WHEN_ALLOWED',
      NEXT_TZ_POSITIVE_02: 'LOCAL_FIRST_REVIEW_POLICY_DETERMINISTIC',
      NEXT_TZ_POSITIVE_03: 'COMMENTS_HISTORY_COMPARE_BINDINGS_CONSISTENT',
    },
    results: run1.positiveResults,
    details: {
      enabledWhenAllowed: run1.baseline.enabledWhenAllowed,
      localFirstPolicyErrors: run1.baseline.localFirstPolicyErrors,
      determinism: run1.determinism,
    },
  });

  writeJson(path.join(outputDir, 'review-tools-group-summary.json'), {
    reviewEntryCount: run1.counts.reviewEntryCount,
    ...
    missingReviewGroupBindingsCount: run1.counts.missingReviewGroupBindingsCount,
    localFirstPolicyErrorCount: run1.counts.localFirstPolicyErrorCount,
  ...
  writeJson(path.join(outputDir, 'final-dod-acceptance-summary.json'), {
    ...
    finalGate: summary.status === 'PASS' ? 'GO_TO_X16_WS04' : 'HOLD',
  });
