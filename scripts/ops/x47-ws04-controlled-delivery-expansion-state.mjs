...
const TOKEN_NAME = 'X47_WS04_CONTROLLED_DELIVERY_EXPANSION_OK';
...
const PACKET_FILE_ALLOWLIST = new Set([
  ...
  'docs/OPS/EVIDENCE/X47_CONTOUR/TICKET_04/',
  'docs/OPS/EVIDENCE/X47_CONTOUR/TICKET_04/negative-results.json',
  'docs/OPS/EVIDENCE/X47_CONTOUR/TICKET_04/positive-results.json',
  'docs/OPS/EVIDENCE/X47_CONTOUR/TICKET_04/final-dod-acceptance-summary.json',
  'docs/OPS/EVIDENCE/X47_CONTOUR/TICKET_04/summary.json',
  'docs/OPS/EVIDENCE/X47_CONTOUR/TICKET_04/ticket-meta.json',
  'docs/OPS/STATUS/X47_WS04_CONTROLLED_DELIVERY_EXPANSION_STATUS_V1.json',
  'scripts/ops/x47-ws04-controlled-delivery-expansion-state.mjs',
  'scripts/ops/x47-ws04-controlled-delivery-expansion-report.mjs',
  'test/contracts/x47-ws04-controlled-delivery-expansion.contract.test.js',
]);
...
function evaluateControlledExpansion(repoRoot) {
  ...
  const x34Signed = readJsonObject(path.join(repoRoot, 'docs/OPS/STATUS/X46_CONTOUR_CLOSEOUT_SIGNED_V1.json'));
  ...
  const missingRefs = [];
  if (!nextRecord) missingRefs.push('NEXT_CONTOUR_OPENING_RECORD_V32_JSON_MISSING');
  if (!x34Signed) missingRefs.push('X46_CONTOUR_CLOSEOUT_SIGNED_V1_JSON_MISSING');
...
export function evaluateX47Ws04ControlledDeliveryExpansionState(input = {}) {
  ...
  return {
    ...
    acceptance,
    determinism,
    finalGate: 'GO_TO_X47_WS99_ONLY_IF_ALL_DOD_AND_ACCEPTANCE_TRUE',
...
function main() {
  ...
  const state = evaluateX47Ws04ControlledDeliveryExpansionState({ repoRoot: process.cwd() });

  if (args.json) {
    process.stdout.write(`${stableStringify(state)}\n`);
  } else {
    process.stdout.write(`X47_WS04_CONTROLLED_DELIVERY_EXPANSION_OK=${state.X47_WS04_CONTROLLED_DELIVERY_EXPANSION_OK}\n`);
