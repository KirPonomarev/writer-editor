...
const TOKEN_NAME = 'X54_WS04_CONTROLLED_DELIVERY_EXPANSION_OK';
...
const PACKET_FILE_ALLOWLIST = new Set([
  ...
  'docs/OPS/EVIDENCE/X54_CONTOUR/TICKET_04/',
  'docs/OPS/EVIDENCE/X54_CONTOUR/TICKET_04/negative-results.json',
  'docs/OPS/EVIDENCE/X54_CONTOUR/TICKET_04/positive-results.json',
  'docs/OPS/EVIDENCE/X54_CONTOUR/TICKET_04/final-dod-acceptance-summary.json',
  'docs/OPS/EVIDENCE/X54_CONTOUR/TICKET_04/summary.json',
  'docs/OPS/EVIDENCE/X54_CONTOUR/TICKET_04/ticket-meta.json',
  'docs/OPS/STATUS/X54_WS04_CONTROLLED_DELIVERY_EXPANSION_STATUS_V1.json',
  'scripts/ops/x54-ws04-controlled-delivery-expansion-state.mjs',
  'scripts/ops/x54-ws04-controlled-delivery-expansion-report.mjs',
  'test/contracts/x54-ws04-controlled-delivery-expansion.contract.test.js',
]);
...
function evaluateControlledExpansion(repoRoot) {
  ...
  const x53Signed = readJsonObject(path.join(repoRoot, 'docs/OPS/STATUS/X53_CONTOUR_CLOSEOUT_SIGNED_V1.json'));
  ...
  if (!x53Signed) missingRefs.push('X53_CONTOUR_CLOSEOUT_SIGNED_V1_JSON_MISSING');
  ...
  const blockingSurfaceExpansionFalse = nextRecord?.entryChecks?.blockingSurfaceExpansion === false
    && x53Signed?.constraints?.blockingSurfaceExpansion === false;
...
export function evaluateX54Ws04ControlledDeliveryExpansionState(input = {}) {
  ...
  return {
    ...
    acceptance,
    determinism,
    finalGate: 'GO_TO_X54_WS99_ONLY_IF_ALL_DOD_AND_ACCEPTANCE_TRUE',
...
function main() {
  ...
  const state = evaluateX54Ws04ControlledDeliveryExpansionState({ repoRoot: process.cwd() });

  if (args.json) {
    process.stdout.write(`${stableStringify(state)}\n`);
  } else {
    process.stdout.write(`X54_WS04_CONTROLLED_DELIVERY_EXPANSION_OK=${state.X54_WS04_CONTROLLED_DELIVERY_EXPANSION_OK}\n`);
