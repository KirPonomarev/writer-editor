...
const DEFAULT_OUTPUT_DIR = 'docs/OPS/EVIDENCE/P1_CONTOUR/TICKET_08';
const DEFAULT_STATUS_PATH = 'docs/OPS/STATUS/CROSS_PLATFORM_NORMALIZATION_HARDENING_v3.json';
...
function main() {
  ...
  const gates = {
    p1_08_newline_normalization_check: state.newlineNormalizationCheck ? 'PASS' : 'FAIL',
    p1_08_unicode_normalization_check: state.unicodeNormalizationCheck ? 'PASS' : 'FAIL',
    p1_08_locale_independent_sorting_check: state.localeIndependentSortingCheck ? 'PASS' : 'FAIL',
    p1_08_case_sensitivity_behavior_check: state.caseSensitivityBehaviorCheck ? 'PASS' : 'FAIL',
    p1_08_reserved_name_behavior_check: state.reservedNameBehaviorCheck ? 'PASS' : 'FAIL',
    p1_08_path_separator_safety_check: state.pathSeparatorSafetyCheck ? 'PASS' : 'FAIL',
    p1_08_release_critical_paths_no_advisory_subset_parity_check: state.releaseCriticalPathsNoAdvisorySubsetParityCheck ? 'PASS' : 'FAIL',
  ...
  const summary = {
    ...
    newlineNormalizationCaseCount: state.newlineNormalizationCases.length,
    unicodeNormalizationCaseCount: state.unicodeNormalizationCases.length,
    localeSortingCaseCount: state.localeIndependentSortingCases.length,
    caseSensitivityCaseCount: state.caseSensitivityBehaviorCases.length,
    reservedNameCaseCount: state.reservedNameBehaviorCases.length,
    pathSeparatorCaseCount: state.pathSeparatorSafetyCases.length,
    releaseCriticalTokenCount: state.releaseCriticalPathParity.requiredTokenIds.length,
  ...
  writeJson(statusPath, {
    version: 3,
    token: 'CROSS_PLATFORM_NORMALIZATION_HARDENING_OK',
  ...
  writeJson(path.join(outputDir, 'newline-normalization-cases.json'), {
    newlineNormalizationCheck: state.newlineNormalizationCheck,
    cases: state.newlineNormalizationCases,
  });

  writeJson(path.join(outputDir, 'unicode-normalization-cases.json'), {
    unicodeNormalizationCheck: state.unicodeNormalizationCheck,
    cases: state.unicodeNormalizationCases,
  });

  writeJson(path.join(outputDir, 'locale-independent-sorting-cases.json'), {
    localeIndependentSortingCheck: state.localeIndependentSortingCheck,
    cases: state.localeIndependentSortingCases,
  });

  writeJson(path.join(outputDir, 'case-sensitivity-reserved-name-cases.json'), {
    caseSensitivityBehaviorCheck: state.caseSensitivityBehaviorCheck,
    reservedNameBehaviorCheck: state.reservedNameBehaviorCheck,
    caseSensitivityCases: state.caseSensitivityBehaviorCases,
    reservedNameCases: state.reservedNameBehaviorCases,
    releaseCriticalPathsNoAdvisorySubsetParityCheck: state.releaseCriticalPathsNoAdvisorySubsetParityCheck,
    releaseCriticalPathParity: state.releaseCriticalPathParity,
  });

  writeJson(path.join(outputDir, 'path-separator-safety-cases.json'), {
    pathSeparatorSafetyCheck: state.pathSeparatorSafetyCheck,
    cases: state.pathSeparatorSafetyCases,
  });
