...
const DEFAULT_FREEZE_PATH = 'docs/OPS/EVIDENCE/X22_CONTOUR/x22-contour-freeze-packet-v1.json';
const DEFAULT_CLOSE_SUMMARY_PATH = 'docs/OPS/STATUS/X22_CONTOUR_CLOSE_SUMMARY_V1.json';
const DEFAULT_SIGNED_PATH = 'docs/OPS/STATUS/X22_CONTOUR_CLOSEOUT_SIGNED_V1.json';
const DEFAULT_NEXT_RECORD_PATH = 'docs/OPS/STATUS/FINAL_TRANSITION_RECORD_V1.json';
...
const WS_MAP = [
  {
    id: 'WS01',
    ticketId: 'X22_WS01',
    statusRef: 'docs/OPS/STATUS/X22_WS01_PROMOTION_CANDIDATE_PACKAGE_v1.json',
    summaryRef: 'docs/OPS/EVIDENCE/X22_CONTOUR/TICKET_01/summary.json',
  },
  {
    id: 'WS02',
    ticketId: 'X22_WS02',
    statusRef: 'docs/OPS/STATUS/X22_WS02_FINAL_PROMOTION_DECISION_PREP_v1.json',
    summaryRef: 'docs/OPS/EVIDENCE/X22_CONTOUR/TICKET_02/summary.json',
  },
];
...
function parseArgs(argv = process.argv.slice(2)) {
  const out = {
    ...
    runId: 'TZ_X22_WS03_CONTOUR_CLOSEOUT_001',
  };
...
function main() {
  ...
  const freezePacket = {
    artifactId: 'X22_CONTOUR_EVIDENCE_FREEZE_PACKET_V1',
    schemaVersion: 1,
    generatedAtUtc,
    owner: 'KIRILL_PLUS_CODEX',
    gate: {
      name: 'GO_IF_X22_WS01_AND_X22_WS02_ACCEPTED',
    ...
    requiredOutputs: [
      'X22_CONTOUR_FREEZE_PACKET_JSON',
      'X22_CONTOUR_CLOSE_SUMMARY_JSON',
      'X22_CONTOUR_CLOSEOUT_SIGNED_JSON',
      'FINAL_TRANSITION_RECORD_JSON',
  ...
  const signedDoc = {
    artifactId: 'X22_CONTOUR_CLOSEOUT_SIGNED_V1',
    schemaVersion: 1,
    taskId: args.runId,
    status: 'SIGNED',
    ...
    gate: {
      name: 'GO_IF_X22_CLOSEOUT_ARTIFACT_SIGNED',
      isOpen: true,
  ...
  const closeSummary = {
    artifactId: 'X22_CONTOUR_CLOSE_SUMMARY_V1',
    schemaVersion: 1,
    taskId: args.runId,
    ...
    closeout: {
      action01_freezeX22EvidencePacket: 'DONE',
      action02_recordX22ContourCloseSummary: 'DONE',
      action03_openNextContourRecord: 'DONE',
    },
    nextTicket: {
      tzId: 'TZ_X22_FINAL_TRANSITION_001',
      priority: 'P1',
      objective: 'EXECUTE_FINAL_TRANSITION_WITH_CANON_LOCK_AND_NON_EXPANDING_BLOCKING_SURFACE',
      gate: 'GO_IF_X22_CLOSEOUT_ARTIFACT_SIGNED',
    },
  };
  ...
  const nextRecord = {
    ticketId: 'TZ_X22_FINAL_TRANSITION_001',
    schemaVersion: 1,
    status: 'READY',
    priority: 'P1',
    mode: 'IMPLEMENTATION',
    ...
    dependsOn: {
      x22CloseoutArtifactRef: path.relative(repoRoot, signedPath).replaceAll(path.sep, '/'),
      requiredGate: 'GO_IF_X22_CLOSEOUT_ARTIFACT_SIGNED',
      gateSatisfied: true,
    },
    objective: 'EXECUTE_FINAL_TRANSITION_WITH_CANON_LOCK_AND_80_20_BLOCKING_DISCIPLINE_PRESERVED',
    scope: {
      in: [
    ...
    entryChecks: {
      ...
      x22ContourClosed: allWsAccepted,
    },
  ...
  const finalGateStatus = Object.values(finalChecks).every(Boolean)
    ? 'GO_TO_FINAL_TRANSITION'
    : 'HOLD';

  const report = {
    status: finalGateStatus === 'GO_TO_FINAL_TRANSITION' ? 'PASS' : 'FAIL',
    runId: args.runId,
    finalGateStatus,
    checks: finalChecks,
    outputs: {
