...
import { evaluateX30Ws01NextContourAlignmentState } from './x30-ws01-next-contour-alignment-state.mjs';

const DEFAULT_OUTPUT_DIR = 'docs/OPS/EVIDENCE/X30_CONTOUR/TICKET_01';
const DEFAULT_STATUS_PATH = 'docs/OPS/STATUS/X30_WS01_NEXT_CONTOUR_ALIGNMENT_STATUS_V1.json';
const DEFAULT_RUN_ID = 'TZ_X30_WS01_NEXT_CONTOUR_ALIGNMENT_001';
...
function main() {
  ...
  const run1 = evaluateX30Ws01NextContourAlignmentState({ repoRoot });
  const run2 = evaluateX30Ws01NextContourAlignmentState({ repoRoot });
  const run3 = evaluateX30Ws01NextContourAlignmentState({ repoRoot });
  ...
  const summary = {
    ...
    objective: 'ALIGN_NEXT_CONTOUR_WITH_CANON_LOCK_AND_80_20_BLOCKING_DISCIPLINE',
