...
const TOKEN_NAME = 'X55_WS03_EVIDENCE_MAINTENANCE_OK';
const EXPECTED_CANON_VERSION = 'v3.13a-final';

const PACKET_FILE_ALLOWLIST = new Set([
  'docs/OPS/EVIDENCE/X55_CONTOUR/',
  'docs/OPS/EVIDENCE/X55_CONTOUR/TICKET_03/',
  'docs/OPS/EVIDENCE/X55_CONTOUR/TICKET_03/negative-results.json',
  'docs/OPS/EVIDENCE/X55_CONTOUR/TICKET_03/positive-results.json',
  'docs/OPS/EVIDENCE/X55_CONTOUR/TICKET_03/final-dod-acceptance-summary.json',
  'docs/OPS/EVIDENCE/X55_CONTOUR/TICKET_03/summary.json',
  'docs/OPS/EVIDENCE/X55_CONTOUR/TICKET_03/ticket-meta.json',
  'docs/OPS/STATUS/X55_WS03_EVIDENCE_MAINTENANCE_STATUS_V1.json',
  'scripts/ops/x55-ws03-evidence-maintenance-state.mjs',
  'scripts/ops/x55-ws03-evidence-maintenance-report.mjs',
  'test/contracts/x55-ws03-evidence-maintenance.contract.test.js',
]);
...
function evaluateEvidenceChain(repoRoot) {
  const nextRecordPath = path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V40.json');
  const x54SignedPath = path.join(repoRoot, 'docs/OPS/STATUS/X54_CONTOUR_CLOSEOUT_SIGNED_V1.json');
  const x54FreezePath = path.join(repoRoot, 'docs/OPS/EVIDENCE/X54_CONTOUR/x54-contour-freeze-packet-v1.json');

  const nextRecord = readJsonObject(nextRecordPath);
  const x54Signed = readJsonObject(x54SignedPath);
  const x54Freeze = readJsonObject(x54FreezePath);

  const missing = [];
  if (!nextRecord) missing.push('NEXT_RECORD_MISSING');
  if (!x54Signed) missing.push('X54_SIGNED_MISSING');
  if (!x54Freeze) missing.push('X54_FREEZE_MISSING');

  const nextRecordRefOk = normalizeString(nextRecord?.dependsOn?.x54CloseoutArtifactRef)
    === 'docs/OPS/STATUS/X54_CONTOUR_CLOSEOUT_SIGNED_V1.json';

  const signedRefOk = normalizeString(x54Signed?.evidencePacketRef)
    === 'docs/OPS/EVIDENCE/X54_CONTOUR/x54-contour-freeze-packet-v1.json';

  let hashMatch = false;
  if (x54Signed && fs.existsSync(x54FreezePath)) {
    const actual = createHash('sha256').update(fs.readFileSync(x54FreezePath)).digest('hex');
    hashMatch = normalizeString(x54Signed.evidencePacketSha256) === `sha256:${actual}`;
  }
...
function evaluateEvidenceRefCompleteness(repoRoot) {
  const requiredRefs = [
    'docs/OPS/STATUS/X55_WS01_NEXT_CONTOUR_ALIGNMENT_STATUS_V1.json',
    'docs/OPS/STATUS/X55_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json',
    'docs/OPS/EVIDENCE/X55_CONTOUR/TICKET_01/summary.json',
    'docs/OPS/EVIDENCE/X55_CONTOUR/TICKET_01/final-dod-acceptance-summary.json',
    'docs/OPS/EVIDENCE/X55_CONTOUR/TICKET_02/summary.json',
    'docs/OPS/EVIDENCE/X55_CONTOUR/TICKET_02/final-dod-acceptance-summary.json',
...
function evaluateLinkStaleness(repoRoot) {
  const ws02StatusPath = path.join(repoRoot, 'docs/OPS/STATUS/X55_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json');
...
export function evaluateX55Ws03EvidenceMaintenanceState(input = {}) {
  ...
  const nextRecord = readJsonObject(path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V40.json'));
  ...
  return {
    ...
    acceptance,
    determinism,
    finalGate: 'GO_TO_X55_WS04_ONLY_IF_ALL_DOD_AND_ACCEPTANCE_TRUE',
...
function main() {
  ...
  const state = evaluateX55Ws03EvidenceMaintenanceState({ repoRoot: process.cwd() });

  if (args.json) {
    process.stdout.write(`${stableStringify(state)}\n`);
  } else {
    process.stdout.write(`X55_WS03_EVIDENCE_MAINTENANCE_OK=${state.X55_WS03_EVIDENCE_MAINTENANCE_OK}\n`);
