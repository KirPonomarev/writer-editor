...
const TOKEN_NAME = 'X27_WS03_EVIDENCE_MAINTENANCE_OK';
const EXPECTED_CANON_VERSION = 'v3.13a-final';

const PACKET_FILE_ALLOWLIST = new Set([
  'docs/OPS/EVIDENCE/X27_CONTOUR/',
  'docs/OPS/EVIDENCE/X27_CONTOUR/TICKET_03/',
  'docs/OPS/EVIDENCE/X27_CONTOUR/TICKET_03/negative-results.json',
  'docs/OPS/EVIDENCE/X27_CONTOUR/TICKET_03/positive-results.json',
  'docs/OPS/EVIDENCE/X27_CONTOUR/TICKET_03/final-dod-acceptance-summary.json',
  'docs/OPS/EVIDENCE/X27_CONTOUR/TICKET_03/summary.json',
  'docs/OPS/EVIDENCE/X27_CONTOUR/TICKET_03/ticket-meta.json',
  'docs/OPS/STATUS/X27_WS03_EVIDENCE_MAINTENANCE_STATUS_V1.json',
  'scripts/ops/x27-ws03-evidence-maintenance-state.mjs',
  'scripts/ops/x27-ws03-evidence-maintenance-report.mjs',
  'test/contracts/x27-ws03-evidence-maintenance.contract.test.js',
]);
...
function evaluateEvidenceChain(repoRoot) {
  const nextRecordPath = path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V12.json');
  const x25SignedPath = path.join(repoRoot, 'docs/OPS/STATUS/X26_CONTOUR_CLOSEOUT_SIGNED_V1.json');
  const x25FreezePath = path.join(repoRoot, 'docs/OPS/EVIDENCE/X26_CONTOUR/x26-contour-freeze-packet-v1.json');
  ...
  const nextRecordRefOk = normalizeString(nextRecord?.dependsOn?.x26CloseoutArtifactRef)
    === 'docs/OPS/STATUS/X26_CONTOUR_CLOSEOUT_SIGNED_V1.json';

  const signedRefOk = normalizeString(x25Signed?.evidencePacketRef)
    === 'docs/OPS/EVIDENCE/X26_CONTOUR/x26-contour-freeze-packet-v1.json';
...
function evaluateEvidenceRefCompleteness(repoRoot) {
  const requiredRefs = [
    'docs/OPS/STATUS/X27_WS01_NEXT_CONTOUR_ALIGNMENT_STATUS_V1.json',
    'docs/OPS/STATUS/X27_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json',
    'docs/OPS/EVIDENCE/X27_CONTOUR/TICKET_01/summary.json',
    'docs/OPS/EVIDENCE/X27_CONTOUR/TICKET_01/final-dod-acceptance-summary.json',
    'docs/OPS/EVIDENCE/X27_CONTOUR/TICKET_02/summary.json',
    'docs/OPS/EVIDENCE/X27_CONTOUR/TICKET_02/final-dod-acceptance-summary.json',
    'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V12.json',
    'docs/OPS/STATUS/X26_CONTOUR_CLOSEOUT_SIGNED_V1.json',
    'docs/OPS/EVIDENCE/X26_CONTOUR/x26-contour-freeze-packet-v1.json',
  ];
...
function evaluateLinkStaleness(repoRoot) {
  const ws02StatusPath = path.join(repoRoot, 'docs/OPS/STATUS/X27_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json');
...
export function evaluateX27Ws03EvidenceMaintenanceState(input = {}) {
  ...
  const nextRecord = readJsonObject(path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V12.json'));
  ...
  return {
    ...
    acceptance,
    determinism,
    finalGate: 'GO_TO_X27_WS04_ONLY_IF_ALL_DOD_AND_ACCEPTANCE_TRUE',
...
function main() {
  ...
  const state = evaluateX27Ws03EvidenceMaintenanceState({ repoRoot: process.cwd() });

  if (args.json) {
    process.stdout.write(`${stableStringify(state)}\n`);
  } else {
    process.stdout.write(`X27_WS03_EVIDENCE_MAINTENANCE_OK=${state.X27_WS03_EVIDENCE_MAINTENANCE_OK}\n`);
