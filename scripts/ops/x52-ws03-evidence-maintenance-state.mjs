...
const TOKEN_NAME = 'X52_WS03_EVIDENCE_MAINTENANCE_OK';
const EXPECTED_CANON_VERSION = 'v3.13a-final';

const PACKET_FILE_ALLOWLIST = new Set([
  'docs/OPS/EVIDENCE/X52_CONTOUR/',
  'docs/OPS/EVIDENCE/X52_CONTOUR/TICKET_03/',
  'docs/OPS/EVIDENCE/X52_CONTOUR/TICKET_03/negative-results.json',
  'docs/OPS/EVIDENCE/X52_CONTOUR/TICKET_03/positive-results.json',
  'docs/OPS/EVIDENCE/X52_CONTOUR/TICKET_03/final-dod-acceptance-summary.json',
  'docs/OPS/EVIDENCE/X52_CONTOUR/TICKET_03/summary.json',
  'docs/OPS/EVIDENCE/X52_CONTOUR/TICKET_03/ticket-meta.json',
  'docs/OPS/STATUS/X52_WS03_EVIDENCE_MAINTENANCE_STATUS_V1.json',
  'scripts/ops/x52-ws03-evidence-maintenance-state.mjs',
  'scripts/ops/x52-ws03-evidence-maintenance-report.mjs',
  'test/contracts/x52-ws03-evidence-maintenance.contract.test.js',
]);
...
function evaluateEvidenceChain(repoRoot) {
  const nextRecordPath = path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V37.json');
  const x51SignedPath = path.join(repoRoot, 'docs/OPS/STATUS/X51_CONTOUR_CLOSEOUT_SIGNED_V1.json');
  const x51FreezePath = path.join(repoRoot, 'docs/OPS/EVIDENCE/X51_CONTOUR/x51-contour-freeze-packet-v1.json');

  const nextRecord = readJsonObject(nextRecordPath);
  const x51Signed = readJsonObject(x51SignedPath);
  const x51Freeze = readJsonObject(x51FreezePath);

  const missing = [];
  if (!nextRecord) missing.push('NEXT_RECORD_MISSING');
  if (!x51Signed) missing.push('X51_SIGNED_MISSING');
  if (!x51Freeze) missing.push('X51_FREEZE_MISSING');

  const nextRecordRefOk = normalizeString(nextRecord?.dependsOn?.x51CloseoutArtifactRef)
    === 'docs/OPS/STATUS/X51_CONTOUR_CLOSEOUT_SIGNED_V1.json';

  const signedRefOk = normalizeString(x51Signed?.evidencePacketRef)
    === 'docs/OPS/EVIDENCE/X51_CONTOUR/x51-contour-freeze-packet-v1.json';

  let hashMatch = false;
  if (x51Signed && fs.existsSync(x51FreezePath)) {
    const actual = createHash('sha256').update(fs.readFileSync(x51FreezePath)).digest('hex');
    hashMatch = normalizeString(x51Signed.evidencePacketSha256) === `sha256:${actual}`;
  }
...
function evaluateEvidenceRefCompleteness(repoRoot) {
  const requiredRefs = [
    ...
    'docs/OPS/EVIDENCE/X52_CONTOUR/TICKET_01/summary.json',
    'docs/OPS/EVIDENCE/X52_CONTOUR/TICKET_01/final-dod-acceptance-summary.json',
...
function evaluateLinkStaleness(repoRoot) {
  const ws02StatusPath = path.join(repoRoot, 'docs/OPS/STATUS/X52_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json');
...
export function evaluateX52Ws03EvidenceMaintenanceState(input = {}) {
  ...
  return {
    ...
    acceptance,
    determinism,
    finalGate: 'GO_TO_X52_WS04_ONLY_IF_ALL_DOD_AND_ACCEPTANCE_TRUE',
...
function main() {
  ...
  const state = evaluateX52Ws03EvidenceMaintenanceState({ repoRoot: process.cwd() });

  if (args.json) {
    process.stdout.write(`${stableStringify(state)}\n`);
  } else {
    process.stdout.write(`X52_WS03_EVIDENCE_MAINTENANCE_OK=${state.X52_WS03_EVIDENCE_MAINTENANCE_OK}\n`);
