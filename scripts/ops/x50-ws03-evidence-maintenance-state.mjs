...
const TOKEN_NAME = 'X50_WS03_EVIDENCE_MAINTENANCE_OK';
const EXPECTED_CANON_VERSION = 'v3.13a-final';

const PACKET_FILE_ALLOWLIST = new Set([
  'docs/OPS/EVIDENCE/X50_CONTOUR/',
  'docs/OPS/EVIDENCE/X50_CONTOUR/TICKET_03/',
  'docs/OPS/EVIDENCE/X50_CONTOUR/TICKET_03/negative-results.json',
  'docs/OPS/EVIDENCE/X50_CONTOUR/TICKET_03/positive-results.json',
  'docs/OPS/EVIDENCE/X50_CONTOUR/TICKET_03/final-dod-acceptance-summary.json',
  'docs/OPS/EVIDENCE/X50_CONTOUR/TICKET_03/summary.json',
  'docs/OPS/EVIDENCE/X50_CONTOUR/TICKET_03/ticket-meta.json',
  'docs/OPS/STATUS/X50_WS03_EVIDENCE_MAINTENANCE_STATUS_V1.json',
  'scripts/ops/x50-ws03-evidence-maintenance-state.mjs',
  'scripts/ops/x50-ws03-evidence-maintenance-report.mjs',
  'test/contracts/x50-ws03-evidence-maintenance.contract.test.js',
]);
...
function evaluateEvidenceChain(repoRoot) {
  const nextRecordPath = path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V35.json');
  const x49SignedPath = path.join(repoRoot, 'docs/OPS/STATUS/X49_CONTOUR_CLOSEOUT_SIGNED_V1.json');
  const x49FreezePath = path.join(repoRoot, 'docs/OPS/EVIDENCE/X49_CONTOUR/x49-contour-freeze-packet-v1.json');

  const nextRecord = readJsonObject(nextRecordPath);
  const x49Signed = readJsonObject(x49SignedPath);
  const x49Freeze = readJsonObject(x49FreezePath);

  const missing = [];
  if (!nextRecord) missing.push('NEXT_RECORD_MISSING');
  if (!x49Signed) missing.push('X49_SIGNED_MISSING');
  if (!x49Freeze) missing.push('X49_FREEZE_MISSING');

  const nextRecordRefOk = normalizeString(nextRecord?.dependsOn?.x49CloseoutArtifactRef)
    === 'docs/OPS/STATUS/X49_CONTOUR_CLOSEOUT_SIGNED_V1.json';

  const signedRefOk = normalizeString(x49Signed?.evidencePacketRef)
    === 'docs/OPS/EVIDENCE/X49_CONTOUR/x49-contour-freeze-packet-v1.json';

  let hashMatch = false;
  if (x49Signed && fs.existsSync(x49FreezePath)) {
    const actual = createHash('sha256').update(fs.readFileSync(x49FreezePath)).digest('hex');
    hashMatch = normalizeString(x49Signed.evidencePacketSha256) === `sha256:${actual}`;
  }
...
function evaluateEvidenceRefCompleteness(repoRoot) {
  const requiredRefs = [
    'docs/OPS/STATUS/X50_WS01_NEXT_CONTOUR_ALIGNMENT_STATUS_V1.json',
    'docs/OPS/STATUS/X50_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json',
    'docs/OPS/EVIDENCE/X50_CONTOUR/TICKET_01/summary.json',
    'docs/OPS/EVIDENCE/X50_CONTOUR/TICKET_01/final-dod-acceptance-summary.json',
    'docs/OPS/EVIDENCE/X50_CONTOUR/TICKET_02/summary.json',
    'docs/OPS/EVIDENCE/X50_CONTOUR/TICKET_02/final-dod-acceptance-summary.json',
    'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V35.json',
    'docs/OPS/STATUS/X49_CONTOUR_CLOSEOUT_SIGNED_V1.json',
    'docs/OPS/EVIDENCE/X49_CONTOUR/x49-contour-freeze-packet-v1.json',
  ];
...
function evaluateLinkStaleness(repoRoot) {
  const ws02StatusPath = path.join(repoRoot, 'docs/OPS/STATUS/X50_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json');
...
export function evaluateX50Ws03EvidenceMaintenanceState(input = {}) {
  ...
  const nextRecord = readJsonObject(path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V35.json'));
  ...
  return {
    ...
    acceptance,
    determinism,
    finalGate: 'GO_TO_X50_WS04_ONLY_IF_ALL_DOD_AND_ACCEPTANCE_TRUE',
...
function main() {
  ...
  const state = evaluateX50Ws03EvidenceMaintenanceState({ repoRoot: process.cwd() });

  if (args.json) {
    process.stdout.write(`${stableStringify(state)}\n`);
  } else {
    process.stdout.write(`X50_WS03_EVIDENCE_MAINTENANCE_OK=${state.X50_WS03_EVIDENCE_MAINTENANCE_OK}\n`);
