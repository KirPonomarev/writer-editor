...
import { evaluateX17Ws03FormatPlanReviewActionsState } from './x17-ws03-format-plan-review-actions-state.mjs';

const DEFAULT_OUTPUT_DIR = 'docs/OPS/EVIDENCE/X17_CONTOUR/TICKET_03';
const DEFAULT_STATUS_PATH = 'docs/OPS/STATUS/X17_WS03_FORMAT_PLAN_REVIEW_ACTIONS_v1.json';
...
function main() {
  ...
  const run1 = evaluateX17Ws03FormatPlanReviewActionsState({ repoRoot });
  const run2 = evaluateX17Ws03FormatPlanReviewActionsState({ repoRoot });
  const run3 = evaluateX17Ws03FormatPlanReviewActionsState({ repoRoot });
  ...
  const summary = {
    ...
    objective: 'ENABLE_FORMAT_PLAN_REVIEW_ACTIONS_COMMAND_BUS_DETERMINISTIC_CAPABILITY_POLICY',
  ...
  const statusDoc = {
    version: 1,
    token: 'X17_WS03_FORMAT_PLAN_REVIEW_ACTIONS_OK',
  ...
  writeJson(path.join(outputDir, 'negative-results.json'), {
    target: {
      ...
      NEXT_TZ_NEGATIVE_04: 'REQUIRED_ACTION_HIDDEN_EXPECT_REJECT',
  ...
  writeJson(path.join(outputDir, 'positive-results.json'), {
    target: {
      NEXT_TZ_POSITIVE_01: 'FORMAT_PLAN_REVIEW_ACTIONS_EXECUTE_VIA_COMMAND_BUS',
  ...
  writeJson(path.join(outputDir, 'format-plan-review-summary.json'), {
  ...
  writeJson(path.join(outputDir, 'final-dod-acceptance-summary.json'), {
    ...
    finalGate: summary.status === 'PASS' ? 'GO_TO_X17_WS04' : 'HOLD',
  });
