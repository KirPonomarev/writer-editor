...
const TOKEN_NAME = 'X53_WS03_EVIDENCE_MAINTENANCE_OK';
const EXPECTED_CANON_VERSION = 'v3.13a-final';

const PACKET_FILE_ALLOWLIST = new Set([
  'docs/OPS/EVIDENCE/X53_CONTOUR/',
  'docs/OPS/EVIDENCE/X53_CONTOUR/TICKET_03/',
  'docs/OPS/EVIDENCE/X53_CONTOUR/TICKET_03/negative-results.json',
  'docs/OPS/EVIDENCE/X53_CONTOUR/TICKET_03/positive-results.json',
  'docs/OPS/EVIDENCE/X53_CONTOUR/TICKET_03/final-dod-acceptance-summary.json',
  'docs/OPS/EVIDENCE/X53_CONTOUR/TICKET_03/summary.json',
  'docs/OPS/EVIDENCE/X53_CONTOUR/TICKET_03/ticket-meta.json',
  'docs/OPS/STATUS/X53_WS03_EVIDENCE_MAINTENANCE_STATUS_V1.json',
  'scripts/ops/x53-ws03-evidence-maintenance-state.mjs',
  'scripts/ops/x53-ws03-evidence-maintenance-report.mjs',
  'test/contracts/x53-ws03-evidence-maintenance.contract.test.js',
]);
...
function evaluateEvidenceChain(repoRoot) {
  const nextRecordPath = path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V38.json');
  const x52SignedPath = path.join(repoRoot, 'docs/OPS/STATUS/X52_CONTOUR_CLOSEOUT_SIGNED_V1.json');
  const x52FreezePath = path.join(repoRoot, 'docs/OPS/EVIDENCE/X52_CONTOUR/x52-contour-freeze-packet-v1.json');

  const nextRecord = readJsonObject(nextRecordPath);
  const x52Signed = readJsonObject(x52SignedPath);
  const x52Freeze = readJsonObject(x52FreezePath);

  const missing = [];
  if (!nextRecord) missing.push('NEXT_RECORD_MISSING');
  if (!x52Signed) missing.push('X52_SIGNED_MISSING');
  if (!x52Freeze) missing.push('X52_FREEZE_MISSING');

  const nextRecordRefOk = normalizeString(nextRecord?.dependsOn?.x52CloseoutArtifactRef)
    === 'docs/OPS/STATUS/X52_CONTOUR_CLOSEOUT_SIGNED_V1.json';

  const signedRefOk = normalizeString(x52Signed?.evidencePacketRef)
    === 'docs/OPS/EVIDENCE/X52_CONTOUR/x52-contour-freeze-packet-v1.json';

  let hashMatch = false;
  if (x52Signed && fs.existsSync(x52FreezePath)) {
    const actual = createHash('sha256').update(fs.readFileSync(x52FreezePath)).digest('hex');
    hashMatch = normalizeString(x52Signed.evidencePacketSha256) === `sha256:${actual}`;
  }
...
function evaluateEvidenceRefCompleteness(repoRoot) {
  const requiredRefs = [
    'docs/OPS/STATUS/X53_WS01_NEXT_CONTOUR_ALIGNMENT_STATUS_V1.json',
    'docs/OPS/STATUS/X53_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json',
    'docs/OPS/EVIDENCE/X53_CONTOUR/TICKET_01/summary.json',
    'docs/OPS/EVIDENCE/X53_CONTOUR/TICKET_01/final-dod-acceptance-summary.json',
    'docs/OPS/EVIDENCE/X53_CONTOUR/TICKET_02/summary.json',
    'docs/OPS/EVIDENCE/X53_CONTOUR/TICKET_02/final-dod-acceptance-summary.json',
...
function evaluateLinkStaleness(repoRoot) {
  const ws02StatusPath = path.join(repoRoot, 'docs/OPS/STATUS/X53_WS02_QUEUE_PRIORITIZATION_STATUS_V1.json');
...
export function evaluateX53Ws03EvidenceMaintenanceState(input = {}) {
  ...
  const nextRecord = readJsonObject(path.join(repoRoot, 'docs/OPS/STATUS/NEXT_CONTOUR_OPENING_RECORD_V38.json'));
  ...
  return {
    ...
    acceptance,
    determinism,
    finalGate: 'GO_TO_X53_WS04_ONLY_IF_ALL_DOD_AND_ACCEPTANCE_TRUE',
...
function main() {
  ...
  const state = evaluateX53Ws03EvidenceMaintenanceState({ repoRoot: process.cwd() });

  if (args.json) {
    process.stdout.write(`${stableStringify(state)}\n`);
  } else {
    process.stdout.write(`X53_WS03_EVIDENCE_MAINTENANCE_OK=${state.X53_WS03_EVIDENCE_MAINTENANCE_OK}\n`);
