...
- Маршрут можно пометить `NOT_APPLICABLE` для backend/CLI/infra/docs-only задач и для механической визуальной правки без изменения дизайн-контракта. Изменение системных цвета, типографики, отступов, состояний или эталонного снимка всегда включает маршрут, даже если diff мал.
