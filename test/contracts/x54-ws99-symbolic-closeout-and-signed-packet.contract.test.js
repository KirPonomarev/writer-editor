...
const MODULE_PATH = path.join(
  ...
  'scripts',
  'ops',
  'x54-ws99-symbolic-closeout-and-signed-packet-state.mjs',
);
...
test('x54 ws99 baseline symbolic closeout passes', async () => {
  const { evaluateX54Ws99SymbolicCloseoutAndSignedPacketState } = await loadModule();
  const state = evaluateX54Ws99SymbolicCloseoutAndSignedPacketState({ repoRoot: REPO_ROOT });

  assert.equal(state.ok, true, JSON.stringify(state, null, 2));
  assert.equal(state.X54_WS99_SYMBOLIC_CLOSEOUT_AND_SIGNED_PACKET_OK, 1);
...
test('x54 ws99 repeatability is stable across three runs', async () => {
  const { evaluateX54Ws99SymbolicCloseoutAndSignedPacketState } = await loadModule();

  const runs = [
    evaluateX54Ws99SymbolicCloseoutAndSignedPacketState({ repoRoot: REPO_ROOT }),
    evaluateX54Ws99SymbolicCloseoutAndSignedPacketState({ repoRoot: REPO_ROOT }),
    evaluateX54Ws99SymbolicCloseoutAndSignedPacketState({ repoRoot: REPO_ROOT }),
  ].map((state) => ({
    ok: state.ok,
