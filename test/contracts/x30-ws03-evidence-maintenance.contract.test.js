...
const MODULE_PATH = path.join(
  ...
  'scripts',
  'ops',
  'x30-ws03-evidence-maintenance-state.mjs',
);
...
test('x30 ws03 baseline evidence maintenance passes', async () => {
  const { evaluateX30Ws03EvidenceMaintenanceState } = await loadModule();
  const state = evaluateX30Ws03EvidenceMaintenanceState({ repoRoot: REPO_ROOT });

  assert.equal(state.ok, true, JSON.stringify(state, null, 2));
  assert.equal(state.X30_WS03_EVIDENCE_MAINTENANCE_OK, 1);
...
test('x30 ws03 repeatability is stable across three runs', async () => {
  const { evaluateX30Ws03EvidenceMaintenanceState } = await loadModule();

  const runs = [
    evaluateX30Ws03EvidenceMaintenanceState({ repoRoot: REPO_ROOT }),
    evaluateX30Ws03EvidenceMaintenanceState({ repoRoot: REPO_ROOT }),
    evaluateX30Ws03EvidenceMaintenanceState({ repoRoot: REPO_ROOT }),
  ].map((state) => ({
    ok: state.ok,
