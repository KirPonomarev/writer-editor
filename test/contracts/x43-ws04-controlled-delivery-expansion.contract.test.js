...
const MODULE_PATH = path.join(
  ...
  'scripts',
  'ops',
  'x43-ws04-controlled-delivery-expansion-state.mjs',
);
...
test('x43 ws04 baseline controlled delivery expansion passes', async () => {
  const { evaluateX43Ws04ControlledDeliveryExpansionState } = await loadModule();
  const state = evaluateX43Ws04ControlledDeliveryExpansionState({ repoRoot: REPO_ROOT });

  assert.equal(state.ok, true, JSON.stringify(state, null, 2));
  assert.equal(state.X43_WS04_CONTROLLED_DELIVERY_EXPANSION_OK, 1);
...
test('x43 ws04 repeatability is stable across three runs', async () => {
  const { evaluateX43Ws04ControlledDeliveryExpansionState } = await loadModule();

  const runs = [
    evaluateX43Ws04ControlledDeliveryExpansionState({ repoRoot: REPO_ROOT }),
    evaluateX43Ws04ControlledDeliveryExpansionState({ repoRoot: REPO_ROOT }),
    evaluateX43Ws04ControlledDeliveryExpansionState({ repoRoot: REPO_ROOT }),
  ].map((state) => ({
    ok: state.ok,
