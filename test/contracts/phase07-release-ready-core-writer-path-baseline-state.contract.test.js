...
const SCRIPT_PATH = 'scripts/ops/phase07-release-ready-core-writer-path-baseline-state.mjs';
...
test('phase07 release ready core writer path baseline: positive run closes release readiness', () => {
  ...
  assert.equal(payload.greenCheckIds.includes('PREVIOUS_PHASE07_RELEASE_READY_CORE_WRITER_PATH_FOUNDATION_PASS'), true);
  assert.equal(payload.greenCheckIds.includes('X78_RELEASE_REQUIRED_SET_PARITY_PASS'), true);
  assert.equal(payload.greenCheckIds.includes('X78_PARITY_BOOLEANS_TRUE'), true);
  assert.equal(payload.greenCheckIds.includes('X79_RELEASE_FINAL_SHIP_READINESS_OK'), true);
  assert.equal(payload.greenCheckIds.includes('X79_RELEASE_FINAL_SHIP_READINESS_CHECKS_TRUE'), true);
  ...
  assert.equal(payload.checkStatusById.X78_RELEASE_REQUIRED_SET_PARITY_PASS.status, 'GREEN');
  assert.equal(payload.checkStatusById.X78_RELEASE_REQUIRED_SET_PARITY_PASS.measured, true);
  assert.equal(payload.checkStatusById.X79_RELEASE_FINAL_SHIP_READINESS_OK.status, 'GREEN');
  assert.equal(payload.checkStatusById.X79_RELEASE_FINAL_SHIP_READINESS_OK.measured, true);
});

test('phase07 release ready core writer path baseline: forced negative path is deterministic', () => {
  ...
  assert.equal(payload.failReason, 'E_PHASE07_RELEASE_READY_CORE_WRITER_PATH_BASELINE_FORCED_NEGATIVE');
  assert.equal(payload.overallStatus, 'HOLD');
  assert.equal(payload.phase07ReleaseReadyCoreWriterPathBaselineStatus, 'HOLD');
