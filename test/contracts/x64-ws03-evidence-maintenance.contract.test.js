...
const MODULE_PATH = path.join(
  ...
  'scripts',
  'ops',
  'x64-ws03-evidence-maintenance-state.mjs',
);
...
test('x64 ws03 baseline evidence maintenance passes', async () => {
  const { evaluateX64Ws03EvidenceMaintenanceState } = await loadModule();
  const state = evaluateX64Ws03EvidenceMaintenanceState({ repoRoot: REPO_ROOT });

  assert.equal(state.ok, true, JSON.stringify(state, null, 2));
  assert.equal(state.X64_WS03_EVIDENCE_MAINTENANCE_OK, 1);
...
test('x64 ws03 repeatability is stable across three runs', async () => {
  const { evaluateX64Ws03EvidenceMaintenanceState } = await loadModule();

  const runs = [
    evaluateX64Ws03EvidenceMaintenanceState({ repoRoot: REPO_ROOT }),
    evaluateX64Ws03EvidenceMaintenanceState({ repoRoot: REPO_ROOT }),
    evaluateX64Ws03EvidenceMaintenanceState({ repoRoot: REPO_ROOT }),
  ].map((state) => ({
    ok: state.ok,
