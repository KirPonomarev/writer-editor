...
const SCRIPT_PATH = 'scripts/ops/phase07-reset-runtime-measurement-baseline-state.mjs';
...
test('phase07 reset runtime measurement baseline: positive run passes with all blocking budgets bound', () => {
  ...
  assert.equal(payload.phase07ResetRuntimeMeasurementBaselineStatus, 'PASS');
  ...
  assert.equal(payload.greenCheckIds.includes('PHASE07_SCENE_SWITCH_RUNTIME_MEASUREMENT_BASELINE_PASS'), true);
  assert.equal(payload.greenCheckIds.includes('PERF_RUN_RESET_MEASUREMENT_PRESENT'), true);
...
test('phase07 reset runtime measurement baseline: forced negative path is deterministic', () => {
  ...
  assert.equal(payload.failReason, 'E_PHASE07_RESET_RUNTIME_MEASUREMENT_BASELINE_FORCED_NEGATIVE');
  assert.equal(payload.overallStatus, 'HOLD');
  assert.equal(payload.phase07ResetRuntimeMeasurementBaselineStatus, 'HOLD');
  assert.equal(payload.phase07ReadinessStatus, 'HOLD');
