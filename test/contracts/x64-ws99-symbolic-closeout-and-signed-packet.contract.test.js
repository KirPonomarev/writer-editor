...
const MODULE_PATH = path.join(
  ...
  'scripts',
  'ops',
  'x64-ws99-symbolic-closeout-and-signed-packet-state.mjs',
);
...
test('x64 ws99 baseline symbolic closeout passes', async () => {
  const { evaluateX64Ws99SymbolicCloseoutAndSignedPacketState } = await loadModule();
  const state = evaluateX64Ws99SymbolicCloseoutAndSignedPacketState({ repoRoot: REPO_ROOT });

  assert.equal(state.ok, true, JSON.stringify(state, null, 2));
  assert.equal(state.X64_WS99_SYMBOLIC_CLOSEOUT_AND_SIGNED_PACKET_OK, 1);
...
test('x64 ws99 repeatability is stable across three runs', async () => {
  const { evaluateX64Ws99SymbolicCloseoutAndSignedPacketState } = await loadModule();

  const runs = [
    evaluateX64Ws99SymbolicCloseoutAndSignedPacketState({ repoRoot: REPO_ROOT }),
    evaluateX64Ws99SymbolicCloseoutAndSignedPacketState({ repoRoot: REPO_ROOT }),
    evaluateX64Ws99SymbolicCloseoutAndSignedPacketState({ repoRoot: REPO_ROOT }),
  ].map((state) => ({
    ok: state.ok,
