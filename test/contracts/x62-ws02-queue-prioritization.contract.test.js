...
const MODULE_PATH = path.join(
  ...
  'scripts',
  'ops',
  'x62-ws02-queue-prioritization-state.mjs',
);
...
test('x62 ws02 baseline queue prioritization passes', async () => {
  const { evaluateX62Ws02QueuePrioritizationState } = await loadModule();
  const state = evaluateX62Ws02QueuePrioritizationState({ repoRoot: REPO_ROOT });

  assert.equal(state.ok, true, JSON.stringify(state, null, 2));
  assert.equal(state.X62_WS02_QUEUE_PRIORITIZATION_OK, 1);
...
test('x62 ws02 repeatability is stable across three runs', async () => {
  const { evaluateX62Ws02QueuePrioritizationState } = await loadModule();

  const runs = [
    evaluateX62Ws02QueuePrioritizationState({ repoRoot: REPO_ROOT }),
    evaluateX62Ws02QueuePrioritizationState({ repoRoot: REPO_ROOT }),
    evaluateX62Ws02QueuePrioritizationState({ repoRoot: REPO_ROOT }),
  ].map((state) => ({
    ok: state.ok,
