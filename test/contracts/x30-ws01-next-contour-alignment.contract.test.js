...
const MODULE_PATH = path.join(
  ...
  'scripts',
  'ops',
  'x30-ws01-next-contour-alignment-state.mjs',
);
...
test('x30 ws01 baseline alignment passes', async () => {
  const { evaluateX30Ws01NextContourAlignmentState } = await loadModule();
  const state = evaluateX30Ws01NextContourAlignmentState({ repoRoot: REPO_ROOT });

  assert.equal(state.ok, true, JSON.stringify(state, null, 2));
  assert.equal(state.X30_WS01_NEXT_CONTOUR_ALIGNMENT_OK, 1);
...
test('x30 ws01 repeatability is stable across three runs', async () => {
  const { evaluateX30Ws01NextContourAlignmentState } = await loadModule();

  const runs = [
    evaluateX30Ws01NextContourAlignmentState({ repoRoot: REPO_ROOT }),
    evaluateX30Ws01NextContourAlignmentState({ repoRoot: REPO_ROOT }),
    evaluateX30Ws01NextContourAlignmentState({ repoRoot: REPO_ROOT }),
  ].map((state) => ({
    ok: state.ok,
