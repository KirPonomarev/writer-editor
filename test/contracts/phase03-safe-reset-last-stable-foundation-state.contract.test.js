...
const SCRIPT_PATH = 'scripts/ops/phase03-safe-reset-last-stable-foundation-state.mjs';
...
test('phase03 safe reset last stable foundation: positive run passes while foundation status remains hold', () => {
  ...
  assert.equal(payload.greenCheckIds.includes('SHELL_LEVEL_SAFE_RESET_ABSENT'), true);
});

test('phase03 safe reset last stable foundation: forced negative path is deterministic', () => {
  ...
  assert.equal(payload.failReason, 'E_PHASE03_SAFE_RESET_LAST_STABLE_FOUNDATION_FORCED_NEGATIVE');
