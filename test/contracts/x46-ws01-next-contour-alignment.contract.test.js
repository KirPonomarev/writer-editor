...
const MODULE_PATH = path.join(
  ...
  'scripts',
  'ops',
  'x46-ws01-next-contour-alignment-state.mjs',
);
...
test('x46 ws01 baseline alignment passes', async () => {
  const { evaluateX46Ws01NextContourAlignmentState } = await loadModule();
  const state = evaluateX46Ws01NextContourAlignmentState({ repoRoot: REPO_ROOT });

  assert.equal(state.ok, true, JSON.stringify(state, null, 2));
  assert.equal(state.X46_WS01_NEXT_CONTOUR_ALIGNMENT_OK, 1);
...
test('x46 ws01 repeatability is stable across three runs', async () => {
  const { evaluateX46Ws01NextContourAlignmentState } = await loadModule();

  const runs = [
    evaluateX46Ws01NextContourAlignmentState({ repoRoot: REPO_ROOT }),
    evaluateX46Ws01NextContourAlignmentState({ repoRoot: REPO_ROOT }),
    evaluateX46Ws01NextContourAlignmentState({ repoRoot: REPO_ROOT }),
  ].map((state) => ({
    ok: state.ok,
