...
const MODULE_PATH = path.join(
  ...
  'scripts',
  'ops',
  'x52-ws03-evidence-maintenance-state.mjs',
);
...
test('x52 ws03 baseline evidence maintenance passes', async () => {
  const { evaluateX52Ws03EvidenceMaintenanceState } = await loadModule();
  const state = evaluateX52Ws03EvidenceMaintenanceState({ repoRoot: REPO_ROOT });

  assert.equal(state.ok, true, JSON.stringify(state, null, 2));
  assert.equal(state.X52_WS03_EVIDENCE_MAINTENANCE_OK, 1);
...
test('x52 ws03 repeatability is stable across three runs', async () => {
  const { evaluateX52Ws03EvidenceMaintenanceState } = await loadModule();

  const runs = [
    evaluateX52Ws03EvidenceMaintenanceState({ repoRoot: REPO_ROOT }),
    evaluateX52Ws03EvidenceMaintenanceState({ repoRoot: REPO_ROOT }),
    evaluateX52Ws03EvidenceMaintenanceState({ repoRoot: REPO_ROOT }),
  ].map((state) => ({
    ok: state.ok,
