...
const MODULE_PATH = path.join(
  ...
  'scripts',
  'ops',
  'x37-ws02-queue-prioritization-state.mjs',
);
...
test('x37 ws02 baseline queue prioritization passes', async () => {
  const { evaluateX37Ws02QueuePrioritizationState } = await loadModule();
  const state = evaluateX37Ws02QueuePrioritizationState({ repoRoot: REPO_ROOT });

  assert.equal(state.ok, true, JSON.stringify(state, null, 2));
  assert.equal(state.X37_WS02_QUEUE_PRIORITIZATION_OK, 1);
...
test('x37 ws02 repeatability is stable across three runs', async () => {
  const { evaluateX37Ws02QueuePrioritizationState } = await loadModule();

  const runs = [
    evaluateX37Ws02QueuePrioritizationState({ repoRoot: REPO_ROOT }),
    evaluateX37Ws02QueuePrioritizationState({ repoRoot: REPO_ROOT }),
    evaluateX37Ws02QueuePrioritizationState({ repoRoot: REPO_ROOT }),
  ].map((state) => ({
    ok: state.ok,
