...
const MODULE_PATH = path.join(
  ...
  'scripts',
  'ops',
  'x48-ws04-controlled-delivery-expansion-state.mjs',
);
...
test('x48 ws04 baseline controlled delivery expansion passes', async () => {
  const { evaluateX48Ws04ControlledDeliveryExpansionState } = await loadModule();
  const state = evaluateX48Ws04ControlledDeliveryExpansionState({ repoRoot: REPO_ROOT });

  assert.equal(state.ok, true, JSON.stringify(state, null, 2));
  assert.equal(state.X48_WS04_CONTROLLED_DELIVERY_EXPANSION_OK, 1);
...
test('x48 ws04 repeatability is stable across three runs', async () => {
  const { evaluateX48Ws04ControlledDeliveryExpansionState } = await loadModule();

  const runs = [
    evaluateX48Ws04ControlledDeliveryExpansionState({ repoRoot: REPO_ROOT }),
    evaluateX48Ws04ControlledDeliveryExpansionState({ repoRoot: REPO_ROOT }),
    evaluateX48Ws04ControlledDeliveryExpansionState({ repoRoot: REPO_ROOT }),
  ].map((state) => ({
    ok: state.ok,
