...
const MODULE_PATH = path.join(
  ...
  'scripts',
  'ops',
  'x27-ws04-controlled-delivery-expansion-state.mjs',
);
...
test('x27 ws04 baseline controlled delivery expansion passes', async () => {
  const { evaluateX27Ws04ControlledDeliveryExpansionState } = await loadModule();
  const state = evaluateX27Ws04ControlledDeliveryExpansionState({ repoRoot: REPO_ROOT });

  assert.equal(state.ok, true, JSON.stringify(state, null, 2));
  assert.equal(state.X27_WS04_CONTROLLED_DELIVERY_EXPANSION_OK, 1);
...
test('x27 ws04 repeatability is stable across three runs', async () => {
  const { evaluateX27Ws04ControlledDeliveryExpansionState } = await loadModule();

  const runs = [
    evaluateX27Ws04ControlledDeliveryExpansionState({ repoRoot: REPO_ROOT }),
    evaluateX27Ws04ControlledDeliveryExpansionState({ repoRoot: REPO_ROOT }),
    evaluateX27Ws04ControlledDeliveryExpansionState({ repoRoot: REPO_ROOT }),
  ].map((state) => ({
    ok: state.ok,
