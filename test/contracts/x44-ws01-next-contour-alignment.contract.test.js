...
const MODULE_PATH = path.join(
  ...
  'scripts',
  'ops',
  'x44-ws01-next-contour-alignment-state.mjs',
);
...
test('x44 ws01 baseline alignment passes', async () => {
  const { evaluateX44Ws01NextContourAlignmentState } = await loadModule();
  const state = evaluateX44Ws01NextContourAlignmentState({ repoRoot: REPO_ROOT });

  assert.equal(state.ok, true, JSON.stringify(state, null, 2));
  assert.equal(state.X44_WS01_NEXT_CONTOUR_ALIGNMENT_OK, 1);
...
test('x44 ws01 repeatability is stable across three runs', async () => {
  const { evaluateX44Ws01NextContourAlignmentState } = await loadModule();

  const runs = [
    evaluateX44Ws01NextContourAlignmentState({ repoRoot: REPO_ROOT }),
    evaluateX44Ws01NextContourAlignmentState({ repoRoot: REPO_ROOT }),
    evaluateX44Ws01NextContourAlignmentState({ repoRoot: REPO_ROOT }),
  ].map((state) => ({
    ok: state.ok,
