...
const MODULE_PATH = path.join(
  ...
  'scripts',
  'ops',
  'x47-ws01-next-contour-alignment-state.mjs',
);
...
test('x47 ws01 baseline alignment passes', async () => {
  const { evaluateX47Ws01NextContourAlignmentState } = await loadModule();
  const state = evaluateX47Ws01NextContourAlignmentState({ repoRoot: REPO_ROOT });

  assert.equal(state.ok, true, JSON.stringify(state, null, 2));
  assert.equal(state.X47_WS01_NEXT_CONTOUR_ALIGNMENT_OK, 1);
...
test('x47 ws01 repeatability is stable across three runs', async () => {
  const { evaluateX47Ws01NextContourAlignmentState } = await loadModule();

  const runs = [
    evaluateX47Ws01NextContourAlignmentState({ repoRoot: REPO_ROOT }),
    evaluateX47Ws01NextContourAlignmentState({ repoRoot: REPO_ROOT }),
    evaluateX47Ws01NextContourAlignmentState({ repoRoot: REPO_ROOT }),
  ].map((state) => ({
    ok: state.ok,
