...
const MODULE_PATH = path.join(
  ...
  'scripts',
  'ops',
  'x42-ws03-evidence-maintenance-state.mjs',
);
...
test('x42 ws03 baseline evidence maintenance passes', async () => {
  const { evaluateX42Ws03EvidenceMaintenanceState } = await loadModule();
  const state = evaluateX42Ws03EvidenceMaintenanceState({ repoRoot: REPO_ROOT });

  assert.equal(state.ok, true, JSON.stringify(state, null, 2));
  assert.equal(state.X42_WS03_EVIDENCE_MAINTENANCE_OK, 1);
...
test('x42 ws03 repeatability is stable across three runs', async () => {
  const { evaluateX42Ws03EvidenceMaintenanceState } = await loadModule();

  const runs = [
    evaluateX42Ws03EvidenceMaintenanceState({ repoRoot: REPO_ROOT }),
    evaluateX42Ws03EvidenceMaintenanceState({ repoRoot: REPO_ROOT }),
    evaluateX42Ws03EvidenceMaintenanceState({ repoRoot: REPO_ROOT }),
  ].map((state) => ({
    ok: state.ok,
