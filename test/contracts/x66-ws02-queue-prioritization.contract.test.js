...
const MODULE_PATH = path.join(
  ...
  'scripts',
  'ops',
  'x66-ws02-queue-prioritization-state.mjs',
);
...
test('x66 ws02 baseline queue prioritization passes', async () => {
  const { evaluateX66Ws02QueuePrioritizationState } = await loadModule();
  const state = evaluateX66Ws02QueuePrioritizationState({ repoRoot: REPO_ROOT });

  assert.equal(state.ok, true, JSON.stringify(state, null, 2));
  assert.equal(state.X66_WS02_QUEUE_PRIORITIZATION_OK, 1);
...
test('x66 ws02 repeatability is stable across three runs', async () => {
  const { evaluateX66Ws02QueuePrioritizationState } = await loadModule();

  const runs = [
    evaluateX66Ws02QueuePrioritizationState({ repoRoot: REPO_ROOT }),
    evaluateX66Ws02QueuePrioritizationState({ repoRoot: REPO_ROOT }),
    evaluateX66Ws02QueuePrioritizationState({ repoRoot: REPO_ROOT }),
  ].map((state) => ({
    ok: state.ok,
