...
const MODULE_PATH = path.join(
  ...
  'scripts',
  'ops',
  'x15-ws03-profile-presets-state.mjs',
);
...
function evaluateBaseline(evaluateX15Ws03ProfilePresetsState) {
  return evaluateX15Ws03ProfilePresetsState({ repoRoot: REPO_ROOT });
}

test('x15 ws03 baseline: profile presets passes', async () => {
  const { evaluateX15Ws03ProfilePresetsState } = await loadModule();
  const state = evaluateBaseline(evaluateX15Ws03ProfilePresetsState);

  assert.equal(state.ok, true, JSON.stringify(state, null, 2));
  assert.equal(state.X15_WS03_PROFILE_PRESETS_OK, 1);
  assert.equal(state.counts.presetCount, 3);
  assert.equal(state.counts.profileEnumCount, 3);
...
test('x15 ws03 negative 01: undeclared command preset is rejected', async () => {
  const { evaluateX15Ws03ProfilePresetsState } = await loadModule();
  const state = evaluateBaseline(evaluateX15Ws03ProfilePresetsState);
  assert.equal(state.negativeResults.NEXT_TZ_NEGATIVE_01, true);
});

test('x15 ws03 negative 02: preset hiding required core command is rejected', async () => {
  const { evaluateX15Ws03ProfilePresetsState } = await loadModule();
  const state = evaluateBaseline(evaluateX15Ws03ProfilePresetsState);
  assert.equal(state.negativeResults.NEXT_TZ_NEGATIVE_02, true);
});

test('x15 ws03 negative 03: panel state conflict is rejected', async () => {
  const { evaluateX15Ws03ProfilePresetsState } = await loadModule();
  const state = evaluateBaseline(evaluateX15Ws03ProfilePresetsState);
  assert.equal(state.negativeResults.NEXT_TZ_NEGATIVE_03, true);
});

test('x15 ws03 negative 04: invalid mode context switch is rejected', async () => {
  const { evaluateX15Ws03ProfilePresetsState } = await loadModule();
  const state = evaluateBaseline(evaluateX15Ws03ProfilePresetsState);
  assert.equal(state.negativeResults.NEXT_TZ_NEGATIVE_04, true);
});

test('x15 ws03 negative 05: preset override without capability binding is rejected', async () => {
  const { evaluateX15Ws03ProfilePresetsState } = await loadModule();
  const state = evaluateBaseline(evaluateX15Ws03ProfilePresetsState);
  assert.equal(state.negativeResults.NEXT_TZ_NEGATIVE_05, true);
});

test('x15 ws03 repeatability: three runs are stable', async () => {
  const { evaluateX15Ws03ProfilePresetsState } = await loadModule();

  const runs = [
    evaluateBaseline(evaluateX15Ws03ProfilePresetsState),
    evaluateBaseline(evaluateX15Ws03ProfilePresetsState),
    evaluateBaseline(evaluateX15Ws03ProfilePresetsState),
  ].map((state) => ({
    ok: state.ok,
    token: state.X15_WS03_PROFILE_PRESETS_OK,
    counts: state.counts,
