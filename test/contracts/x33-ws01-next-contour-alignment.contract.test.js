...
const MODULE_PATH = path.join(
  ...
  'scripts',
  'ops',
  'x33-ws01-next-contour-alignment-state.mjs',
);
...
test('x33 ws01 baseline alignment passes', async () => {
  const { evaluateX33Ws01NextContourAlignmentState } = await loadModule();
  const state = evaluateX33Ws01NextContourAlignmentState({ repoRoot: REPO_ROOT });

  assert.equal(state.ok, true, JSON.stringify(state, null, 2));
  assert.equal(state.X33_WS01_NEXT_CONTOUR_ALIGNMENT_OK, 1);
...
test('x33 ws01 repeatability is stable across three runs', async () => {
  const { evaluateX33Ws01NextContourAlignmentState } = await loadModule();

  const runs = [
    evaluateX33Ws01NextContourAlignmentState({ repoRoot: REPO_ROOT }),
    evaluateX33Ws01NextContourAlignmentState({ repoRoot: REPO_ROOT }),
    evaluateX33Ws01NextContourAlignmentState({ repoRoot: REPO_ROOT }),
  ].map((state) => ({
    ok: state.ok,
