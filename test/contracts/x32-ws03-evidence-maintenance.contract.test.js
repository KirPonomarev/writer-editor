...
const MODULE_PATH = path.join(
  ...
  'scripts',
  'ops',
  'x32-ws03-evidence-maintenance-state.mjs',
);
...
test('x32 ws03 baseline evidence maintenance passes', async () => {
  const { evaluateX32Ws03EvidenceMaintenanceState } = await loadModule();
  const state = evaluateX32Ws03EvidenceMaintenanceState({ repoRoot: REPO_ROOT });

  assert.equal(state.ok, true, JSON.stringify(state, null, 2));
  assert.equal(state.X32_WS03_EVIDENCE_MAINTENANCE_OK, 1);
...
test('x32 ws03 repeatability is stable across three runs', async () => {
  const { evaluateX32Ws03EvidenceMaintenanceState } = await loadModule();

  const runs = [
    evaluateX32Ws03EvidenceMaintenanceState({ repoRoot: REPO_ROOT }),
    evaluateX32Ws03EvidenceMaintenanceState({ repoRoot: REPO_ROOT }),
    evaluateX32Ws03EvidenceMaintenanceState({ repoRoot: REPO_ROOT }),
  ].map((state) => ({
    ok: state.ok,
