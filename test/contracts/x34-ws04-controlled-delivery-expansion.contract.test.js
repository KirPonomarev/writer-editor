...
const MODULE_PATH = path.join(
  ...
  'scripts',
  'ops',
  'x34-ws04-controlled-delivery-expansion-state.mjs',
);
...
test('x34 ws04 baseline controlled delivery expansion passes', async () => {
  const { evaluateX34Ws04ControlledDeliveryExpansionState } = await loadModule();
  const state = evaluateX34Ws04ControlledDeliveryExpansionState({ repoRoot: REPO_ROOT });

  assert.equal(state.ok, true, JSON.stringify(state, null, 2));
  assert.equal(state.X34_WS04_CONTROLLED_DELIVERY_EXPANSION_OK, 1);
...
test('x34 ws04 repeatability is stable across three runs', async () => {
  const { evaluateX34Ws04ControlledDeliveryExpansionState } = await loadModule();

  const runs = [
    evaluateX34Ws04ControlledDeliveryExpansionState({ repoRoot: REPO_ROOT }),
    evaluateX34Ws04ControlledDeliveryExpansionState({ repoRoot: REPO_ROOT }),
    evaluateX34Ws04ControlledDeliveryExpansionState({ repoRoot: REPO_ROOT }),
  ].map((state) => ({
    ok: state.ok,
