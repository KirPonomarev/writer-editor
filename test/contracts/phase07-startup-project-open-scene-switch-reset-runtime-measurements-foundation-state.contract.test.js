...
test('phase07 runtime measurements foundation: positive run passes with honest readiness hold and honest open gaps', () => {
  ...
  assert.equal(payload.checkStatusById.STARTUP_MEASUREMENT_NOT_BOUND.status, 'OPEN_GAP');
  assert.equal(payload.checkStatusById.STARTUP_MEASUREMENT_NOT_BOUND.measured, false);
  assert.equal(payload.checkStatusById.SCENE_SWITCH_MEASUREMENT_NOT_BOUND.status, 'OPEN_GAP');
  assert.equal(payload.checkStatusById.SCENE_SWITCH_MEASUREMENT_NOT_BOUND.measured, false);
