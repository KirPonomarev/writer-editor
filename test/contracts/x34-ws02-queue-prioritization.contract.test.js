...
const MODULE_PATH = path.join(
  ...
  'scripts',
  'ops',
  'x34-ws02-queue-prioritization-state.mjs',
);
...
test('x34 ws02 baseline queue prioritization passes', async () => {
  const { evaluateX34Ws02QueuePrioritizationState } = await loadModule();
  const state = evaluateX34Ws02QueuePrioritizationState({ repoRoot: REPO_ROOT });

  assert.equal(state.ok, true, JSON.stringify(state, null, 2));
  assert.equal(state.X34_WS02_QUEUE_PRIORITIZATION_OK, 1);
...
test('x34 ws02 repeatability is stable across three runs', async () => {
  const { evaluateX34Ws02QueuePrioritizationState } = await loadModule();

  const runs = [
    evaluateX34Ws02QueuePrioritizationState({ repoRoot: REPO_ROOT }),
    evaluateX34Ws02QueuePrioritizationState({ repoRoot: REPO_ROOT }),
    evaluateX34Ws02QueuePrioritizationState({ repoRoot: REPO_ROOT }),
  ].map((state) => ({
    ok: state.ok,
