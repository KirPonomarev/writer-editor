...
for(const era of ['C1','REVIEW','HOSTILE','MEDIA','REOPEN','TABLE_FILES','IMPORT_TRANSACTION','IMPORT_LOSS','MEDIA_DISPLAY','MEDIA_TEXT','TABLE_PROPERTIES','COMPOSITE_FIDELITY'])test(era+' Word reader successor admits one complete set and rejects mixed or altered bindings', async () => {
  ...
  if (!['REOPEN','TABLE_FILES','IMPORT_TRANSACTION','IMPORT_LOSS','MEDIA_DISPLAY','MEDIA_TEXT','TABLE_PROPERTIES','COMPOSITE_FIDELITY'].includes(era)) {
