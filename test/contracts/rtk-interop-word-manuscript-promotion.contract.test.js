...
for(const era of ['C1','REVIEW','HOSTILE','MEDIA','REOPEN'])test(era+' Word reader successor admits one complete set and rejects mixed or altered bindings', async () => {
  const cert = await import(pathToFileURL(path.join(ROOT, 'scripts/ops/r24/corrective/post-audit-certification-set.mjs')));
  const expected = era==='C1'?cert.R24_INTEROP_WORD_TABLES_C1_SUCCESSOR:era==='REVIEW'?cert.R24_INTEROP_WORD_TABLES_REVIEW_SUCCESSOR:era==='HOSTILE'?cert.R24_INTEROP_WORD_HOSTILE_SUCCESSOR:era==='MEDIA'?cert.R24_INTEROP_WORD_MEDIA_SUCCESSOR:cert.R24_INTEROP_WORD_NATIVE_REOPEN_SUCCESSOR, candidate = 'f'.repeat(40);
  const historicalHead = era==='C1'?'3a5f0b2080e861d779cf5838a2a34c6e7d5e4244':era==='REVIEW'?'3dbe5404aad3b583fdbf929f2b49cf016f819fc6':era==='HOSTILE'?'1c9dae7a3dcdc3fc79e22ea9b53b0c2ca67bcd5d':era==='MEDIA'?'ccc9b4b02db839241405f2667313c8ec5450455e':null;
  ...
  const git = args => {
    if (args[0] === 'rev-parse') {
      for (const set of [cert.R24_INTEROP_WORD_PROMOTION_SUCCESSOR,cert.R24_INTEROP_WORD_TABLES_C1_SUCCESSOR,cert.R24_INTEROP_WORD_TABLES_REVIEW_SUCCESSOR,cert.R24_INTEROP_WORD_HOSTILE_SUCCESSOR,cert.R24_INTEROP_WORD_MEDIA_SUCCESSOR,cert.R24_INTEROP_WORD_NATIVE_REOPEN_SUCCESSOR]) {
  ...
  // REOPEN changes Lab admission only; its runtime reader is intentionally unchanged.
  if (era !== 'REOPEN') {
