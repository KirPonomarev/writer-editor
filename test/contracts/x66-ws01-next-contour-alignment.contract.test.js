...
const MODULE_PATH = path.join(
  ...
  'scripts',
  'ops',
  'x66-ws01-next-contour-alignment-state.mjs',
);
...
test('x66 ws01 baseline alignment passes', async () => {
  const { evaluateX66Ws01NextContourAlignmentState } = await loadModule();
  const state = evaluateX66Ws01NextContourAlignmentState({ repoRoot: REPO_ROOT });

  assert.equal(state.ok, true, JSON.stringify(state, null, 2));
  assert.equal(state.X66_WS01_NEXT_CONTOUR_ALIGNMENT_OK, 1);
...
test('x66 ws01 repeatability is stable across three runs', async () => {
  const { evaluateX66Ws01NextContourAlignmentState } = await loadModule();

  const runs = [
    evaluateX66Ws01NextContourAlignmentState({ repoRoot: REPO_ROOT }),
    evaluateX66Ws01NextContourAlignmentState({ repoRoot: REPO_ROOT }),
    evaluateX66Ws01NextContourAlignmentState({ repoRoot: REPO_ROOT }),
  ].map((state) => ({
    ok: state.ok,
