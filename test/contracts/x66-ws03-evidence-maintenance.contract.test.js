...
const MODULE_PATH = path.join(
  ...
  'scripts',
  'ops',
  'x66-ws03-evidence-maintenance-state.mjs',
);
...
test('x66 ws03 baseline evidence maintenance passes', async () => {
  const { evaluateX66Ws03EvidenceMaintenanceState } = await loadModule();
  const state = evaluateX66Ws03EvidenceMaintenanceState({ repoRoot: REPO_ROOT });

  assert.equal(state.ok, true, JSON.stringify(state, null, 2));
  assert.equal(state.X66_WS03_EVIDENCE_MAINTENANCE_OK, 1);
...
test('x66 ws03 repeatability is stable across three runs', async () => {
  const { evaluateX66Ws03EvidenceMaintenanceState } = await loadModule();

  const runs = [
    evaluateX66Ws03EvidenceMaintenanceState({ repoRoot: REPO_ROOT }),
    evaluateX66Ws03EvidenceMaintenanceState({ repoRoot: REPO_ROOT }),
    evaluateX66Ws03EvidenceMaintenanceState({ repoRoot: REPO_ROOT }),
  ].map((state) => ({
    ok: state.ok,
