...
const MODULE_PATH = path.join(
  ...
  'scripts',
  'ops',
  'x53-ws99-symbolic-closeout-and-signed-packet-state.mjs',
);
...
test('x53 ws99 baseline symbolic closeout passes', async () => {
  const { evaluateX53Ws99SymbolicCloseoutAndSignedPacketState } = await loadModule();
  const state = evaluateX53Ws99SymbolicCloseoutAndSignedPacketState({ repoRoot: REPO_ROOT });

  assert.equal(state.ok, true, JSON.stringify(state, null, 2));
  assert.equal(state.X53_WS99_SYMBOLIC_CLOSEOUT_AND_SIGNED_PACKET_OK, 1);
...
test('x53 ws99 repeatability is stable across three runs', async () => {
  const { evaluateX53Ws99SymbolicCloseoutAndSignedPacketState } = await loadModule();

  const runs = [
    evaluateX53Ws99SymbolicCloseoutAndSignedPacketState({ repoRoot: REPO_ROOT }),
    evaluateX53Ws99SymbolicCloseoutAndSignedPacketState({ repoRoot: REPO_ROOT }),
    evaluateX53Ws99SymbolicCloseoutAndSignedPacketState({ repoRoot: REPO_ROOT }),
  ].map((state) => ({
    ok: state.ok,
