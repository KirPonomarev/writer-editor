...
test('central sheet decision is a DOM-free helper consumed by editor runtime', () => {
  ...
  assert.match(editorSource, /const sourcePageCount = Math\.max\(1, decisionPageCount\);/);
  assert.match(editorSource, /const runtimePageCount = Math\.max\(1, structuralMinimumPageCount\);/);
  assert.match(editorSource, /const scrollPageCount = runtimePageCount;/);
  assert.match(editorSource, /sourcePageCount,/);
  assert.match(editorSource, /pageCount: runtimePageCount,/);
  assert.match(editorSource, /scrollPageCount,/);
  assert.doesNotMatch(editorSource, /const sourcePageCount = Math\.max\(decisionPageCount, structuralMinimumPageCount\);/);
  ...
  assert.match(editorSource, /editor\.dataset\.centralSheetStructuralRuntimePageCount = String\(structuralMinimumPageCount \|\| pageCount\);/);
  assert.match(editorSource, /editor\.dataset\.centralSheetBoundedOverflowSourcePageCount = String\(resolvedSourcePageCount \|\| resolvedPageCount\);/);
