...
const outputDir = process.env.BOUNDARY_COORDINATE_TYPE_OUT_DIR
  ? path.resolve(process.env.BOUNDARY_COORDINATE_TYPE_OUT_DIR)
  : await mkdtemp(path.join(os.tmpdir(), '06a2-boundary-coordinate-type-'));
...
function buildPlainText(paragraphCount) {
  const sentence = 'Boundary coordinate type proof paragraph for a derived visual sheet boundary over one Tiptap editor.';
...
async function setEditorPayload(win, paragraphCount) {
  win.webContents.send('editor:set-text', {
    ...
    title: 'boundary-coordinate-type-smoke',
    path: '',
    kind: 'chapter-file',
    metaEnabled: true,
    projectId: 'boundary-coordinate-type-smoke',
    bookProfile: null,
  });
}
...
app.whenReady().then(async () => {
  try {
    ...
    await saveCapture(win, '06a2-before-boundary-type.png');
    ...
    const marker = '06A2_BOUNDARY';
    await win.webContents.insertText(marker);
    await sleep(800);
    const afterType = await collectState(win, 'after-boundary-type');
    await saveCapture(win, '06a2-after-boundary-type.png');
    ...
    const payload = {
      ...
      dialogCalls,
      screenshots: [
        path.join(outputDir, '06a2-before-boundary-type.png'),
        path.join(outputDir, '06a2-after-boundary-type.png'),
      ],
    };
    await fs.writeFile(path.join(outputDir, 'result.json'), JSON.stringify(payload, null, 2));
    process.stdout.write('BOUNDARY_COORDINATE_TYPE_SMOKE_RESULT:' + JSON.stringify(payload) + '\\n');
    app.exit(0);
  } catch (error) {
...
if (exitCode !== 0) {
  ...
  throw new Error(`Electron boundary coordinate type smoke failed with exit ${exitCode}`);
}

const result = JSON.parse(await readFile(path.join(outputDir, 'result.json'), 'utf8'));
if (!result.ok) {
  throw new Error(result.error || 'Electron boundary coordinate type smoke returned not ok');
}
...
assert.match(
  result.boundaryCandidate.proofStrength,
  /^REAL_COORDINATE_CARET_(RANGE|POSITION)_FROM_POINT_PASS$/u,
  'candidate must use a real browser coordinate caret API'
);
assert.equal(result.caretPlacement.ok, true, 'caret must be placed from browser coordinate API');
assert.match(
  result.caretPlacement.proofStrength,
  /^REAL_COORDINATE_CARET_(RANGE|POSITION)_FROM_POINT_PASS$/u,
  'caret placement must use a real browser coordinate caret API'
);
assert.equal(result.caretPlacement.selectionInsideProse, true, 'selection must be inside ProseMirror before type');
assert.equal(result.caretPlacement.activeElementInsideProse, true, 'active element must be inside ProseMirror before type');
assert.equal(
...
assert.equal(markerCount(result.afterType.text, result.marker), 1, 'boundary marker must appear once after type');
assert.equal(markerCount(result.afterUndo.text, result.marker), 0, 'undo must remove boundary marker');
assert.equal(markerCount(result.afterRedo.text, result.marker), 1, 'redo must restore boundary marker');
...
assert.equal(result.afterRedo.rightInspectorVisible, true, 'right inspector must remain visible');
...
const summary = {
  ...
  boundaryCoordinateInsideTextRect: result.boundaryCandidate.insideTextRect,
  boundaryCoordinateInsideFirstSheet: result.boundaryCandidate.insideFirstSheet,
  ...
  markerAfterUndo: markerCount(result.afterUndo.text, result.marker),
  markerAfterRedo: markerCount(result.afterRedo.text, result.marker),
...
process.stdout.write(`BOUNDARY_COORDINATE_TYPE_SMOKE_SUMMARY:${JSON.stringify(summary)}\n`);
