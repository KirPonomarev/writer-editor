...
test('preload ui command bridge: review exact apply uses bridge with intent-only payload', () => {
  ...
  assert.ok(handler.includes('invokePreloadUiCommandBridge(commandId, payload)'))
  assert.match(handler, /const commandId = cleanLinkReturn\s*\? REVIEW_SURFACE_EXACT_TEXT_APPLY_BATCH_COMMAND_ID\s*: REVIEW_SURFACE_EXACT_TEXT_APPLY_COMMAND_ID/)
  assert.ok(handler.includes("changeId.startsWith('docx-clean-link-label-')"))
  assert.ok(handler.includes('reviewSurfaceBuildExactTextApplyBatchPayload(requestId, [changeId])'))
