...
const targetPageCount = Number.parseInt(process.env.EDITOR_SHEET_6000_DIAGNOSTIC_TARGET_PAGE_COUNT || '6000', 10);
const rowTimeoutMs = Number.parseInt(process.env.EDITOR_SHEET_6000_DIAGNOSTIC_TIMEOUT_MS || '180000', 10);
const allowResourceStop = process.env.EDITOR_SHEET_STRESS_ALLOW_RESOURCE_STOP === 'true';
assert.equal(targetPageCount, 6000, 'diagnostic target page count must be 6000');
...
const outputDir = process.env.EDITOR_SHEET_STRESS_OUT_DIR
  ...
  : await mkdtemp(path.join(os.tmpdir(), `editor-sheet-${targetPageCount}-light-health-probe-`));
...
app.whenReady().then(async () => {
  try {
    ...
    const calibratedUnitCount = Number.parseInt(
      process.env.EDITOR_SHEET_6000_DIAGNOSTIC_UNIT_COUNT || String(Math.ceil(targetPageCount * 2.057)),
      10,
    );
    if (!Number.isInteger(calibratedUnitCount) || calibratedUnitCount <= 0) {
      throw new Error('DIAGNOSTIC_UNIT_COUNT_INVALID');
    }
    await checkpoint('diagnostic_calibrated_unit_count', {
      targetPageCount,
      calibratedUnitCount,
    });
    process.stdout.write('LONGFORM_STRESS_PROGRESS:' + JSON.stringify({
      event: 'diagnostic-calibrated-unit-count',
      calibratedUnitCount,
    }) + '\\n');
    const diagnostic = await runScenario(win, {
      label: 'diagnostic-' + String(targetPageCount),
      phase: 'DIAGNOSTIC',
      targetPageCount,
    ...
    process.stdout.write('EDITOR_SHEET_6000_LIGHT_HEALTH_PROBE_RESULT:' + JSON.stringify(payload) + '\\n');
    app.exit(0);
  } catch (error) {
    const payload = {
      ok: false,
...
function parseResult(stdout) {
  const line = String(stdout)
    .split(/\r?\n/u)
    .find((item) => item.startsWith('EDITOR_SHEET_6000_LIGHT_HEALTH_PROBE_RESULT:'));
  assert.ok(line, '6000 light health probe must emit result payload');
  return JSON.parse(line.slice('EDITOR_SHEET_6000_LIGHT_HEALTH_PROBE_RESULT:'.length));
}
...
const helperPath = path.join(outputDir, `editor-sheet-${targetPageCount}-light-health-probe-helper.cjs`);
...
assert.equal(payload.ok, true, payload.error || 'EDITOR_SHEET_6000_LIGHT_HEALTH_PROBE_FAILED');
assert.equal(payload.networkRequests, 0);
assert.equal(payload.dialogCalls, 0);
assert.equal(payload.scenarios.length, 1);

const diagnostic = payload.scenarios.find((scenario) => scenario.phase === 'DIAGNOSTIC');
assert.ok(diagnostic, 'diagnostic scenario must exist');
assertScenario(diagnostic);
assert.ok(diagnostic.typing, 'diagnostic scenario must include typing telemetry');
assert.equal(diagnostic.typing.focusResult.ok, true);
assert.notEqual(diagnostic.typing.afterTypeHash, diagnostic.typing.beforeTypeHash);
assert.equal(diagnostic.typing.afterUndoHash, diagnostic.typing.beforeTypeHash);
assert.equal(diagnostic.typing.afterRedoHash, diagnostic.typing.afterTypeHash);
assert.equal(diagnostic.typing.afterCleanupHash, diagnostic.typing.beforeTypeHash);
