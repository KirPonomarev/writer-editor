...
test('preload workspace query bridge: main has one query bridge handler with strict allowlist', () => {
  ...
  assert.ok(source.includes("'query.metadataInspector'"))
  ...
  assert.ok(source.includes("if (queryId === 'query.metadataInspector') {"))
  assert.ok(source.includes('return handleWorkspaceMetadataInspectorQuery(payload);'))
...
test('preload workspace query bridge: editor tree collab and review surface reads use query bridge only', () => {
  ...
  assert.ok(source.includes("const METADATA_INSPECTOR_QUERY_ID = 'query.metadataInspector';"))
