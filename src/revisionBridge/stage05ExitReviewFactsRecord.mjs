...
const RESULT_KIND = 'STAGE_05O_STAGE05_EXIT_REVIEW_FACTS_RECORD_RESULT_001';
const DECISION_KIND = 'STAGE_05O_STAGE05_EXIT_REVIEW_FACTS_RECORD_DECISION_001';
const CONTOUR_ID = 'STAGE05O_STAGE05_EXIT_REVIEW_FACTS_RECORD_ONLY_001';

const ALLOWED_CHANGED_BASENAMES = new Set([
  'stage05ExitReviewFactsRecord.mjs',
  'stage05ExitReviewFactsRecord.contract.test.js',
  'STAGE05O_STAGE05_EXIT_REVIEW_FACTS_RECORD_ONLY_001.md',
]);
...
const HASH_REF_SPECS = Object.freeze([
  ...
  },
  {
    ...
    advisoryReason: 'STAGE05N_DECISION_RECORD_NOT_EXIT_READY',
  },
]);
...
const BLOCKING_REASON_CODES = new Set([
  ...
  'MISSING_OWNER_POLICY_OPTION_OBSERVATION',
  'UNKNOWN_OWNER_POLICY_OPTION_OBSERVATION',
...
const ALLOWED_INPUT_KEYS = new Set([
  ...
  'ownerPolicyOptionObserved',
  'ownerPolicyOptionObservation',
  'stage05nOwnerPolicyDecisionRecordOutput',
...
export const STAGE05_EXIT_REVIEW_FACTS_RECORD_REASON_CODES = Object.freeze({
  ...
  STAGE05N_DECISION_RECORD_NOT_EXIT_READY: 'STAGE05N_DECISION_RECORD_NOT_EXIT_READY',
  MISSING_OWNER_POLICY_OPTION_OBSERVATION: 'MISSING_OWNER_POLICY_OPTION_OBSERVATION',
  UNKNOWN_OWNER_POLICY_OPTION_OBSERVATION: 'UNKNOWN_OWNER_POLICY_OPTION_OBSERVATION',
  OWNER_POLICY_OPTION_OBSERVED_ONLY_NOT_ACCEPTED: 'OWNER_POLICY_OPTION_OBSERVED_ONLY_NOT_ACCEPTED',
...
export const STAGE05_EXIT_REVIEW_FACTS_RECORD_DECISIONS = Object.freeze({
  STAGE05_EXIT_REVIEW_FACTS_RECORDED: 'STAGE05_EXIT_REVIEW_FACTS_RECORDED',
  STAGE05_EXIT_REVIEW_FACTS_BLOCKED: 'STAGE05_EXIT_REVIEW_FACTS_BLOCKED',
  STOP_OWNER_POLICY_REQUIRED: 'STOP_OWNER_POLICY_REQUIRED',
});

export const STAGE05_EXIT_REVIEW_FACTS_OWNER_POLICY_OPTIONS = OWNER_POLICY_OPTIONS;
...
function pushChangedBasenameReasons(input, reasons) {
  if (!Array.isArray(input?.changedBasenames)) {
    reasons.push(STAGE05_EXIT_REVIEW_FACTS_RECORD_REASON_CODES.MISSING_CHANGED_BASENAMES_EVIDENCE);
    return;
  }
  ...
  if (input.changedBasenames.some((basename) => !ALLOWED_CHANGED_BASENAMES.has(basename))) {
    reasons.push(STAGE05_EXIT_REVIEW_FACTS_RECORD_REASON_CODES.FORBIDDEN_BASENAME_CHANGE);
  }
}

function pushInputShapeReasons(input, reasons) {
  if (Object.keys(input).some((key) => !ALLOWED_INPUT_KEYS.has(key))) {
    reasons.push(STAGE05_EXIT_REVIEW_FACTS_RECORD_REASON_CODES.UNKNOWN_FIELD_FORBIDDEN);
  }
  if (containsCallable(input)) {
    reasons.push(STAGE05_EXIT_REVIEW_FACTS_RECORD_REASON_CODES.CALLABLE_FIELD_FORBIDDEN);
  }
  if (containsUserProjectPath(input)) {
    reasons.push(STAGE05_EXIT_REVIEW_FACTS_RECORD_REASON_CODES.USER_PROJECT_PATH_FORBIDDEN);
  }
}
...
function resolveHashRef(input = {}, spec) {
  ...
  const reasonCodes = [];
  if (!evidencePresent) {
    reasonCodes.push(STAGE05_EXIT_REVIEW_FACTS_RECORD_REASON_CODES[spec.missingReason]);
  } else {
    reasonCodes.push(STAGE05_EXIT_REVIEW_FACTS_RECORD_REASON_CODES[spec.advisoryReason]);
  }
  if (stale) {
    reasonCodes.push(STAGE05_EXIT_REVIEW_FACTS_RECORD_REASON_CODES[spec.staleReason]);
  }

  return withCanonicalHash({
    refKind: 'Stage05ExitReviewRequiredHashRef',
...
function resolveOwnerPolicyOptionObservation(input = {}) {
  const sourceOutput = isObject(input.stage05nOwnerPolicyDecisionRecordOutput)
    ? input.stage05nOwnerPolicyDecisionRecordOutput
    : {};
  const sourceRecord = isObject(sourceOutput.policyOptionRecord) ? sourceOutput.policyOptionRecord : {};
  const rawValue = hasText(input.ownerPolicyOptionObserved)
    ? input.ownerPolicyOptionObserved
    : (
      hasText(input.ownerPolicyOptionObservation)
        ? input.ownerPolicyOptionObservation
        : (hasText(sourceRecord.selectedPolicyOption) ? sourceRecord.selectedPolicyOption : '')
    );
  const reasonCodes = [];
  if (!hasText(rawValue)) {
    reasonCodes.push(STAGE05_EXIT_REVIEW_FACTS_RECORD_REASON_CODES.MISSING_OWNER_POLICY_OPTION_OBSERVATION);
  } else if (!Object.values(OWNER_POLICY_OPTIONS).includes(rawValue)) {
    reasonCodes.push(STAGE05_EXIT_REVIEW_FACTS_RECORD_REASON_CODES.UNKNOWN_OWNER_POLICY_OPTION_OBSERVATION);
  } else {
    reasonCodes.push(STAGE05_EXIT_REVIEW_FACTS_RECORD_REASON_CODES.OWNER_POLICY_OPTION_OBSERVED_ONLY_NOT_ACCEPTED);
  }

  return withCanonicalHash({
    observationKind: 'Stage05OwnerPolicyOptionObservation',
    contourId: CONTOUR_ID,
    selectedPolicyOption: rawValue,
    optionKnown: Object.values(OWNER_POLICY_OPTIONS).includes(rawValue),
    observedOnly: true,
...
function pushForbiddenClaimReasons(input = {}, reasons) {
  if (
    ...
    reasons.push(STAGE05_EXIT_REVIEW_FACTS_RECORD_REASON_CODES.FORBIDDEN_POLICY_ACCEPTANCE_CLAIM);
  }
  if (
    ...
    reasons.push(STAGE05_EXIT_REVIEW_FACTS_RECORD_REASON_CODES.FORBIDDEN_PROJECT_TRUTH_ACCEPTANCE_CLAIM);
  }
  if (
    ...
    reasons.push(STAGE05_EXIT_REVIEW_FACTS_RECORD_REASON_CODES.FORBIDDEN_STAGE05_EXIT_READY_CLAIM);
  }
  if (isClaimed(input.stage06PreAdmittedClaimed) || isClaimed(input.stage06PreAdmissionClaimed)) {
    reasons.push(STAGE05_EXIT_REVIEW_FACTS_RECORD_REASON_CODES.FORBIDDEN_STAGE06_PRE_ADMISSION_CLAIM);
  }
  if (isClaimed(input.stage06PermissionClaimed) || isClaimed(input.stage06AdmissionClaimed)) {
    reasons.push(STAGE05_EXIT_REVIEW_FACTS_RECORD_REASON_CODES.FORBIDDEN_STAGE06_PERMISSION_CLAIM);
  }
  if (
    ...
    reasons.push(STAGE05_EXIT_REVIEW_FACTS_RECORD_REASON_CODES.FORBIDDEN_APPLYTXN_CLAIM);
  }
  if (
    ...
    reasons.push(STAGE05_EXIT_REVIEW_FACTS_RECORD_REASON_CODES.FORBIDDEN_RUNTIME_APPLY_CLAIM);
  }
  if (isClaimed(input.storageMigrationClaimed) || isClaimed(input.storageMutationClaimed)) {
    reasons.push(STAGE05_EXIT_REVIEW_FACTS_RECORD_REASON_CODES.FORBIDDEN_STORAGE_MIGRATION_CLAIM);
  }
  if (isClaimed(input.projectWriteClaimed) || isClaimed(input.projectWritePerformedClaimed)) {
    reasons.push(STAGE05_EXIT_REVIEW_FACTS_RECORD_REASON_CODES.FORBIDDEN_PROJECT_WRITE_CLAIM);
  }
  if (
    ...
    reasons.push(STAGE05_EXIT_REVIEW_FACTS_RECORD_REASON_CODES.FORBIDDEN_STABLE_ID_CREATION_CLAIM);
  }
  if (
    ...
    reasons.push(STAGE05_EXIT_REVIEW_FACTS_RECORD_REASON_CODES.FORBIDDEN_LINEAGE_CREATION_CLAIM);
  }
  if (isClaimed(input.reviewAnchorPromotedClaimed) || isClaimed(input.reviewAnchorHandlePromotedClaimed)) {
    reasons.push(STAGE05_EXIT_REVIEW_FACTS_RECORD_REASON_CODES.FORBIDDEN_REVIEW_ANCHOR_PROMOTION_CLAIM);
  }
  if (isClaimed(input.structuralAutoApplyClaimed) || isClaimed(input.moveSplitMergeAutoApplyClaimed)) {
    reasons.push(STAGE05_EXIT_REVIEW_FACTS_RECORD_REASON_CODES.FORBIDDEN_STRUCTURAL_AUTO_APPLY_CLAIM);
  }
  if (
    ...
    reasons.push(STAGE05_EXIT_REVIEW_FACTS_RECORD_REASON_CODES.FORBIDDEN_UI_DOCX_NETWORK_DEPENDENCY_CLAIM);
  }
}
  ...
  for (const value of rawValues) {
    ...
    if (matchedTerms.length > 0) {
      findings.push(withCanonicalHash({
        findingKind: 'ForbiddenStage05ExitReviewPermissionLanguageFinding',
        contourId: CONTOUR_ID,
        language: value,
        matchedTerms: uniqueSorted(matchedTerms),
        reasonCode: STAGE05_EXIT_REVIEW_FACTS_RECORD_REASON_CODES.FORBIDDEN_PERMISSION_LANGUAGE_FOUND,
      }));
    }
  }
  return findings;
...
function createDecision(blockedReasons) {
  ...
  const outputDecision = blockedReasons.some((reason) => stopReasons.has(reason))
    ? STAGE05_EXIT_REVIEW_FACTS_RECORD_DECISIONS.STOP_OWNER_POLICY_REQUIRED
    : (
      blockedReasons.length > 0
        ? STAGE05_EXIT_REVIEW_FACTS_RECORD_DECISIONS.STAGE05_EXIT_REVIEW_FACTS_BLOCKED
        : STAGE05_EXIT_REVIEW_FACTS_RECORD_DECISIONS.STAGE05_EXIT_REVIEW_FACTS_RECORDED
    );
...
function buildReviewBom({ hashRefs, ownerPolicyOptionObservation, blockedReasons, permissionFindings }) {
  return withCanonicalHash({
    bomKind: 'Stage05ExitReviewFactsBOM',
    contourId: CONTOUR_ID,
    factsOnly: true,
    ...
    ownerPolicyOptionObservedOnlyCount: ownerPolicyOptionObservation.observedOnly ? 1 : 0,
    ...
    ownerPolicyOptionReasonCodes: ownerPolicyOptionObservation.reasonCodes,
    hashRefHashes: hashRefs.map((ref) => ref.canonicalHash).sort(),
    ownerPolicyOptionObservationHash: ownerPolicyOptionObservation.canonicalHash,
...
function buildNextStepPreview({ blockedReasons, reviewBom }) {
  return withCanonicalHash({
    packetKind: 'Stage05ExitReviewFactsNextStepPreview',
    contourId: CONTOUR_ID,
    previewOnly: true,
    nextStepRecommendationOnly: true,
    automaticNextContourOpened: false,
    recommendedSeparateNextReview: 'FALSE_GREEN_REVIEW_OR_OWNER_DECISION_RECON_ONLY',
...
export function compileStage05ExitReviewFactsRecord(input = {}) {
  ...
  const ownerPolicyOptionObservation = resolveOwnerPolicyOptionObservation(input);
  for (const code of ownerPolicyOptionObservation.reasonCodes) {
    if (BLOCKING_REASON_CODES.has(code)) {
      blockedReasons.push(code);
    }
  }
  ...
  if (permissionFindings.length > 0) {
    blockedReasons.push(STAGE05_EXIT_REVIEW_FACTS_RECORD_REASON_CODES.FORBIDDEN_PERMISSION_LANGUAGE_FOUND);
  }
  ...
  return withCanonicalHash({
    ...
    stage05ExitReviewFactsRecorded:
      decision.outputDecision === STAGE05_EXIT_REVIEW_FACTS_RECORD_DECISIONS.STAGE05_EXIT_REVIEW_FACTS_RECORDED,
    stage05ExitReviewFactsRecordOnly: true,
    factsOnly: true,
...
export const runStage05ExitReviewFactsRecord = compileStage05ExitReviewFactsRecord;
export const compileStage05ExitReviewFactsRecordOnly = compileStage05ExitReviewFactsRecord;
export const runStage05ExitReviewFactsRecordOnly = compileStage05ExitReviewFactsRecord;
