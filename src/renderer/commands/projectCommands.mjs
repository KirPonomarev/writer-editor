...
export const COMMAND_IDS = Object.freeze({
  ...
  PROJECT_IMPORT_FULL_ARCHIVE_V1: COMMAND_KEY_TO_ID.PROJECT_IMPORT_FULL_ARCHIVE_V1,
...
export const EXTRA_COMMAND_IDS = Object.freeze({
  ...
  REVIEW_EXPORT_FULL_MANUSCRIPT_DOCX_REVIEW_PACKET: 'cmd.project.review.exportFullManuscriptDocxReviewPacket',
...
async function runReviewExportFullManuscriptDocxReviewPacketBridge(electronAPI, input = {}) {
  if (!electronAPI || typeof electronAPI !== 'object') {
    return fail(
      'E_COMMAND_FAILED',
      EXTRA_COMMAND_IDS.REVIEW_EXPORT_FULL_MANUSCRIPT_DOCX_REVIEW_PACKET,
      'ELECTRON_API_UNAVAILABLE',
    );
  }
  ...
  try {
    ...
    return fail(
      'E_COMMAND_FAILED',
      EXTRA_COMMAND_IDS.REVIEW_EXPORT_FULL_MANUSCRIPT_DOCX_REVIEW_PACKET,
      'REVIEW_FULL_MANUSCRIPT_DOCX_EXPORT_IPC_FAILED',
      { message: error && typeof error.message === 'string' ? error.message : 'UNKNOWN' },
    );
  }
  ...
  if (bridged && bridged.ok === false && bridged.error && typeof bridged.error === 'object') {
    const error = bridged.error;
    return fail(
      typeof error.code === 'string' ? error.code : 'E_REVIEW_FULL_MANUSCRIPT_DOCX_EXPORT_FAILED',
      typeof error.op === 'string' ? error.op : EXTRA_COMMAND_IDS.REVIEW_EXPORT_FULL_MANUSCRIPT_DOCX_REVIEW_PACKET,
      typeof error.reason === 'string' ? error.reason : 'REVIEW_FULL_MANUSCRIPT_DOCX_EXPORT_FAILED',
      error.details && typeof error.details === 'object' && !Array.isArray(error.details) ? error.details : undefined,
    );
  }
  return fail(
    ...
    bridged && typeof bridged.reason === 'string'
      ? bridged.reason
      : 'REVIEW_FULL_MANUSCRIPT_DOCX_EXPORT_INVALID_RESPONSE',
  );
}
...
export function registerProjectCommands(registry, options = {}) {
  ...
  registry.registerCommand(
    {
      id: EXTRA_COMMAND_IDS.REVIEW_EXPORT_FULL_MANUSCRIPT_DOCX_REVIEW_PACKET,
      label: 'Export Full Manuscript Review DOCX Packet',
      group: 'review',
      surface: ['menu', 'palette'],
      hotkey: '',
    },
    async (input = {}) => runReviewExportFullManuscriptDocxReviewPacketBridge(electronAPI, input),
  );
