...
- Phase 03 file-authority hardening is implemented and locally verified pending delivery: one shared infrastructure port performs bounded identity-checked reads for DOCX content, DOCX Review, TXT, Review Packet, and Markdown local-file intake, and physically validates DOCX, Markdown, TXT, and Review Packet export targets both after Save As and again inside the disk queue.
- The new Markdown authority routes keep picker, opaque one-use preview state, safe-create payload, Save As target, and recovery paths in main; renderer input is request metadata only and public success envelopes contain no filesystem paths. These routes are not yet visible-product-bound, and canonical saved-scene serialization remains Phase 04.
- Current closure classification: DOCX minimal export, DOCX content-only safe-create import, TXT flows, Review Packet V1 import/export, canonical same-document exact apply through the real Review UI, and Apply crash reconciliation are delivered; DOCX Review is evidence only; Phase 03 file authority is implemented pending delivery; the local Markdown file product flow remains partial until Phase 04; Word and Google artifact claims, Mindmap export, and project-level cross-scene apply are deferred.
