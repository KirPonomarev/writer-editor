...
- Active local truth-repair contour is `REVIEW_BRIDGE_FIRST_USEFUL_RELEASE_TRUTH_REPAIR_001_STATUS.json` with status `implemented_verified_pending_delivery`: the current branch removes public `cmd.project.review.importPacket` from the UI command bridge and public menu handlers, keeps `cmd.project.review.importLocalPacket` as the only public local JSON packet intake path, and keeps `handleReviewSurfaceImportPacketCommandSurface` internal to main-owned session normalization and DOCX preview reuse.
