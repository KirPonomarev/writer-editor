...
- DOCX review preview session activation is implemented locally and tracked by `REVIEW_BRIDGE_DOCX_PREVIEW_SESSION_ACTIVATION_001_STATUS.json`: `cmd.project.review.activateDocxReviewPreviewSession` accepts pathless DOCX bytes, converts DOCX comments into bounded Review Packet comment threads, activates an in-memory Review session through Stage01 preview, and opens Review/Comments on success.
