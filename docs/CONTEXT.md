...
- Phase 05 product discoverability and honest labels are implemented and locally verified pending delivery: the minimal-profile File menu exposes DOCX, TXT, and Markdown content import plus DOCX Minimal, Markdown, and all three TXT exports; Review distinguishes Review Packet Exact Apply from DOCX Review Evidence with Comments preview and Tracked changes diagnostic limits.
...
- Current closure classification: DOCX minimal export, DOCX content-only safe-create import, TXT flows, bounded Markdown V1 local-file import/export, Review Packet V1 import/export, canonical same-document exact apply through the real Review UI, Apply crash reconciliation, and external file authority are delivered; DOCX Review is evidence only; Phase 05 discoverability is implemented pending delivery and Phase 06 capability evidence remains open; Word and Google artifact claims, Mindmap export, and project-level cross-scene apply are deferred.
