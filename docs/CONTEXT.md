...
- Current closure classification: DOCX minimal export, DOCX content-only safe-create import, TXT flows, bounded Markdown V1 local-file import/export, Review Packet V1 import/export, canonical same-document exact apply through the real Review UI, Apply crash reconciliation, and external file authority are delivered; DOCX Review is evidence only; Phase 05 discoverability and Phase 06 capability evidence remain open; Word and Google artifact claims, Mindmap export, and project-level cross-scene apply are deferred.
