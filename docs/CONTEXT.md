...
- Review Bridge first useful release gate is opened by `REVIEW_BRIDGE_FIRST_USEFUL_RELEASE_GATE_001_STATUS.json` as a bounded feature gate only: local JSON review packet import to Review surface plus one exact text safe apply with receipt and recovery evidence, with DOCX comments-only preview as a separate manual preview surface.
