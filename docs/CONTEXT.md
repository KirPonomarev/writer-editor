...
- Current closure classification: DOCX minimal export, DOCX content-only safe-create import, TXT flows, Review Packet V1 import/export, and canonical same-document exact apply through the real Review UI are delivered; DOCX Review is evidence only; the local Markdown file product flow is partial product flow; crash reconciliation remains the next P0; Word and Google artifact claims, Mindmap export, and project-level cross-scene apply are deferred.
