...
- All scenes TXT export is delivered on current mainline via PR `1068` at merge SHA `a27bd9757dc94bcb166e349302dbebdab1ce3c07`; it remains a separate post-MVP contour and does not widen `IMPORT_EXPORT_PRODUCT_ACCEPTANCE_GATE_001_STATUS.json`.
...
- Current post-MVP TXT import lane is delivered on current mainline via PR `1061` at merge SHA `cc0f226f68341c649423ba8761b91939cd2dbade`; it remains separate from that closed MVP gate and does not widen `IMPORT_EXPORT_PRODUCT_ACCEPTANCE_GATE_001_STATUS.json`.
...
- Current closure classification: DOCX minimal export, DOCX content-only safe-create import, TXT flows, bounded Markdown V1 local-file import/export, Review Packet V1 import/export, canonical same-document exact apply through the real Review UI, Apply crash reconciliation, external file authority, and Phase 05 discoverability are delivered; DOCX Review is evidence only; Phase 06 capability evidence remains open; Word and Google artifact claims, Mindmap export, and project-level cross-scene apply are deferred.
