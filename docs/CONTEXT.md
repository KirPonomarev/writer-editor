...
- Current DOCX review local-file entry work is tracked by `REVIEW_BRIDGE_DOCX_REVIEW_LOCAL_FILE_ENTRY_001_STATUS.json`: Review menu exposes `cmd.project.review.openDocxReviewPreviewSession`, main owns DOCX picker/stat/read/restat and passes selected bytes into the delivered activation path, while renderer/menu payload authority stays limited to `requestId`.
- The DOCX review local-file entry does not route through DOCX import preview or DOCX safe-create and does not write manuscript truth, project truth, receipt, or recovery evidence.
