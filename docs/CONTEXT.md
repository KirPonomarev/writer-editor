...
- Literal block markers, widened code fences, code blank lines, flat lists, Unicode, CRLF normalization, combined inline marks, parenthesized links, inline backtick runs, and explicit downgrade cases are covered by deterministic goldens. Full CommonMark/GFM, final menu labels, duplicate alias cleanup, and unified capability evidence remain outside Phase 04.
- Current closure classification: DOCX minimal export, DOCX content-only safe-create import, TXT flows, Review Packet V1 import/export, canonical same-document exact apply through the real Review UI, Apply crash reconciliation, and Phase 03 external file authority are delivered; DOCX Review is evidence only; Phase 04 Markdown product completion is implemented pending delivery; Phase 05 discoverability and Phase 06 capability evidence remain open; Word and Google artifact claims, Mindmap export, and project-level cross-scene apply are deferred.
