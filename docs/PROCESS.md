...
### Агентский протокол для задач интерфейса

Протокол обязателен, если задача создаёт новый продуктовый экран или заметно
меняет существующий дизайн-контракт: композицию, пользовательский сценарий,
модель состояний компонента, типографику, палитру, токены, систему отступов или
утверждённый визуальный эталон.

Шаг 0 выполняется до открытия дизайн-референсов и вызова любого дизайн-MCP:

1. Спросить: меняет ли требуемый результат дизайн-контракт продуктового интерфейса?
2. Если нет — записать `DESIGN_TOOL_ROUTER: NOT_APPLICABLE` и завершить дизайн-протокол. Не запускать дизайн-исследование только потому, что инструменты доступны.
3. Если да — записать `DESIGN_TOOL_ROUTER: APPLIED`, назвать конкретный визуальный вопрос и только после этого идти по маршруту ниже.

Статус `ACTIVE` в матрице означает «доступен для вызова», а не «обязателен в
каждой задаче». Для одного вопроса выбирается минимальный достаточный инструмент;
резерв открывается только при доказанном пробеле текущей матрицы.

Порядок работы:
1. Сначала определить источник истины: канон, актуальный Design OS, утверждённый скрин/Figma или прямое решение владельца.
2. Прочитать `docs/YALKEN_DESIGN_OS_CHANGE_GUIDE_V2_2.md` и `docs/references/YALKEN_DESIGN_TOOL_MATRIX_V1.md`.
3. Выполнить `npm run brain:refs -- "ключевые слова задачи"` и добавить найденные внутренние материалы в ТЗ.
4. Для продуктового UI начать с Lazyweb. Другие инструменты выбирать по триггерам матрицы и вопросу задачи, а не запускать весь пул автоматически.
5. Если направление не утверждено, подготовить не больше трёх проверяемых вариантов и получить выбор владельца до переноса в код.
6. Реализовать выбранное направление только существующими поверхностями Yalken и в пределах allowlist/diff budget задачи.
7. После реализации выполнить релевантные `Audit`, `Heuristic`, `Project Wallace`, проверку в Browser и `Finalize` по условиям матрицы.

В ТЗ или итоговом отчёте UI-контура фиксируются поля:
- `DESIGN_TOOL_ROUTER: APPLIED | NOT_APPLICABLE`
- `DESIGN_SOURCE_OF_TRUTH`: какой эталон имеет приоритет
- `DESIGN_RESEARCH_SOURCES`: какие внутренние и внешние источники реально использованы
- `DESIGN_SELECTED_DIRECTION`: что выбрано владельцем или уже было закреплено каноном
- `DESIGN_AUDIT_TOOLS`: какие проверки реально выполнены
- `DESIGN_TOOL_DEVIATIONS`: что пропущено или отклонено и почему

При `NOT_APPLICABLE` достаточно зафиксировать только `DESIGN_TOOL_ROUTER`;
остальные дизайн-поля не заполняются и инструменты не вызываются.

`NOT_APPLICABLE` обязателен для backend/CLI/infra/docs-only задач и механической
визуальной правки, которая не меняет дизайн-контракт. Малый diff не является
исключением, если меняются системные цвет, типографика, отступы, состояния или
эталонный снимок.

Матрица не создаёт новый продуктовый канон, не заменяет решение владельца, не
расширяет область задачи и не разрешает новые зависимости, сеть, облачные
сервисы или внешний дизайн-системный runtime. При конфликте инструментального
совета с каноном или утверждённым эталоном агент обязан остановить перенос этого
совета и явно записать расхождение.
...
- Если `main` ушёл вперёд от task base SHA и это делает PR немержабельным по scope-policy, задача останавливается и запрашивает новый owner-approved base SHA; тихий rebase запрещён.
