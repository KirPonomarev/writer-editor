...
- Current post-MVP TXT import lane is delivered on current mainline via PR `1061` at merge SHA `cc0f226f68341c649423ba8761b91939cd2dbade`; it stays outside that closed MVP gate, does not widen `IMPORT_EXPORT_PRODUCT_ACCEPTANCE_GATE_001_STATUS.json`, and does not reopen the accepted MVP artifact set.
...
- Phase 03 file-authority hardening is implemented and locally verified pending delivery: bounded identity-checked reads cover DOCX content, DOCX Review, TXT, Review Packet, and Markdown local files, while one physical target validator covers DOCX, Markdown, TXT, and Review Packet exports with queue-time revalidation.
- New Markdown local-file authority routes keep picker, preview token, safe-create payload, Save As target, and recovery paths in main and accept only request metadata from renderer; visible command binding and canonical semantic serialization remain Phase 04.
