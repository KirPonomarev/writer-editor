...
- Apply crash reconciliation is implementation-verified and pending delivery under `REVIEW_BRIDGE_APPLY_CRASH_RECONCILIATION_001_STATUS.json`: durable operationId journals, canonical before/after hashes, verified recovery snapshots, and startup reconciliation now distinguish not-applied, applied-without-receipt, and conflict without automatic repeat Apply.
...
- Current closure classification: DOCX minimal export, DOCX content-only safe-create import, TXT flows, Review Packet V1 import/export, and canonical same-document exact apply through the real Review UI are delivered; Apply crash reconciliation is verified pending merge; DOCX Review is evidence only; the local Markdown file product flow is partial product flow; file-authority and path-boundary hardening is next after delivery; Word and Google artifact claims, Mindmap export, and project-level cross-scene apply are deferred.
