...
- Phase 04 Markdown product completion is implemented and locally verified pending delivery: visible palette actions bind the main-owned picker, pathless write-free preview, explicit one-use-token accept, canonical `doc-v2` safe-create, and main-owned Save As.
...
- Ten deterministic goldens cover literal block markers, code fences and blank lines, lists, Unicode, CRLF, inline marks and links, backtick delimiters, and explicit downgrades. Full CommonMark/GFM, final File/Review labels, transition-alias cleanup, and unified capability evidence remain Phase 05 and Phase 06 work.
- Current closure classification: DOCX minimal export, DOCX content-only safe-create import, TXT flows, Review Packet V1 import/export, canonical same-document exact apply through the real Review UI, Apply crash reconciliation, and Phase 03 external file authority are delivered; DOCX Review is evidence only; Phase 04 Markdown completion is implemented pending delivery; Word and Google artifact claims, Mindmap export, and project-level cross-scene apply are deferred.
