...
- Current closure classification: DOCX minimal export, DOCX content-only safe-create import, TXT flows, Review Packet V1 import/export, canonical same-document exact apply through the real Review UI, and Apply crash reconciliation are delivered; DOCX Review is evidence only; the local Markdown file product flow is partial product flow; file-authority and path-boundary hardening is next; Word and Google artifact claims, Mindmap export, and project-level cross-scene apply are deferred.
