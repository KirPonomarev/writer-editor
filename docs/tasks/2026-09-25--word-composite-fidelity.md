...
Native Office conversion of an unchanged relationship hyperlink to a complex HYPERLINK field is handled only as inert text correspondence. Balanced non-nested fields must contain one bounded http(s) target, no credentials or tracked edits inside the field, and their exact original ranges/targets must match authenticated local preserved link marks. Unknown instructions, nested/unbalanced fields, changed destinations and edited field contents remain blocked. No field execution or network access is added.

The current Lab public-locator fixture now uses the real fenced manifest authority introduced by W1, replacing its obsolete algorithmic ACK stub. All 92 required Lab tests and doctor pass; three unchanged historical fixture directories were restored from registered canonical Lab and remain unit-test inputs only. The new companion binding set is append-only; existing Lab binding sets, runtime code and raw oracles are unchanged.

Native SOURCE and PACKAGED composite checks prove that editing the second of two identical scene fragments changes only its authenticated scene. The first scene remains byte-identical, and the same PNG retains distinct alt text and dimensions in both scenes after product reopen, re-export and native Word reopen. The independent ZIP/XML reader compares ordered paragraphs and image hashes/extents. These are extra W6 regressions with zero self-awarded cell credit.
