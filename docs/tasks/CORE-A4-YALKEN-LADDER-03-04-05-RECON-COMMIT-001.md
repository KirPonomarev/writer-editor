TASK_ID: CORE-A4-YALKEN-LADDER-03-04-05-RECON-COMMIT-001
MILESTONE: A4
TYPE: CORE
...
## MICRO_GOAL
Выполнить один narrow commit только для queue-head pair `CORE-A4-YALKEN-LADDER-03-04-05-RECON-001.md` и `CONTOUR_03_04_05_ORDER_RECONCILIATION_DECISION_RECORD_V1.json` плюс одного commit-task artifact. Этот task использует exact-path staging only, создает ровно один commit, не патчит terminal record outcome fields, не делает второй commit и не открывает runtime admission.

## ENTRY_CRITERIA
- CURRENT_LANE_OWNER_BLOCKER_DECISION_RECORD_V1.json says `ALLOW_ONE_SEPARATE_QUEUE_HEAD_PATCH_TASK_ONLY`
- CURRENT_LANE_COMMIT_REMEDIATION_LEDGER_V1.json keeps the same queue head and exact-path staging requirement
- CORE-A4-YALKEN-LADDER-03-04-05-RECON-PRECOMMIT-001.md stays green and says `READY_FOR_NARROW_COMMIT`

## ARTIFACT
- docs/tasks/CORE-A4-YALKEN-LADDER-03-04-05-RECON-COMMIT-001.md
...
- COMMIT_TASK_BASENAME: CORE-A4-YALKEN-LADDER-03-04-05-RECON-COMMIT-001.md
...
- post-commit outcome patch is a separate later task
...
7) Остановиться; outcome patch не делать в этом task.

## CHECKS
CHECK_01_PRE_OWNER_DECISION_PASS
CMD: node -e 'const fs=require("node:fs");const j=JSON.parse(fs.readFileSync("docs/OPS/STATUS/CURRENT_LANE_OWNER_BLOCKER_DECISION_RECORD_V1.json","utf8"));if(j.decision!=="ALLOW_ONE_SEPARATE_QUEUE_HEAD_PATCH_TASK_ONLY")process.exit(1);if(j.queueHeadTaskMayOpen!==true)process.exit(1);if(j.commitLoopStarted!==false)process.exit(1);if(j.runtimeWritesAdmitted!==false)process.exit(1);process.exit(0);'
PASS: exit 0

CHECK_02_PRE_LEDGER_AND_PRECOMMIT_PASS
CMD: node -e 'const fs=require("node:fs");const ledger=JSON.parse(fs.readFileSync("docs/OPS/STATUS/CURRENT_LANE_COMMIT_REMEDIATION_LEDGER_V1.json","utf8"));const t=fs.readFileSync("docs/tasks/CORE-A4-YALKEN-LADDER-03-04-05-RECON-PRECOMMIT-001.md","utf8");if(ledger.firstExecutableQueueHeadBasename!=="CORE-A4-YALKEN-LADDER-03-04-05-RECON-001.md")process.exit(1);if(ledger.exactPathStagingRequired!==true)process.exit(1);if(!t.includes("STATUS: PRE_COMMIT_CHECKS_GREEN_READY_FOR_NARROW_COMMIT_NO_STAGING_NO_COMMIT_RUNTIME_WRITES_NOT_ADMITTED"))process.exit(1);process.exit(0);'
PASS: exit 0

CHECK_03_PRE_CAPTURE_TASK_STATUS_BASELINE
CMD: git status --porcelain --untracked-files=all > /tmp/CORE-A4-YALKEN-LADDER-03-04-05-RECON-COMMIT-001.prestatus && echo OK
PASS: OUT == OK

CHECK_04_PRE_CAPTURE_HEAD_BASELINE
CMD: git rev-parse HEAD > /tmp/CORE-A4-YALKEN-LADDER-03-04-05-RECON-COMMIT-001.head && echo OK
PASS: OUT == OK
...
CMD: node -e 'const fs=require("node:fs");const {execSync}=require("node:child_process");const allow=new Set(process.argv.slice(1));if(!allow.size){console.error("ALLOWLIST is empty");process.exit(2);}const baselinePath="/tmp/CORE-A4-YALKEN-LADDER-03-04-05-RECON-COMMIT-001.prestatus";if(!fs.existsSync(baselinePath)){console.error("Missing baseline status snapshot");process.exit(1);}const parse=(txt)=>{if(!txt.trimEnd()) return new Set();return new Set(txt.trimEnd().split("\\n").map((line)=>{const p=line.slice(3);const parts=p.split(" -> ");return parts[parts.length-1];}));};const before=parse(fs.readFileSync(baselinePath,"utf8"));const after=parse(execSync("git status --porcelain --untracked-files=all",{encoding:"utf8"}));for(const p of after){if(before.has(p)) continue;if(!allow.has(p)){console.error(`Disallowed delta: ${p}`);process.exit(1);}}process.exit(0);' docs/tasks/CORE-A4-YALKEN-LADDER-03-04-05-RECON-COMMIT-001.md docs/tasks/CORE-A4-YALKEN-LADDER-03-04-05-RECON-001.md docs/OPS/STATUS/CONTOUR_03_04_05_ORDER_RECONCILIATION_DECISION_RECORD_V1.json
PASS: exit 0
...
- LINE_01: CORE-A4-YALKEN-LADDER-03-04-05-RECON-COMMIT-001
- LINE_02: SCOPE CORE-A4-YALKEN-LADDER-03-04-05-RECON-COMMIT-001 CORE-A4-YALKEN-LADDER-03-04-05-RECON-001 CONTOUR_03_04_05_ORDER_RECONCILIATION_DECISION_RECORD_V1
