...
Owner reboot pause retained one failed large C3 candidate with zero credit: Word returned a target-range mismatch before changing the primary document. The original source and failed returned bytes were identical. After explicit owner resume, a read-only range diagnostic and the same property-then-text native sequence passed on the retained source; the transient failure was not reproduced or declared fixed. The existing adapter now retains the actual mismatching range, owned document identity and bounded prefix in its failure, and the existing timing wrapper marks a returned provider failure as FAIL. Exact range matching remains required before every edit. Full fresh native acceptance, not this diagnostic, determines credit.
