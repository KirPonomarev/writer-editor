...
## MICRO_GOAL
Выполнить один narrow commit только для single-file cleanup в `CORE-A4-YALKEN-LADDER-03-04-05-RECON-OUTCOME-COMMIT-001.md` плюс одного cleanup commit-task artifact. Этот task использует exact-path staging only, создает ровно один commit, не делает outcome patch, не создает новые terminal records и не открывает runtime admission.

## ENTRY_CRITERIA
- `CORE-A4-YALKEN-LADDER-03-04-05-RECON-OUTCOME-COMMIT-001.md` exists and stays `NARROW_COMMIT_ONLY_RUNTIME_WRITES_NOT_ADMITTED`
- target file contains `CHECK_01_PRE_OUTCOME_SCOPE_SELF_CONTAINED`
- target file contains no live reference to `CORE-A4-YALKEN-LADDER-03-04-05-RECON-OUTCOME-PRECOMMIT-001.md`
- HEAD stays `27ccc0ba89334ef90038e40bddc491ea802d4fba` before this batch starts
...
- TARGET_TASK_BASENAME: CORE-A4-YALKEN-LADDER-03-04-05-RECON-OUTCOME-COMMIT-001.md
...
- no post-commit outcome patch inside this batch
...
7) Остановиться; post-commit patch не делать в этом task.

## CHECKS
CHECK_01_PRE_TARGET_SELF_CONTAINED_PASS
...
CMD: node -e 'const fs=require("node:fs");const {execSync}=require("node:child_process");const allow=new Set(process.argv.slice(1));const baselinePath="/tmp/CORE-A4-YALKEN-LADDER-03-04-05-RECON-OUTCOME-COMMIT-CLEANUP-001.prestatus";if(!fs.existsSync(baselinePath))process.exit(1);const parse=(txt)=>{if(!txt.trimEnd()) return new Set();return new Set(txt.trimEnd().split("\\n").map((line)=>{const p=line.slice(3);const parts=p.split(" -> ");return parts[parts.length-1];}));};const before=parse(fs.readFileSync(baselinePath,"utf8"));const after=parse(execSync("git status --porcelain --untracked-files=all",{encoding:"utf8"}));for(const p of after){if(before.has(p)) continue;if(!allow.has(p))process.exit(1);}process.exit(0);' docs/tasks/CORE-A4-YALKEN-LADDER-03-04-05-RECON-OUTCOME-COMMIT-CLEANUP-001.md docs/tasks/CORE-A4-YALKEN-LADDER-03-04-05-RECON-OUTCOME-COMMIT-001.md
PASS: exit 0

CHECK_06_POST_STAGED_SCOPE_EQUALS_ALLOWLIST
CMD: node -e 'const {execSync}=require("node:child_process");const want=[...process.argv.slice(1)].sort();const got=execSync("git diff --cached --name-only",{encoding:"utf8"}).trim().split("\\n").filter(Boolean).sort();if(JSON.stringify(want)!==JSON.stringify(got))process.exit(1);process.exit(0);' docs/tasks/CORE-A4-YALKEN-LADDER-03-04-05-RECON-OUTCOME-COMMIT-CLEANUP-001.md docs/tasks/CORE-A4-YALKEN-LADDER-03-04-05-RECON-OUTCOME-COMMIT-001.md
PASS: exit 0
...
- LINE_02: SCOPE CORE-A4-YALKEN-LADDER-03-04-05-RECON-OUTCOME-COMMIT-CLEANUP-001 CORE-A4-YALKEN-LADDER-03-04-05-RECON-OUTCOME-COMMIT-001
...
- Любая попытка patch любого terminal record внутри этого task → STOP.
...
- Никаких second commit or terminal patch actions inside this batch.
