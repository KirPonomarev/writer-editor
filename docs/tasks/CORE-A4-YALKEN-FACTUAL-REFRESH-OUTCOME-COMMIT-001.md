TASK_ID: CORE-A4-YALKEN-FACTUAL-REFRESH-OUTCOME-COMMIT-001
MILESTONE: A4
TYPE: CORE
...
## MICRO_GOAL
Выполнить один narrow commit только для outcome pair `CORE-A4-YALKEN-FACTUAL-REFRESH-OUTCOME-001.md` и `CURRENT_LANE_FACTUAL_DOC_REFRESH_COMPLETE_RECORD_V1.json` плюс одного commit-task artifact. Этот task использует exact-path staging only, создает ровно один commit, не патчит terminal record outcome fields после commit в этом же batch, не делает второй commit и не открывает runtime admission.

## ENTRY_CRITERIA
- outcome target task exists and stays `POST_COMMIT_OUTCOME_PATCH_ONLY_NO_STAGING_NO_COMMIT_RUNTIME_WRITES_NOT_ADMITTED`
- target record preserves factual refresh contract fields and already carries commit outcome metadata for `b1c1b58936238373d6402a7d6fa41d6503519c1a`
- this task keeps no live dependency on any separate precommit scaffold file

## ARTIFACT
- docs/tasks/CORE-A4-YALKEN-FACTUAL-REFRESH-OUTCOME-COMMIT-001.md
...
- TARGET_TASK_BASENAME: CORE-A4-YALKEN-FACTUAL-REFRESH-OUTCOME-001.md
- TARGET_RECORD_BASENAME: CURRENT_LANE_FACTUAL_DOC_REFRESH_COMPLETE_RECORD_V1.json
- COMMIT_TASK_BASENAME: CORE-A4-YALKEN-FACTUAL-REFRESH-OUTCOME-COMMIT-001.md
...
CMD: node -e 'const fs=require("node:fs");const t=fs.readFileSync("docs/tasks/CORE-A4-YALKEN-FACTUAL-REFRESH-OUTCOME-001.md","utf8");const j=JSON.parse(fs.readFileSync("docs/OPS/STATUS/CURRENT_LANE_FACTUAL_DOC_REFRESH_COMPLETE_RECORD_V1.json","utf8"));if(!t.includes("STATUS: POST_COMMIT_OUTCOME_PATCH_ONLY_NO_STAGING_NO_COMMIT_RUNTIME_WRITES_NOT_ADMITTED"))process.exit(1);if(j.status!=="FACTUAL_REFRESH_COMPLETE_RUNTIME_WRITES_NOT_ADMITTED")process.exit(1);if(j.commitSha!=="b1c1b58936238373d6402a7d6fa41d6503519c1a")process.exit(1);process.exit(0);'
PASS: exit 0

CHECK_02_PRE_OUTCOME_TASK_AND_TARGET_RECORD_PASS
CMD: node -e 'const fs=require("node:fs");const t=fs.readFileSync("docs/tasks/CORE-A4-YALKEN-FACTUAL-REFRESH-OUTCOME-001.md","utf8");const j=JSON.parse(fs.readFileSync("docs/OPS/STATUS/CURRENT_LANE_FACTUAL_DOC_REFRESH_COMPLETE_RECORD_V1.json","utf8"));if(!t.includes("STATUS: POST_COMMIT_OUTCOME_PATCH_ONLY_NO_STAGING_NO_COMMIT_RUNTIME_WRITES_NOT_ADMITTED"))process.exit(1);if(j.status!=="FACTUAL_REFRESH_COMPLETE_RUNTIME_WRITES_NOT_ADMITTED")process.exit(1);if(j.runtimeWritesAdmitted!==false)process.exit(1);if(j.runtimeAdmissionGranted!==false)process.exit(1);if(j.formalCutoverClaimed!==false)process.exit(1);if(j.broadShellAdmissionClaimed!==false)process.exit(1);if(j.commitSha!=="b1c1b58936238373d6402a7d6fa41d6503519c1a")process.exit(1);process.exit(0);'
PASS: exit 0

CHECK_03_PRE_CAPTURE_TASK_STATUS_BASELINE
CMD: git status --porcelain --untracked-files=all > /tmp/CORE-A4-YALKEN-FACTUAL-REFRESH-OUTCOME-COMMIT-001.prestatus && echo OK
PASS: OUT == OK

CHECK_04_PRE_CAPTURE_HEAD_BASELINE
CMD: git rev-parse HEAD > /tmp/CORE-A4-YALKEN-FACTUAL-REFRESH-OUTCOME-COMMIT-001.head && echo OK
PASS: OUT == OK
...
CMD: node -e 'const fs=require("node:fs");const {execSync}=require("node:child_process");const allow=new Set(process.argv.slice(1));const baselinePath="/tmp/CORE-A4-YALKEN-FACTUAL-REFRESH-OUTCOME-COMMIT-001.prestatus";if(!fs.existsSync(baselinePath))process.exit(1);const parse=(txt)=>{if(!txt.trimEnd()) return new Set();return new Set(txt.trimEnd().split("\\n").map((line)=>{const p=line.slice(3);const parts=p.split(" -> ");return parts[parts.length-1];}));};const before=parse(fs.readFileSync(baselinePath,"utf8"));const after=parse(execSync("git status --porcelain --untracked-files=all",{encoding:"utf8"}));for(const p of after){if(before.has(p)) continue;if(!allow.has(p))process.exit(1);}process.exit(0);' docs/tasks/CORE-A4-YALKEN-FACTUAL-REFRESH-OUTCOME-COMMIT-001.md docs/tasks/CORE-A4-YALKEN-FACTUAL-REFRESH-OUTCOME-001.md docs/OPS/STATUS/CURRENT_LANE_FACTUAL_DOC_REFRESH_COMPLETE_RECORD_V1.json
PASS: exit 0

CHECK_07_POST_STAGED_SCOPE_EQUALS_ALLOWLIST
CMD: node -e 'const {execSync}=require("node:child_process");const want=[...process.argv.slice(1)].sort();const got=execSync("git diff --cached --name-only",{encoding:"utf8"}).trim().split("\\n").filter(Boolean).sort();if(JSON.stringify(want)!==JSON.stringify(got))process.exit(1);process.exit(0);' docs/tasks/CORE-A4-YALKEN-FACTUAL-REFRESH-OUTCOME-COMMIT-001.md docs/tasks/CORE-A4-YALKEN-FACTUAL-REFRESH-OUTCOME-001.md docs/OPS/STATUS/CURRENT_LANE_FACTUAL_DOC_REFRESH_COMPLETE_RECORD_V1.json
PASS: exit 0

CHECK_08_POST_HEAD_MOVED_BY_EXACTLY_ONE_COMMIT
CMD: node -e 'const fs=require("node:fs");const {execSync}=require("node:child_process");const before=fs.readFileSync("/tmp/CORE-A4-YALKEN-FACTUAL-REFRESH-OUTCOME-COMMIT-001.head","utf8").trim();const after=execSync("git rev-parse HEAD",{encoding:"utf8"}).trim();if(!before||!after||before===after)process.exit(1);const count=execSync(`git rev-list --count ${before}..${after}`,{encoding:"utf8"}).trim();if(count!=="1")process.exit(1);process.exit(0);'
PASS: exit 0
...
- LINE_01: CORE-A4-YALKEN-FACTUAL-REFRESH-OUTCOME-COMMIT-001
- LINE_02: SCOPE CORE-A4-YALKEN-FACTUAL-REFRESH-OUTCOME-COMMIT-001 CORE-A4-YALKEN-FACTUAL-REFRESH-OUTCOME-001 CURRENT_LANE_FACTUAL_DOC_REFRESH_COMPLETE_RECORD_V1
